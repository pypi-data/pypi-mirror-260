...
class BoxedIntsSubconfig:
    some_boxed_int_1: BoxedInt = with_default(BoxedInt.from_str, default=BoxedInt(17))
    some_boxed_int_2: BoxedInt = required(BoxedInt.from_str)


@config(namespace=("ns1", "sub-ns1"))
class NamespacedSubconfig:
...
class FrameworkConfig:
    ...
    boxed_ints: BoxedIntsSubconfig = field(default_factory=BoxedIntsSubconfig)
...
class ApplicationConfig:
    ...
    namespaced_subconfig: NamespacedSubconfig = field(
        default_factory=NamespacedSubconfig
    )
    non_namespaced_subconfig: NonNamespacedSubconfig = field(
        default_factory=NonNamespacedSubconfig
    )
    framework_config: FrameworkConfig = field(default_factory=FrameworkConfig)
...
if __name__ == "__main__":
    ...
    with patch.dict(os.environ, env):
        pprint.pprint(ApplicationConfig())
