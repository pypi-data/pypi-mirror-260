...
from ..elements.entities import (  # noqa E402
    ...
    def _create_statement(self, ent, pad):
        ...
        if isinstance(pad['_return'], str):
            if pad['_return'] == '_items':
                ...
                if isinstance(ent, N) and ent.label == pad['_return'] and ent._var:
                    a.append(ent)
                elif isinstance(ent, R) and ent.Type == pad['_return'] and ent._var:
                    a.append(ent)
                else:
                    a.extend([x for x in ent.nodes()
                              if x.label == pad['_return'] and x._var])
                if not a:
                    self.error = {
            ...
            if retblock.get('_nodes'):
                ...
                if '*' in labels:
                    a.append('*')
                elif (isinstance(ent, N) and ent._var):
                    ...
                    for n in ent.nodes():
                        if n.label in labels and n._var:
                            if labels[n.label]:
            ...
            if retblock.get('_edges'):
                ...
                if isinstance(ent, R) and ent._var:
