...
class DynamoModel(BaseModel, Generic[T]):
    ...
    @classmethod
    def batch_get(cls: Type[T], id_list: List[str], consistent_read: bool = True) -> List[Optional[T]]:
        items = []
        futures = []
        with ThreadPoolExecutor(max_workers=32) as worker:
            for item_id in id_list:
                future = worker.submit(cls.__dynamo__.get_item, cls._table_name(), item_id, consistent_read)
