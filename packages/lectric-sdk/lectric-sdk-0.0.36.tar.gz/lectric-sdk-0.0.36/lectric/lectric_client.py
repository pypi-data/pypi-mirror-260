...
class LectricClient:
    ...
    @propagate_exception
    def get_trashcan_entries(self, deleter: str=None, exact: bool=True, start_date: date=None, end_date: date=None):
        if not exact:
            raise NotImplementedError("Getting trash can entries is not supported for approx.")
        else:
            return trashcan_entries_exact_trashcan_entries_get.sync_detailed(client=self.client, deleter=deleter, start_date=start_date, end_date=end_date)
