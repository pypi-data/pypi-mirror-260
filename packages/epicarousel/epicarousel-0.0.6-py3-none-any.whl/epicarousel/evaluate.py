...
def cluster_evaluation(adata_obs, label_key, cluster_key):
    ...
    '''
    from sklearn.metrics import adjusted_rand_score, adjusted_mutual_info_score, normalized_mutual_info_score, homogeneity_score, completeness_score, fowlkes_mallows_score
    ...
    FMI = fowlkes_mallows_score(adata_obs[label_key], adata_obs[cluster_key])
    
    print('AMI:%.3f\tARI:%.3f\tNMI:%.3f\tHomo:%.3f\tCS:%.3f\tFMI:%.3f\t'%(AMI,ARI,NMI,HOM,completeness,FMI))
    return AMI, ARI, NMI, HOM, completeness, FMI
