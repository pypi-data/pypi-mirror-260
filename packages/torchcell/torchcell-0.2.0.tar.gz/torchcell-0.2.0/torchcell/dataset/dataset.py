# torchcell/dataset/dataset
# [[torchcell.dataset.dataset]]
# https://github.com/Mjvolk3/torchcell/tree/main/torchcell/dataset/dataset
# Test file: tests/torchcell/dataset/test_dataset.py
...
class Dataset(torch.utils.data.Dataset, ABC):
    ...
    def _infer_num_classes(self, y: Tensor | None) -> int:
        if y is None:
            return 0
    ...
    def __getitem__(
        self, idx: int | np.integer | IndexType
...
def files_exist(files: list[str]) -> bool:
    ...
    with ThreadPoolExecutor() as executor:
        results = list(
            tqdm(
                executor.map(check_file_existence, files),
                total=num_files,
                desc="Checking files",
            )
        )
