...
def patch_promplate():
    ...
    with suppress(ModuleNotFoundError):
        ...
        def patch_args(func: FunctionType):
            def f(*args, **kwargs):
                return func(*(to_js(i) for i in args), **{i: to_js(j) for i, j in kwargs.items()})

            return FunctionType(f.__code__, func.__globals__, func.__name__, f.__defaults__, f.__closure__)

        for cls in o.v1.__dict__.values():
            if isclass(cls):
                for name, func in [*cls.__dict__.items()]:
                    if isfunction(func):
                        setattr(cls, name, patch_args(func))
