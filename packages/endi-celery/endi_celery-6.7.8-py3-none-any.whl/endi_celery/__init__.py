...
from endi_celery.tasks.export import MODELS_CONFIGURATION as EXPORT_MODELS_CONFIGURATION
...
def configure_accounting_parser_service(config, settings):
    ...
    for interface, default_module_path, setting_key in (
        (
            ...
            "endi_celery.parsers.sage.parser_factory",
            "endi_celery.interfaces.IAccountingFileParser",
        ),
        (
            IAccountingOperationProducer,
            "endi_celery.parsers.sage.producer_factory",
            "endi_celery.interfaces.IAccountingOperationProducer",
        ),
    ):
...
def worker(global_config, **settings):
    ...
    from endi import prepare_config
