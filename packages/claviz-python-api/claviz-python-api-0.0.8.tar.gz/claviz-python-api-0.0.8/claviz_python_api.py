...
def get_claviz_token(url: str, username: str, password: str) -> str:
    ...
    token_response = requests.post(
        f"{claviz_id_authority}protocol/openid-connect/token",
        data={
            "client_id": claviz_id_app_name,
            "grant_type": "password",
            "username": username,
            "password": password,
        }
...
class ClavizClient:
    ...
    def get_fact_data(self, fact_id: str) -> any:
        response = self.session.get(
            f"{self.url}/api/facts/fact-data?factId={fact_id}"
        )
        return response.json()
    ...
    def get_started_background_functions(self) -> List[any]:
        response = self.session.get(
            f"{self.url}/api/functionsManager/background"
        )
        return response.json()
    ...
    def import_facts(self, collection_id: str, facts: List[Dict[str, Any]]) -> None:
        response = self.session.post(
            f"{self.url}/api/facts/import?collectionId={collection_id}&verbose=true", json=facts
        )
        return response.json()
    ...
    def start_background_function(self, function_id: str) -> None:
        self.session.put(
            f"{self.url}/api/functionsManager/background/{function_id}/start"
        )
    ...
    def stop_background_function(self, function_id: str) -> None:
        self.session.put(
            f"{self.url}/api/functionsManager/background/{function_id}/stop"
        )
    ...
    def execute_function(self, function_id: str, parameters: Dict[str, Any] = {}, engine_name=None):
        ...
        response = self.session.post(
            f"{self.url}/api/functions/{function_id}", json=parameters
        )

        if response.status_code == 202:
            while True:
                function_instance_status = self.get_function_instance_status(
                    response.json()['functionInstanceId']
                )
                if function_instance_status['status'] == 'success' or function_instance_status['status'] == 'error':
                    self.destroy_function_instance(
                        response.json()['functionInstanceId']
                    )
    ...
    def get_function_instance_status(self, function_instance_id):
        response = self.session.get(
            f"{self.url}/api/functions/functionInstances/{function_instance_id}"
        )
        return response.json()
