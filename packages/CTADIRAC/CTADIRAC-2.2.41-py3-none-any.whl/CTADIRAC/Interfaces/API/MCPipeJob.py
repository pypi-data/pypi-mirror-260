...
class MCPipeJob(CTAJob):
    ...
    def set_only_corsika(self, only_corsika=False) -> None:
        ...
        if only_corsika is True:
            DIRAC.gLogger.info("Set simulations of CORSIKA only, without piping to sim_telarray")
    ...
    def set_random_mono_probability(self, random_mono_probability=None) -> None:
        ...
        if random_mono_probability is not None:
            if 0 <= float(random_mono_probability) <= 1:
                DIRAC.gLogger.info(f"Set random mono probability to: {random_mono_probability}")
                self.random_mono_probability = (
    ...
    def run_corsika_sim_telarray(self, i_step, debug=False) -> None:
        ...
        prod_args = (
            f"{prod_args} {self.site} {self.particle} {self.pointing_dir} {self.zenith_angle}"
        )

        cs_step = self.setExecutable(prod_exe, arguments=prod_args, logFile="CorsikaSimtel_Log.txt")
    ...
    def set_metadata_and_register_data(self, i_step) -> None:
        ...
        for element in itertools.product(*self.output_file_metadata.values()):
            ...
            file_meta_data, moon_str, div, div_str = self.build_file_metadata(combination)
            ...
            self.verify_number_of_simtel_events(data_output_pattern, log_str, i_step, i_substep)
            i_substep += 1
...
class MCPipeMuonJob(MCPipeJob):
    ...
    def set_metadata_and_register_data(self, i_step) -> None:
        ...
        for degraded in self.degraded_values:
            ...
            self.verify_number_of_simtel_events(data_output_pattern, "", i_step, i_substep)
            i_substep += 1
            ...
            if self.half_moon == "--with-half-moon":
                ...
                file_meta_data_json = json.dumps(file_meta_data)
                data_output_pattern = f"Data/*moon-ref-degraded-{degraded:.2f}.simtel.zst"
                ...
                log_file_pattern = f"Data/*moon-ref-degraded-{degraded:.2f}.log_hist.tar"
                self.upload_and_register_log(
                    meta_data_json,
                    file_meta_data_json,
                    log_file_pattern,
