...
class TranscodingProfileGenerator:
    config = dict(preset="veryfast", schema=1, framerate=25)
    ...
    def generate(self, renditions, packaging):
        if self.config["schema"] == 1:
            return self.generate_v1(renditions, packaging)
        elif self.config["schema"] == 2:
            return self.generate_v2(renditions, packaging)
        else:
            raise Exception("Unknown profile version")

    def generate_v1(self, renditions, packaging):
        common = {
            "codecv": "h264",
    ...
    def generate_v2(self, renditions, packaging):
        ...
        if video_frame_rate is None:
            ...
            self.messages.append(
                WarningMessage(f"Default video framerate of {video_frame_rate} was selected, since it could not be detected in the source"))
        ...
        min_gop_size = self.calculate_min_gop_size(video_frame_rate, audio_sample_rate)
        (
            selected_duration,
            candidate_durations,
        ) = self.calculate_recommended_durations(
        ...
        if not any(d for d in candidate_durations if d[0] == target_segment_duration):
            dur_strings = "\n - " + "\n - ".join(
                [f'"{s}" (= {g} frames = {d:.3f}s)' for g, d, s in candidate_durations]
            )
            self.messages.append(
                InfoMessage(
                    f"The target segment duration (of {target_segment_duration}s) will cause audio and video segments to not align perfectly, which could cause issues during manifest manipulation.\n"
        ...
        for i, r in enumerate(video_renditions):
            rung = {
                ...
                # -2 seems to ensure that the calculated width is divisible by 2, a constraint of libx264
                "scale": {"width": -2, "height": r["resolution"][1]},
                "bitrate": r["bitrate"],
            }
        ...
        packaging_options = {}
        packaging_options["hls_client_manifest_version"] = packaging.get("version", 4)
