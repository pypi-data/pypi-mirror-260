...
class Db:
    ...
    def __init__(self, settings: DbSettingsABC):
        ...
        self.__redis_pool = ConnectionPool.from_url(settings.redis_uri) if settings.redis_uri is not None else None
        self.__sql_read_engine = create_engine(
            ...
            pool_use_lifo=True,
            echo=False,
        ) if settings.sql_read_uri is not None and settings.sql_uri is not None else None
        self.__sql_write_engine = create_engine(
            ...
            pool_use_lifo=True,
            echo=False
        ) if settings.sql_write_uri is not None and settings.sql_uri is not None else None
    ...
    def get_mongo(self) -> Optional[MongoClient]:
        ...
        return MongoClient(self.__settings.mongo_db_uri)
