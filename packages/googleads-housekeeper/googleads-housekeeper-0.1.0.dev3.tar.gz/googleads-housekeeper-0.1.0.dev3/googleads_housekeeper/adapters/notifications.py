...
class MessagePayload:
    ...
    """
    task_name: str
    placements_excluded_sample: GaarfReport | None
...
class GoogleCloudAppEngineEmailNotifications(BaseNotifications):
    ...
    def send(self, payload: MessagePayload, **kwargs: str):
        ...
        if placements_excluded := payload.placements_excluded_sample:
            ...
            body = self.generate_notification_html(task_name,
                                                   placements_excluded)
        else:
    ...
    def generate_notification_html(task_name, placements):
        html_body = """
        <!DOCTYPE html>
        <html lang="en">
        <head>
        ...
        html_body += f"""
                <p>The CPR tool has excluded the following placements based on your defined thresholds:</p>
                <p><strong>Task Name:</strong>{task_name}</p>
                <p><strong>Formula:</strong> impressions > 10 and clicks = 0 (<a href="[Direct Task Link]" class="task-link">View Task</a>)</p>"""

        for placement in placements:
            exclusion_reason = getattr(placement, "reason", "")
            html_body += f"""
                <ul class="placement-list">
                    <li><strong>Placement Name:</strong> {placement["name"]} (<a href="[Placement Link]">{placement["placement"]}</a>) - 
    ...
    def send(self, payload: MessagePayload) -> None:
        ...
        if not (placements_excluded_sample :=
            ...
            self.client.files_upload(
                channels=channel,
                initial_comment=
                f'{task_name}: {payload.total_placement_excluded} placements excluded',
