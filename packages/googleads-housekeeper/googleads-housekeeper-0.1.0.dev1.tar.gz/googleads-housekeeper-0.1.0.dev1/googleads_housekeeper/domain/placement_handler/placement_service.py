...
class PlacementExcluder:
    ...
    def _create_placement_operation(
        ...
        if (placement_info.campaign_type in self.associable_with_negative_lists
                and exclusion_level in (enums.ExclusionLevelEnum.CAMPAIGN,
        ...
        if placement_info.placement_type == enums.PlacementTypeEnum.YOUTUBE_VIDEO.name:
...
def find_placements_for_exclusion(
    ...
    for account in task.accounts:
        account_allowlisted_placements = views.allowlisted_placements_as_tuples(
            uow, account)
        placements = get_placements_for_account(account, task,
                                                specification,
                                                report_fetcher)
    ...
    for placement in placements:
        if has_allowlisted_placements:
            placement['allowlisting'] = (
                ...
                placement.account_id) in allowlisted_placements
        else:
