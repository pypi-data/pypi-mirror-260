...
class DataSource:
                            ...
                            if result:
                                ...
                                if q.isValid():
                                    ...
                                    if totalRows > 0:
                                        ...
                                        select = "SELECT * FROM automatic_data WHERE id > {}".format(newestRecordID)
                                        result = q.exec(select)
                                        if result:
                                            progress = 0
                                            progressPercent = 0
        ...
        if self._db is not None:
            ...
            if result:
                ...
                if 'sidereal_utc' not in self._adf.columns:
                    ...
                    self._adf['sidereal_utc'] = self._adf.apply(lambda x: timestamp2sideral(x['timestamp_ms']), axis=1)
    ...
    def _saveClassifications(self):
        if self._db is not None and self._adf is not None and self._dataChangedInMemory is True:
            ...
            rows = []
            for rowNr in range(0, len(self._parent.eventDataChanges)):
                if self._parent.eventDataChanges[rowNr] is True:
                    rows.append(rowNr)
            ...
            for rid in rows:
                ...
                q.prepare("UPDATE automatic_data SET classification = ?, attributes = ? WHERE id=? AND event_status=?")
                q.addBindValue(row['classification'].values[0])
                q.addBindValue(row['attributes'].values[0])
                q.addBindValue(rid)
    ...
    def dailyByClassification(self, df: pd.DataFrame, filters: str, dateFrom: str = None, dateTo: str = None,
                              considerBackground: bool = False, totalRow: bool = True,
    ...
    def getCfgRevisionFromID(self, eventId: int):
        ...
        if self._adf is not None:
            ...
            revision = self._adf.loc[(self._adf['id'] == eventId) & (self._adf['event_status'] == 'Fall'), ['revision']].iloc[0,0]
        return revision
    ...
    def classifyEvents(self, fromID: int, toID: int, overwrite: bool = False):
        ...
        if df is not None:
            ...
            subsetPeak = peakRecords.loc[:, ['id', 'up_thr', 'S', 'avgS', 'N', 'diff', 'avg_diff', 'diff_start', 'diff_end', 'std_dev']]
            ...
            for idx in fallRecords.index:
                ...
                if cla == '':
                    ...
                    stdDev = subsetPeak.loc[idp, 'std_dev']
                    ...
                    if ESDfilter != 0:
                        ...
                        # (lightings).
                        # The reference band S-N,  expressed as the average of S-N values read at the begin (left)
    ...
    def attributeEvents(self, fromID: int, toID: int, overwrite: bool = False):
        ...
        df = self.getADpartialFrame(idFrom=fromID, idTo=toID)
        totalIds = int(len(df.index) / 3)
        self._parent.updateStatusBar("updating attributes on {} events".format(totalIds))
        progress = 0
        progressPercent = 0
        self._parent.updateProgressBar(progressPercent)
        currentId = 0
        ...
        if df is not None:
            ...
            for idx in fallRecords.index:
                ...
                if attribs == '' or overwrite:
                    ...
                    # attributes are stored in adf['attributes'] as a json string, in the Fall record

                    self._adf.loc[mask, 'attributes'] = self._adf.loc[mask, 'attributes'] + " " + str(myId)
