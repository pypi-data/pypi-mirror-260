...
from aiokafka.errors import (
    ...
    OperationNotAttempted, StaleMetadata)
...
class Sender:
    ...
    async def _maybe_wait_for_pid(self):
        ...
        while True:
            ...
            if self._txn_manager.transactional_id is not None:
                cooridnator_type = CoordinationType.TRANSACTION
                node_id = await self._find_coordinator(
                    CoordinationType.TRANSACTION,
                    self._txn_manager.transactional_id)
            else:
                cooridnator_type = CoordinationType.GROUP
                node_id = self.client.get_random_node()
            success = await self._do_init_pid(node_id, cooridnator_type)
            if not success:
    ...
    async def _find_coordinator(self, coordinator_type, coordinator_key):
        ...
        while True:
            try:
                ...
                raise err
            except (Errors.CoordinatorNotAvailableError, Errors.NodeNotReadyError):
                await self.client.force_metadata_update()
    ...
    async def _do_init_pid(self, node_id, coorinator_type):
        handler = InitPIDHandler(self)
        return (await handler.do(node_id, coorinator_type))
    ...
    async def _do_add_partitions_to_txn(self, tps):
        ...
        return (await handler.do(node_id, CoordinationType.TRANSACTION))
    ...
    async def _do_txn_offset_commit(self, offsets, group_id, fut):
        ...
        return (await handler.do(node_id, CoordinationType.GROUP))
...
class BaseHandler:
    ...
    async def do(self, node_id, coordinator_type=None):
        req = self.create_request()
        if coordinator_type is None:
            raise Exception(f"`coordinator_type` required in do()")
        try:
            resp = await self._sender.client.send(
                node_id, req, group=self.group)
        except (Errors.NodeNotReadyError, StaleMetadata) as stale_err:
            log.warning("Could not send %r: %r", req.__class__, stale_err)
            await asyncio.sleep(self._default_backoff, loop=self._loop)
            self._sender._coordinator_dead(coordinator_type)
            return False            
