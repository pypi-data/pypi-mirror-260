...
class TagDescriptor(Generic[T]):
    def __init__(
        self,
        method: Callable[[T], Tag | TagContextManager],
    ...
    def __get__(self, instance: Union[T, None], owner: Type[T]) -> TagContextManager:
        if instance is None:
            raise AttributeError(f"{self._method_name} is only accessible on instances of {owner.__name__}")
        ...
        if tag is None:
            ...
            if not isinstance(tag, TagContextManager):
                tag = TagContextManager(tag, instance._html)  # type: ignore (we need to access private property)
            ...
            if self._method_name not in instance._tags_called:  # type: ignore (we need to access private property)
                instance._tags_called.append(self._method_name)  # type: ignore (we need to access private property)
