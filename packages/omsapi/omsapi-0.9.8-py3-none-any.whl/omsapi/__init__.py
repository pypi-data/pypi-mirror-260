...
class OMSQuery(object):
    ...
    def __init__(self, base_url, resource, verbose, cookies, oms_auth, tsg_auth, cert_verify, throw_on_err, retry_on_err_sec, proxies):
...
class OMSMetaQuery(object):
    ...
    def __init__(self, base_url, verbose, cookies, oms_auth, tsg_auth, cert_verify, retry_on_err_sec, proxies):
        self.attribute_validation = True
        self.base_url = base_url
        self.verbose = verbose
        self.cookies = cookies
...
class OMSAPI(object):
    ...
    def query(self, resource, query_validation=True):
        ...
        q = OMSQuery(self.base_url, resource=resource, verbose=self.verbose,
                     cookies=self.cookies, oms_auth=self.oms_auth, tsg_auth=self.tsg_auth,cert_verify=self.cert_verify, throw_on_err=self.throw_on_err, retry_on_err_sec=self.err_sec, proxies=self.proxies)
    ...
    def query_metadata(self, query_validation=True):
        ...
        q = OMSMetaQuery(self.api_url, verbose=self.verbose,
                     cookies=self.cookies, oms_auth=self.oms_auth, tsg_auth=self.tsg_auth, cert_verify=self.cert_verify, retry_on_err_sec=self.err_sec, proxies=self.proxies)
