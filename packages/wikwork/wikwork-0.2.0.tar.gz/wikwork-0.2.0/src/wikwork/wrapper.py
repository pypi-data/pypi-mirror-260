    ...
    if output_words_filename:
        with open (output_words_filename, 'w', encoding='utf-8') as csvfile:
            ...
            if (headword_list
                ...
                is_german = True
                entryvars = ['word_separation', 'pronunciations','definitions',
'origins','synonyms','opposites', 'hyponyms','hypernyms', 'examples',
'expressions','word_combos', 'word_formations', 'refs_and_more_info','sources',
'abbreviations', 'sayings', 'adj_comp','adj_comp2','adj_super','adj_super2',
'verb_present_1s','verb_present_2s','verb_present_3s', 'verb_pret_1s',
'verb_subj_ii','verb_imper_s','verb_imper_p', 'verb_past_part', 'verb_helper',
'noun_gender','noun_nom_p', 'noun_gen_s','noun_adj_decl']
                flathead.extend( colnm + '1' for colnm in entryvars )
            else:
                is_german = False
            ...
            for row in headword_list:
                ...
                if is_german and row.entries:
                    flattened.extend(getattr(row.entries[0],colnm)
                        for colnm in entryvars)
                elif is_german:
                    flattened.extend('' for colnm in entryvars)
...
def words_wrapper(input_words_filename: str,
    ...
    output_words_filename : str, optional
        ...
        of columns are all the attributes for the first GermanEntry in
        the entries list (if the objects are GermanWord objects), where the
        column name is the attribute name with a '1' appended. The last set
        of columns are from the audio_files list: `filename`, `prob_license`,
        `prob_author`, `revision`, and `download_status` attributes of each
        item in `Headword.audio_files` (column headers have a counter
        starting at 1 appended to the variable name ['filename1',
        'prob_license1', ...]
    output_audios_filename : str, optional
    ...
    Returns
    -------
    A list of `page_audio.Headword` objects or german.GermanWord objects, one
    ...
    for i, word in enumerate(input_list):
        if headword_lang_code not in ['de']:
