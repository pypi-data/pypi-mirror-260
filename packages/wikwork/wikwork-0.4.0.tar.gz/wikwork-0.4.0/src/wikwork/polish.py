...
def _parse_verb_template(self: PolishWord, line: str,
    ...
    if res:
        ...
        wiki_temp, err_msg = parse._parse_one_template(template)
        ...
        if aspect == 'imperfective' and not current_entry.verb_aspect:
            ...
            regular_keys = ['dokonany','koniugacja','robić','robię','robi',
            'robią','robiłem','robił','robiła','robili','robiono','rób',
            'robiąc','robiony','robieni','robienie']
            set_diff = set(temp_dict.keys()) - set(regular_keys)
            if len(set_diff) > 0:
                current_entry.verb_irr_temp = True
                logger.info('Word %s: verb irreg, has keys: %s',
        ...
        if aspect == 'perfective' and not current_entry.verb_aspect:
            ...
            regular_keys = ['dokonany','koniugacja','zrobić','zrobię','zrobi',
            'zrobią','zrobiłem','zrobił','zrobiła','zrobili','zrobiono','zrób',
            'zrobiwszy','zrobiony', 'zrobieni','zrobienie']
            set_diff = set(temp_dict.keys()) - set(regular_keys)
            if len(set_diff) > 0:
                current_entry.verb_irr_temp = True
                logger.info('Word %s: verb irreg, has keys: %s',
...
#        '{{XXXX}}' : 'word_separations',
    ...
    parse._parse_multi(self, temp_to_attrib_dict=temp_to_attrib_dict,
        ...
        in_target_add_entry=in_target_sect2,
