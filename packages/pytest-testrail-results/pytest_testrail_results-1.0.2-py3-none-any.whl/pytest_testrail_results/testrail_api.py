...
# Learn more:
#
# https://support.testrail.com/hc/en-us/categories/7076541806228-API-Manual
#
...
class APIClient:
    def __init__(self, base_url, user, password, **kwargs):  # type: ignore
        """
    ...
    def send_get(self, uri, **kwargs):  # type: ignore
        """
        Send GET
    ...
    def send_post(self, uri, data, **kwargs):  # type: ignore
        """
        Send POST

        Issues a POST request (write) against the API and returns the result (as Python dict).

        :param uri: The API method to call including parameters (e.g., get_case/1) is
        :type uri: str
    ...
    @staticmethod
    def get_error(json_response):  # type: ignore
        """Extract error contained in an API response.
