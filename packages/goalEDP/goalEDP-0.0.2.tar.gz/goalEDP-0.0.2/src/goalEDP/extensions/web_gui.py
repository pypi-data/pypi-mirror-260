    ...
    def __init__(self, explainer: Explainer) -> None:
        ...
        async def parseEvents(reqData: Any) -> List[Event]:
            if ("events" in reqData):
                ...
                return await self.explainer.history.getAsync(reqData["filters"])
        ...
        @self.server.route('/history_get', methods=['POST'])
        async def historyGet():
            filters = request.json
            hist: list[Event] = await self.explainer.history.getAsync(filters)
