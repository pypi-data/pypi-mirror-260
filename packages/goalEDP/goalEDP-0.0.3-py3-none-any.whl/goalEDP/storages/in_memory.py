...
class InMemoryHistory(History):
    ...
    async def getAsync(self, filters: dict) -> List[Event]:
        res: list[Event] = []
        cursor = True
    ...
    async def countOutsAsync(self, handlers: List[EventHandler], minTime: int = 0, maxTime: int = sys.maxsize) -> int:
        ...
        for h in handlers:
            for topic in h.publishedTopics:
                topics.add(topic)
        events = await self.getAsync({'topics': list(topics), 'minTime': minTime, 'maxTime': maxTime})
        return len(events)
    ...
    async def addAsync(self, event: Event) -> None:
