...
class GenerateSourceType:
    ...
    def split_and_upload(self):
        if os.path.exists("/Users/bingtao/workspace"):
            path = '/Users/bingtao/workspace/MyDiary/farfarfun/funread/hubs'
        else:
            path = '/home/bingtao/workspace/hubs/funread-dat/hubs'

        with BookSourceDownload(path=path, cate1="book") as runner:
            data = runner.export_sources()
            step = len(data) // 9
            for i in tqdm(range(0, len(data), step)):
                self.drive.upload_file(content=data[i: i + 1000],
                                       git_path=f"{self.dir_path}/progress-{i / step + 1}.json")
