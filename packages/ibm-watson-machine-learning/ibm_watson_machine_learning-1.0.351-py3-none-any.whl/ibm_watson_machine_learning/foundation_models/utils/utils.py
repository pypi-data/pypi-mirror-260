...
class FMDeprecationWarning(UserWarning): ...


def _raise_foundation_models_deprecation_warning(name: str) -> None:
    warnings.warn(f"Importing from {name} "
                  "is deprecated and the modules it contains will not be supported in the future. "
                  f"Please import from {name.replace('ibm_watson_machine_learning', 'ibm_watsonx_ai')} instead.\n"
                  "To install ibm-watsonx-ai run `pip install -U ibm-watsonx-ai`.",
                  category=FMDeprecationWarning,
                  stacklevel=2
                )