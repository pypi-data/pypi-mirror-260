	...
	def recode_data(self, args=None):
		...
		if recode_expr is not None:
			...
			try:
				...
				if new_column:
					# Add a column.
					datawidths = [len(str(expr_values[i][1])) for i in range(len(expr_values))]
					datawidths.append(len(recode_col))
					colwidth = max(datawidths)
					ff = tkfont.nametofont("TkFixedFont")
					tblstyle = ttk.Style()
					tblstyle.configure('tblstyle', font=ff)
					charpixels = int(1.3 * ff.measure(u"0"))
					colhdrs = list(self.tbl["columns"])
					colwidths = [self.tbl.column(col, 'width') for col in colhdrs]
					hdr_attrs = {col: self.tbl.heading(col) for col in colhdrs}
					for col in hdr_attrs:
						del hdr_attrs[col]["state"]
					hdr_attrs[recode_col] = {"text": recode_col, "command": lambda _col=recode_col: treeview_sort_column(self.tbl, _col, False)}
					colhdrs.append(recode_col)
					colwidths.append(colwidth*charpixels)
					self.tbl["columns"] = tuple(colhdrs)
					for i, col in enumerate(colhdrs):
						self.tbl.column(col, width = colwidths[i], stretch=tk.NO)
					for col in hdr_attrs:
						self.tbl.heading(col, **hdr_attrs[col])
		...
		if self.plot_data is not None and len(self.plot_data) > 0 and len(self.plot_data[0]) > 0:
			if plot_type == "Count by category":
				...
				if self.groupby_var.get() == '':
					...
					pathlist = []
					for g in groups:
						xd = [self.plot_data[0][i] for i in range(datarows) if self.plot_data[2][i] == g]
						yd = [self.plot_data[1][i] for i in range(datarows) if self.plot_data[2][i] == g]
						grplbl = g
						if wrap_at_underscores:
							grplbl = g.replace("_", " ")
						grplbl = "\n".join(textwrap.wrap(grplbl, width=wrapwidth))
						pathlist.append(self.plot_axes.scatter(xd, yd, alpha=self.alpha, label=grplbl))
					paths = reduce(lambda x,y: x+y, (p.get_paths() for p in pathlist), tuple())
					splot = mpl_coll.PathCollection(paths)
