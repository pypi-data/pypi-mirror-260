...
class PredictWrapper(object):
    ...
    def transform(self, pre_transform_df: pd.DataFrame) -> pd.DataFrame:
        ...
        if isinstance(transformed_data, sparse.csr_matrix):
            # We convert 0 to None because when we densify a sparse matrix, the intrepretation of missing value
            # in the XGB model changes. Look at https://github.com/dmlc/xgboost/issues/1634
            return pd.DataFrame(
                transformed_data.toarray(), columns=post_transform_features
            ).replace({0: np.nan})
        else:
            return pd.DataFrame(
