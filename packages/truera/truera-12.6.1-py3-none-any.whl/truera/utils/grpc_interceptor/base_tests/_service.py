...
from truera.client.private.grpc import channel_creator
...
class ErroringHandler(Handler):
    ...
    def handle_unary_unary(self, request, servicer_context):
        ...
        return super(ErroringHandler,
                     self).handle_unary_unary(request, servicer_context)

    def handle_unary_stream(self, request, servicer_context):
        _set_error_code(servicer_context)
        return super(ErroringHandler,
                     self).handle_unary_stream(request, servicer_context)

    def handle_stream_unary(self, request_iterator, servicer_context):
        _set_error_code(servicer_context)
        return super(ErroringHandler, self
                    ).handle_stream_unary(request_iterator, servicer_context)

    def handle_stream_stream(self, request_iterator, servicer_context):
        _set_error_code(servicer_context)
        return super(ErroringHandler, self
                    ).handle_stream_stream(request_iterator, servicer_context)
...
class Service(object):
    ...
    def __init__(self, server_interceptors, handler=Handler()):
        self.handler = handler
        self._server_pool = logging_pool.pool(2)
        self._server = grpc_interceptor_base.intercept_server(
        ...
        self._server.start()
        self.channel = channel_creator.create_channel(
            'localhost:%d' % port, wait_for_connectivity=True
        )
