...
def _load_model_from_local_file(path):
    ...
    supported_spark_regression_models = [
    ...
    for model_type in supported_spark_regression_models:
        try:
    ...
    if not deserialized_model:
        raise ValueError(
            f"Model could not be loaded. Verify that model is one of {supported_spark_regression_models}"
        )
