...
class TestBotte:
    ...
    def test_handle_msg(self):
        ...
        @tb.handle_msg(commands=['help', 'start'])
    ...
    def test_handle_msg_reg(self):
        bot = Botte.Botte('')
        msg = self.create_text_message(r'https://web.telegram.org/')

        # noinspection PyUnusedLocal
        @bot.handle_msg(regexp=r'((https?):((//)|(\\\\))+([\w\d:#@%/;$()~_?\+-=\\\.&](#!)?)*)')
    ...
    def test_handle_msg_lambda(self):
        bot = Botte.Botte('')
        msg = self.create_text_message(r'lambda_text')

        # noinspection PyUnusedLocal
        @bot.handle_msg(func=lambda message: r'lambda' in message.text)
        def command_url(message):
            msg.text = 'got'
    ...
    def test_handle_msg_lambda_fail(self):
        bot = Botte.Botte('')
    ...
    def test_handle_msg_reg_fail(self):
        bot = Botte.Botte('')
    ...
    def test_send_audio(self):
        ...
        tb = Botte.Botte(TOKEN)
        ret_msg = tb.send_audio(CHAT_ID, file_data, duration = 1, performer='Kreaitorai', title='pyTelegram')
        assert ret_msg.content_type == 'audio'
        assert ret_msg.audio.performer == 'Kreaitorai'
    ...
    def test_extract_entity(self):
        entities_map = {"https://core.telegram.org/api/entities": "https://core.telegram.org/api/entities",
                       "https://github.com/Kreaitorai/botte": "https://github.com/Kreaitorai/botte",
...
let number = loop {
    ...
    def test_chat_commands(self):
        ...
        scope = Botte.types.BotCommandScopeChat(CHAT_ID)
        ret_msg = tb.set_my_commands([Botte.types.BotCommand(command, description)], scope, lang)
    ...
    def test_typed_middleware_handler(self):
        ...
        @tb.handle_msg(func=lambda m: m.text == 'got')
        def command_handler(message):
