...
from Botte.service_utils import is_pil_image, is_dict, is_string, is_bytes, chunks, generate_random_token, pil_image_to_file
...
class ThreadPool:
    ...
    def __init__(self, Botte, num_threads=2):
        self.Botte = Botte
    ...
    def on_exception(self, worker_thread, exc_info):
        if self.Botte.exception_handler is not None:
            handled = self.Botte.exception_handler.handle(exc_info)
        else:
            handled = False
        if not handled:
...
def user_link(user: types.User, include_id: bool = False) -> str:
    ...
    :type user: :obj:`Botte.types.User`
...
def webhook_google_functions(bot, request):
    ...
    :type bot: :obj:`Botte.Botte` or :obj:`Botte.async_Botte.AsyncBotte`
...
def antiflood(function: Callable, *args, number_retries=5, **kwargs):
    ...
    .. code-block:: python3
        ...
        from Botte.util import antiflood
    ...
    :return: None
    """
    from Botte.apihelper import ApiTelegramException
