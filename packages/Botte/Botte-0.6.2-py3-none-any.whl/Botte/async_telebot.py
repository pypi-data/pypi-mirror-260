...
# storages
from Botte.asyncio_storage import StateMemoryStorage, StatePickleStorage, StateStorageBase
from Botte.asyncio_handler_backends import BaseMiddleware, CancelUpdate, SkipHandler, State, ContinueHandling

from inspect import signature, iscoroutinefunction

from Botte import util, types, asyncio_helper
import asyncio
from Botte import asyncio_filters
...
class AsyncBotte:
    ...
    .. code-block:: python3
        :caption: Using asynchronous implementation of Botte.

        from Botte.async_Botte import AsyncBotte
        bot = AsyncBotte('token') # get token from @BotFather
    ...
    :type state_storage: :class:`Botte.asyncio_storage.StateMemoryStorage`, optional
    ...
    async def runthebot(self, timeout: Optional[int]=20, skip_pending: Optional[bool]=False, request_timeout: Optional[int]=None,
        ...
        :param logger_level: Custom logging level for runthebot logging.
        ...
        if handlers and not(skip_handlers):
            try:
                for handler in handlers:
                    params = []
                    process_update = await self._test_handle_msg(handler, message)
    ...
    async def process_new_updates(self, updates: List[types.Update]):
        ...
        :type updates: :obj:`list` of :obj:`Botte.types.Update`
    ...
    async def process_new_messages(self, new_messages):
        ...
        await self._process_updates(self.handle_msgs, new_messages, 'message')

    async def process_new_edited_messages(self, edited_message):
        """
        :meta private:
        """
        await self._process_updates(self.edited_handle_msgs, edited_message, 'edited_message')
    ...
    async def _test_handle_msg(self, handle_msg, message):
        """
    ...
    def add_custom_filter(self, custom_filter: Union[asyncio_filters.SimpleCustomFilter, asyncio_filters.AdvancedCustomFilter]):
        ...
        :type custom_filter: :class:`Botte.asyncio_filters.SimpleCustomFilter` or :class:`Botte.asyncio_filters.AdvancedCustomFilter`
    ...
    def setup_middleware(self, middleware: BaseMiddleware):
        ...
        .. note::
            ...
            Take a look at the :class:`Botte.asyncio_handler_backends.BaseMiddleware` section for more.

        :param middleware: Middleware-class.
        :type middleware: :class:`Botte.asyncio_handler_backends.BaseMiddleware`
    ...
    def handle_msg(self, commands=None, regexp=None, func=None, content_types=None, chat_types=None, **kwargs):
        """
        ...
        if isinstance(commands, str):
            logger.warning("handle_msg: 'commands' filter should be List of strings (commands), not string.")
            commands = [commands]
    ...
    def add_handle_msg(self, handler_dict):
        ...
        Note that you should use register_handle_msg to add handle_msg.
    ...
    def register_handle_msg(self, callback: Callable[[Any], Awaitable], content_types: Optional[List[str]]=None, commands: Optional[List[str]]=None,
        ...
        if content_types is None:
            content_types = ["text"]
        if isinstance(commands, str):
            logger.warning("register_handle_msg: 'commands' filter should be List of strings (commands), not string.")
            commands = [commands]
    ...
    def edited_handle_msg(self, commands=None, regexp=None, func=None, content_types=None, chat_types=None, **kwargs):
        ...
        if isinstance(commands, str):
            logger.warning("edited_handle_msg: 'commands' filter should be List of strings (commands), not string.")
            commands = [commands]
    ...
    def add_edited_handle_msg(self, handler_dict):
        ...
        Note that you should use register_edited_handle_msg to add edited_handle_msg.
    ...
    def register_edited_handle_msg(self, callback: Callable[[Any], Awaitable], content_types: Optional[List[str]]=None,
        ...
        """
        if isinstance(commands, str):
            logger.warning("register_edited_handle_msg: 'commands' filter should be List of strings (commands), not string.")
            commands = [commands]
    ...
    def enable_saving_states(self, filename="./.state-save/states.pkl"):
        ...
        .. note::
            It is recommended to pass a :class:`~Botte.asyncio_storage.StatePickleStorage` instance as state_storage
            to Botte class.
    ...
    async def run_webhooks(self,
        ...
        # for webhooks
        try:
            from Botte.ext.aio import AsyncWebhookListener
