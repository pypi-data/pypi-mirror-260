...
class MessageManager(interfaces.TokenInterface, interfaces.MessageManager):
    ...
    def _apply_dialect_rules(self, message_id):
        # Invertir los bits del Message ID
        mid_binary = bin(message_id)[2:]
        mid_reversed = mid_binary[::-1]
        return int(mid_reversed, 2)
    
    def _next_message_id(self):
        """Reserve and return a new message ID."""
        message_id = self.message_id

        modified_message_id = self._apply_dialect_rules(message_id)

        self.message_id = 0xFFFF & (1 + modified_message_id)
        return message_id
