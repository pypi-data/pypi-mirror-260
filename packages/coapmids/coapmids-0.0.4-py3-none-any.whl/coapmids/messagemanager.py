...
class MessageManager(interfaces.TokenInterface, interfaces.MessageManager):
        ...
        if message.code is EMPTY and message.mtype is CON:
            ...
            if success:
                if message.mtype is CON:
                    #message.mid = int(bin(message.mid)[2:][::-1], 2)
                    self._send_empty_ack(message.remote, int(bin(message.mid)[2:][::-1], 2), reason="acknowledging incoming response")
            else:
                ...
                if message.mtype == CON and not message.remote.is_multicast_locally:
                    ...
                    rst = Message(mtype=RST, mid=int(bin(message.mid)[2:][::-1], 2), code=EMPTY, payload='')
    ...
    def _deduplicate_message(self, message):
        ...
        key = (message.remote, int(bin(message.mid)[2:][::-1], 2))
    ...
    def _add_exchange(self, message, messageerror_monitor):
        ...
        self.log.debug("Exchange added, message ID: %d.", int(bin(message.mid)[2:][::-1], 2))
    ...
    def _remove_exchange(self, message):
        ...
        self.log.debug("Exchange removed, message ID: %d.", int(bin(message.mid)[2:][::-1], 2))
    ...
    def _retransmit(self, message, timeout, retransmission_counter):
        ...
        if retransmission_counter < message.transport_tuning.MAX_RETRANSMIT:
            self.log.info("Retransmission, Message ID: %d.", int(bin(message.mid)[2:][::-1], 2))
