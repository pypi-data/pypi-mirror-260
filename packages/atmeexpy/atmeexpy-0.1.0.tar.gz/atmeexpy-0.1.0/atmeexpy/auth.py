...
class AtmeexAuth(httpx.Auth):
    ...
    async def try_auth(self) -> bool:
        with httpx.Client() as client:
            # raises exception on network errors
            resp = await client.send(self._gen_auth_request())
    ...
    def auth_with_email(self) -> typing.Generator[httpx.Request, httpx.Response, None]:
        response = yield self._gen_auth_request()
        self.handle_auth_response(response)
    ...
    def _gen_auth_request(self):
        payload = {
            "email": self.email,
            "password": self.password,
            "grant_type": "basic",
        }
        return httpx.Request("POST", ATMEEX_API_BASE_URL + "/auth/signin", json=payload, headers=COMMON_HEADERS)