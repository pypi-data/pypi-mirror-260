...
class PipelineOrchestratorValidator:
    ...
    def validate_parameters(
        X, y, test_size: float, validation_size: float, verbose: int
    ):
        ...
        PipelineOrchestratorValidator.validate_train_test_size(
    ...
    def validate_X_y(data, instance_must_be_dataframe=True):
        ...
        if not isinstance(data, available_types):
            raise TypeError(
                f"X must be a of types {available_types}. Got {type(data)} instead."
            )

        enforced_type = pd.DataFrame if instance_must_be_dataframe else pd.Series
        collection_length_is_3 = len(data) == 3
        all_elements_are_right_type = all(
            isinstance(elem, enforced_type) for elem in data
        )
    ...
    def validate_train_test_size(test_size, validation_size):
        ...
        if validation_size + test_size >= 1.0:
            raise ValueError(
                f"validation_size and test_size together cannot be larger than 1. Got validation_size={validation_size} and test_size={test_size}"
            )
