...
class PipelineOrchestrator:
    ...
    def __init__(
        ...
        if infer_type_estimator_from_data:
            self.type_estimator = (
                "regressor" if self.preperator_service.is_regression else "classifier"
            )
        else:
    ...
    def register_metric(
        ...
        modeling_type = (
            "regression" if type_estimator == "regressor" else "classification"
        )
    ...
    def get_pipeline_metadata(self) -> Dict[str, PipelineMetadata]:
        ...
        if not self._pipeline_metadata:
            raise ValueError(
                "No metadata available. Please run fortify() first to generate the metadata."
            )
    ...
    def pre_optimize(
        ...
        self: PipelineOrchestrator
        """
        self.component_service.optimize_estimator_list(
    ...
    def build(
        ...
        if "base" in generations:
            self.pipelines[
                "base"
            ] = self.component_service.generate_pipeline_for_base_models(
        ...
        if "stacked" in generations:
            self.pipelines[
                "stacked"
            ] = self.component_service.generate_pipeline_for_stacked_models(
    ...
    def fortify(
        ...
        for pipeline_name, pipeline in pipelines_copy.items():
            if not pipeline.estimators:
                logger.warning(
                    f"'{pipeline_name}' has no estimators. It will be removed."
                )
            ...
            if robust_pipeline is not None:
                logger.notice(
                    f"MultiEstimatorPipeline '{pipeline_name}' is fortified successfully"
                )
                if optimize_n_jobs:
    ...
    def _stress_test_pipeline(
        ...
        try:
            ...
            if fraction_to_explain:  # if more than 0.0
                try:
                    logger.notice(
                        f"Getting explained features for pipeline '{pipeline_name}'..."
                    )
            ...
            logger.notice(
                f"Getting explained distribution for pipeline '{pipeline_name}'..."
            )
            explained_distribution = performance_service.get_distribution(
                plot=plot_explaining
            )
    ...
    def _reassemble_Xy_for_refitting(
        self, on_train: bool, on_test: bool, on_validation: bool
    ):
        X_parts = []
        y_parts = []
    ...
    def _validate_generations(self, generations: List[str]) -> None:
        ...
        if "evolved" in generations and not "stacked" in generations:
            raise ValueError(
                "generations must include 'stacked' if 'evolved' is included"
            )
