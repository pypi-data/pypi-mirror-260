...
def _check_sklearn_metrics():
    ...
    score_diff = set(all_registered_scores) - set(all_sklearn_metrics)  # Check if there are any scores that are not in the sklearn.metrics module
    if score_diff:
        logger.warn_always(f"Registered scores {score_diff} from the SCORE_TYPES constant are not in the sklearn.metrics module")
...
def _warn_error_metrics_and_cross_val_score(scoring: _BaseScorer):
    ...
    if scoring:
        ...
        if func_name.lower().endswith("error"):
            logger.warn_always("Scoring function '{}' is an error metric. "
                ...
                "\nConsider registering a custom scoring function that returns the negative of the error metric, with the maximize_scoring parameter set to True."
                .format(func_name))
