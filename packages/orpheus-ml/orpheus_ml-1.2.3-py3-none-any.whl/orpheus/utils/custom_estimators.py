...
class MultiEstimatorWrapper(BaseEstimator):
    ...
    def predict(self, X, **kwargs) -> np.ndarray[np.ndarray[float]]:
        """
    ...
    def predict_proba(self, X, **kwargs) -> np.ndarray[np.ndarray[float]]:
        """
    ...
    def score(self, X, y, metric: Optional[Callable] = None, **predict_kwargs) -> np.ndarray[float]:
        """
    ...
    def _predict_wrapper(self, X: pd.DataFrame, predict_func_name: str, **kwargs) -> np.ndarray[np.ndarray[float]]:
        """
        ...
        def predict(estimator) -> str | np.ndarray[np.ndarray[float]]:
            try:
