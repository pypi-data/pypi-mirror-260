...
from orpheus.validations.estimators import estimators_are_valid, is_estimator, pipeline_has_estimator_step
...
class MultiEstimatorPipeline(Pipeline):
    ...
    def predict_log_proba(self, X, **kwargs) -> np.ndarray[np.ndarray[float]]:
        """
    ...
    def decision_function(self, X, **kwargs) -> np.ndarray[np.ndarray[float]]:
        """
    ...
    def optimize_n_jobs(
        self, X: pd.DataFrame, n_iter: int = 10, exclude: Optional[Literal["transformers", "estimators"]] = None
    ...
    def get_name_by_index(self, index: int) -> str | None:
        """
    ...
    def update_scores(self, scores: np.ndarray[float], memory_size: int = 10) -> None:
        """
    ...
    def _set_n_jobs(self, n_jobs: int, exclude: Optional[Literal["transformers", "estimators"]] = None) -> None:
        for name, step in self.named_steps.items():
    ...
    def _predict_func_error_handler(
        self, predict_func: str, X: pd.DataFrame, exclude_transformers: bool = False, **kwargs
    ) -> np.ndarray[np.ndarray[float]]:
    ...
    def _check_if_estimator_wrapper_is_set(self):
        ...
        if not estimators_are_valid(self):
            self.steps[-1] = (self.steps[-1][0], MultiEstimatorWrapper(self.steps[-1][1]))
