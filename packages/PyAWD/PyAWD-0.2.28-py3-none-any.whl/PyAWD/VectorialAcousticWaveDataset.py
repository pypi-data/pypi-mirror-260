...
class VectorialAcousticWaveDataset(torch.utils.data.Dataset):
    ...
    def __init__(self, size, nx=128, sx=1., ddt=0.01, dt=2, t=10, interrogators=[(0, 0)], velocity_model="Marmousi"):
        try:
            ...
            self.amplitude_factor = np.random.random(size)*2
            self.generate_data()
    ...
    def __getitem__(self, idx):
        return self.epicenters[idx], self.data[idx][:, :, ::int(1/self.sx), ::int(1/self.sx)], self.max_velocities[idx], self.force_delay[idx]