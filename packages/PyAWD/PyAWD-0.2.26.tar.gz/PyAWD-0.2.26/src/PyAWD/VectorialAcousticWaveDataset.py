...
def solve_vectorial_pde(grid, nx, ndt, ddt, epicenter, velocity_model, max_velocity):
    """
    ...
    f[0].data[:] = s_x_t
    f[1].data[:] = s_y_t
    eq = dvt.Eq(u.dt2, f+((max_velocity*1e4)*velocity_model**2)*(u.laplace))
...
class VectorialAcousticWaveDataset(torch.utils.data.Dataset):
    ...
    def __init__(self, size, nx=128, sx=1., ddt=0.01, dt=2, t=10, interrogators=[(0, 0)], velocity_model="Marmousi"):
        try:
            ...
            if velocity_model == "Marmousi":
                ...
                self.max_velocities = (np.random.random(size)*0.5+0.5)*750
    ...
    def generate_data(self):
        ...
        for i in tqdm(range(self.size)):
            data = solve_vectorial_pde(self.grid, self.nx, self.ndt, self.ddt, self.epicenters[i], self.velocity_model, self.max_velocities[i])
    ...
    def plot_item(self, idx):
        ...
        for interrogator in self.interrogators:
            ...
            i += 1
        epicenter, item, max_velocity = self[idx]
        ...
        for i in range(self.nt):
            ...
            ax[i].set_title("t = " + str(i*(item.shape[1]//self.nt)*self.dt) + "s, velocity factor = " + str(max_velocity))
            ax[i].axis("off")
        plt.tight_layout()
        plt.show()
        print(colors)
    ...
    def __getitem__(self, idx):
        return self.epicenters[idx], self.data[idx][:, :, ::int(1/self.sx), ::int(1/self.sx)], self.max_velocities[idx]