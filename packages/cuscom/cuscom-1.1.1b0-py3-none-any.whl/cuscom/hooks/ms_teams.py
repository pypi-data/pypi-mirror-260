...
class MSTeamsWebhookHook(HttpHook):
    ...
    :param facts: List of arguments to display as key value
    :type facts: List[dict]
...
def on_failure(connection: Optional[Connection] = None, msg_template: Optional[str] = None):
    ...
    def fn(context: Dict[str, Any]):
        arguments = {
            ...
            "facts": [
                {"name": "dag", "value": context.get("task_instance").dag_id},
                {"name": "task", "value": context.get("task_instance").task_id},
                {"name": "execution_date", "value": context.get("execution_date")},
            ],
        }
        ...
        if dag_run is not None and hasattr(dag_run, "conf"):
            if dag_run.conf:
                arguments["facts"].extend(
                    [{"name": key, "value": value} for key, value in mask(dag_run.conf).items()]
                )
