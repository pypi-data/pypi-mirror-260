...
def deploy_model(  # noqa: C901
    ...
    try:
        ...
        if helper_models is not None and packages_to_check is not None:
            check_for_helper_model_dependency_mismatch(
                c, main_model_requirements, helper_models, packages_to_check
            )
...
def check_for_helper_model_dependency_mismatch(
    c: ServiceSettings,
    ...
    for helper_model in helper_models:
        helper_model_target = Path().resolve() / helper_model
        mm.download_deployment_folder(
            helper_model, "prod", target_path=str(helper_model_target)
        )
        ...
        try:
            ...
            shutil.rmtree(helper_model_target)
