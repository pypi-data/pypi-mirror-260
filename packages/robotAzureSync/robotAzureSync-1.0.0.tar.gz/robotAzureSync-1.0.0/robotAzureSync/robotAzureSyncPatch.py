...
def update_azure_test_case(
    ...
    test_case_id_match = re.search(f"{sync_configuration['tag_config']['test_case']}\s*(\d+)", cases_dict["Tags"])
    ...
    for robot_file in path_robot_files:
        if "todo_organize.robot" not in robot_file:
            ...
            for case in cases:
                ...
                user_story_ids = tags.get(sync_config['tag_config']['user_story'], [])
