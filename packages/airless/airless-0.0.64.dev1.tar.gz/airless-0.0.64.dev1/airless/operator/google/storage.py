...
class BatchWriteDetectAggregateOperator(BaseEventOperator):
    ...
    def execute(self, data, topic):
        ...
        prefix = data.get('prefix')
        threshold = data['threshold']
        deadline_until_all_files_processed = threshold.get('deadline_all_files_processed', 60)  # minutes
        ...
        for b in self.gcs_hook.list(bucket, prefix):
            ...
            if (b.time_deleted is None) and ((b.time_created > last_timestamp) or (b.time_created < (datetime.now(timezone.utc) - timedelta(minutes=deadline_until_all_files_processed)))):
