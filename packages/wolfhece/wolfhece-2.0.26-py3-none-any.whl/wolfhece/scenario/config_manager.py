...
class Config_Manager_2D_GPU:
    ...
    def load_ic(self, path:Path) -> InitialConditions_scenario:
        ...
        if INITIAL_CONDITIONS in low_keys:
            return InitialConditions_scenario(self.workingdir / INITIAL_CONDITIONS.value / path)
        else:
            return None
    ...
    def get_name_initial_conditions(self) -> list[str]:
        ...
        names = []
        if INITIAL_CONDITIONS in low_keys:
            names = [curpath for curpath in self.configs.keys()[low_keys.index(INITIAL_CONDITIONS)]]
        ...
        if isinstance(mydata, dict):
            ...
            if mydata[IS_SIMUL]:
                ...
                if ctrl:
                    ...
                    if res_path.exists():
                        sim = SimpleSimulation.load(mydata['path'])
                        sim.write_initial_condition_from_record(res_path, None, self.parent.workingdir / 'initial_conditions' / mydata['path'].name)

                else:
                    self._txtctrl.write(_('\n\n CTRL + double click to open the simulation in the UI'))
                    self._txtctrl.write(_('\n\n SHIFT+ double click to edit simumation parameters in the UI'))
