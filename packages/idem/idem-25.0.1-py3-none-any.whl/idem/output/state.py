...
def display(hub, data):
    ...
    for tag in data:
        ...
        if isinstance(comment, str) or not isinstance(comment, Iterable):
            comment = [comment]
        ...
        formatted_comments = f"{tcolor} Comment:\n"
        # Add each comment with a bullet point
        for c in comment:
            formatted_comments += f"{tcolor}  - {c}\n"
        # Reset color after building comments
        formatted_comments += endc
        strs.append(formatted_comments)
        ...
        if result:
            ...
            elif hub.idem.rules.init.STATE_SKIPPED in comment:
                _increment_count(fun_count, fun, "skipped")

    strs = strs + _format_fun_counts(fun_count)
    return "\n".join(strs)


def _format_fun_counts(fun_map: Dict[str, Dict[str, int]]) -> List[str]:
    ...
    for fun, result_and_count in fun_map.items():
        ...
        elif result_and_count.get(False, 0) > 0:
            strs.append(f"{fun}: {result_and_count[False]} failed")

        elif result_and_count.get(None, 0) > 0:
            strs.append(f"{fun}: {result_and_count[None]} skipped")

    return strs


def _increment_count(map: Dict[str, Dict[str, int]], key1: str, key2: str):
