...
def _replace_arg_reference_with_arg_value(
    ...
    if (
        ...
        and arg_reference_def != chunk_arg_value
    ):
        if not isinstance(arg_value, str):
            # Cast  arg_value to string before replacing it in 'chunk_arg_value'
            try:
                arg_value = str(arg_value)
            except Exception:
                raise ValueError(
                    f'Cannot set argument value "{arg_value}" in "{arg_reference_def}", '
                    "since it cannot be cast to a string"
                )
