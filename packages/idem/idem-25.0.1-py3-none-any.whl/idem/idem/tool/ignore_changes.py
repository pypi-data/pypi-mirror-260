...
def ignore_parameter_changes(
    ...
    for param_path in ignore_changes:
        ...
        if arg_key in ["hub", "ctx", "name"]:
            continue
        # We can only assign None to the optional parameters which default is not inspect._empty
        if arg_key in param_signatures:
            if param_signatures.get(arg_key).default != inspect._empty:
                try:
                ...
                hub.log.warning(
                    f"'{arg_key}' is a required parameter and therefore cannot be ignored."
                )
        else:
            hub.log.warning(
                f"No-op: '{arg_key}' is not a parameter in the method signature."
            )
