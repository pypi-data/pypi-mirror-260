...
import pop.loader
from pop.exc import PreContractFailed
...
async def run(
    ...
    try:
        ...
        # A result of "None" means the state was skipped
        ret = dict(comment=[STATE_SKIPPED, msg], result=None, ret=None)

        # Other states, even ones with exceptions get their post contracts run
        # If the pre-contract fails, post contracts do not run so we need to fire the state-post event here.
