...
from cassandra.cluster import Cluster, Session, HostDistance, ResultSet
...
class ScyllaService(IDatabaseService):
    ...
    def select(self, dictionary_input: Dict[str, Any], table_name: str) -> ResultSet:
        ...
        try:
            return self.session.execute_async(prepared_statement.bind(values)).result()
    ...
    def _execute_query(self, statement: PreparedStatement, values: List[Any]) -> None:
        self._semaphore.acquire()
        try:
