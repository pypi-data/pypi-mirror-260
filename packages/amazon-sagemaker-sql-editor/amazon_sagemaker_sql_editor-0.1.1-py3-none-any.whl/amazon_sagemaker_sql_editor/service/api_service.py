...
class GetConnectionsApiService(InstanceAttributeMetricsContextMixin):
    ...
    async def get_response(
        self, connection_filter_type: str = None, connection_filter_source: str = None
    ) -> dict:
        """
        ...
        async with client_factory.create_client(ClientType.BOTO, glue_client_args) as glue_client:
            ...
            try:
                ...
                if boto_error_code in ["EntityNotFoundException", "AccessDeniedException"]:
                    raise SagemakerSQLApiServiceError(error)
                if boto_error_code in [
                    "OperationTimeoutException",
                    "GlueEncryptionException",
                    "InvalidInputException",
                ]:
                    raise SagemakerSQLApiServiceFault(error)
...
class PostDataSourcesApiService(InstanceAttributeMetricsContextMixin):
    ...
    async def get_response(
        ...
        response = await self.acache.get_response_memoized(
            self._get_response, path=path, refresh=refresh
        )
    ...
    async def _get_response(self, path: str) -> dict:
        """
