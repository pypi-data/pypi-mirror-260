...
class SearchService(ClassAttributeMetricsContextMixin):
    ...
    async def _initialize_index(self) -> bool:
        ...
        connections_response = (await GetConnectionsApiService().get_response())["connections"]
        ...
        if len(un_indexed_connections) == 0:
            ...
            for connection_name in un_indexed_connections:
                ...
                await self.schema_service.submit_request(
                    GetSchemaRequest(path=connection_name, refresh=False)
                )
    ...
    async def search(self, request: SearchRequest) -> SearchResponse:
        ...
        if not self.index_ready:
            updated_index_ready = await self._initialize_index()
    ...
    async def refresh_index_for_path(self, path: str):
        ...
        if path == "":
            ...
            connections_response = (await self.get_connections_api_service.get_response())[
                "connections"
            ]
            connections = [connection_detail["name"] for connection_detail in connections_response]
        else:
