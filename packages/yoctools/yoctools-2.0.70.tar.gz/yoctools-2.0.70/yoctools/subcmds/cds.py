            ...
            try:
                with codecs.open(csv_file, 'r', 'UTF-8') as f:
                    for f in f.readlines():
                        line_split = f.split(',')
                        sname = line_split[0]
                        if opt.solution and (sname not in specify_solution_list):
                            continue
                        def _build_with_csv():
                            ...
                            for i in range(1, len(line_split)):
                                ...
                                if c.startswith("e"):
                                    platform = 'smartl'
                                if opt.platform and (platform not in specify_platform_list):
                                    continue
                        ...
                        if sname != 'solutions':
                            if sname.startswith('soc_'):
                                ...
                                for k in kernels:
                                    kernel = k
                                    if opt.kernel and (kernel not in specify_kernel_list):
                                        continue
                                    _build_with_csv()
                            else:
                                kernel = None
