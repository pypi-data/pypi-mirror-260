...
class RichDisplayMixin:
    ...
    def _repr_mimebundle_(
        ...
        **kwargs: Any,
        ...
        return self._to_rich_obj()._repr_mimebundle_(include, exclude, **kwargs)


def get_default_table_with_panel(**panel_kwargs) -> Tuple[Panel, Table]:
    table = Table.grid()
    computed_panel_args = {"border_style": Style(color=CHALK_WEBSITE_GREEN), "box": box.ROUNDED, "padding": (1, 2)}
    computed_panel_args.update(**panel_kwargs)
    panel = Panel.fit(table, **computed_panel_args)
    return panel, table
...
def _get_last_updated_markdown(last_update: Optional[dt.datetime]) -> "Markdown":
