...
class _UploadFeaturesValidationErrors:
    ...
    def expected_mapping(e: Any):
        return ChalkError(
            ...
            message=f"Multi upload detected a list of items, but some list items are `{type(e).__name__}`. If you "
    ...
    def expected_same_length_lists(
        ...
        return ChalkError(
            code=ErrorCode.INVALID_QUERY,
            message=f"expected all values in mapping to be lists of the same length, found {expected_length} "
    ...
    def structs_not_supported(fqn: str, obj: Any) -> ChalkError:
        return ChalkError(
            code=ErrorCode.INVALID_QUERY,
            message=f"Multi-upload does not yet support uploading structs, "
...
def _validate_mapping(inputs: Mapping) -> List[ChalkError]:
    if len(inputs) == 0:
        return []
    ...
    for k, v in inputs.items():
        if not isinstance(v, list):
            return [_UploadFeaturesValidationErrors.expected_list_in_mapping(k, v)]
        if num_rows is None:
            reference_col = k
            num_rows = len(v)
        elif num_rows != len(v):
            return [_UploadFeaturesValidationErrors.expected_same_length_lists(num_rows, reference_col, len(v), k)]
        if _is_struct_like_object(v[0]):
            return [_UploadFeaturesValidationErrors.structs_not_supported(k, v[0])]
...
def to_multi_upload_inputs(
    ...
    if isinstance(inputs, DataFrame):
        ...
        for keyset, common_mappings_list in common_keys_to_mappings.items():
        ...
        if isinstance(inputs, (pl.DataFrame, pl.lazyframe.frame.LazyFrame)):
