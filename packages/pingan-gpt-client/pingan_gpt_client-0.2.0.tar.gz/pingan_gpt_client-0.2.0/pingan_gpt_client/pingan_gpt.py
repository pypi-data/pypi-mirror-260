...
class PATechGPTClient:
    ...
    def __payload_template(self,
                           ...
                           own_history: Optional[List[str]] = None,
                           ) -> Dict[str, Any]:
        if "temperature" not in generate_param:
            generate_param["temperature"] = 0.5
        if "top_p" not in generate_param:
            generate_param["top_p"] = 0.5
        if "max_new_tokens" not in generate_param:
            generate_param["max_new_tokens"] = 1000
        ...
        if is_use_own_history:
            payload["isUseOwnHistory"] = is_use_own_history
        if own_history:
            payload["ownHistory"] = own_history
        return payload
    ...
    def pingan_gpt_batch_inference(self, prompts: List[str],
                                   ...
                                   own_history: Optional[List[str]] = None,
                                   timeout=120,
                                   token: Optional[str] = None, only_answer=True,
