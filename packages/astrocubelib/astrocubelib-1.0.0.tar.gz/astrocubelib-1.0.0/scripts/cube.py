...
if args.info:
    print('\nThe input file must contains the following parameters:'
          ...
          '\n-               # verbose'
          '\nFalse           # Imposition to only fit emission lines'
          '\nFalse           # Imposition to only fit absorption lines'
...
espectro = finput[j].split()[0].split(',');j+=1
mask     = finput[j].split()[0];j+=1
nametab  = finput[j].split()[0];j+=1
verbose  = finput[j].split()[0];j+=1

try:
    emi_line = ast.literal_eval(finput[j].split()[0]);j+=1
except:
    emi_line = False

try:
    abs_line = ast.literal_eval(finput[j].split()[0]);j+=1
except:
    abs_line = False
...
# Loading the cube
cube = fits.open(espectro[0])

if len (espectro) > 2:
    try:
        espectro[1]=int(espectro[1])
        spec = cube[espectro[1]].data
    except:
        spec = cube[espectro[1]].data
...
if len (hedkey) == 4:
   try:
       hdr0 = cube[hedkey[3]].header
   except:
       hdr0 = cube[int(hedkey[3])].header

else:
   hdr0 = cube[0].header

if len (espectro) == 3:
    wl = cube[espectro[2]].data
else:
    try:
        hdr   = [hdr0[hedkey[0]], hdr0[hedkey[1]]]
        try:
            hdr[0] = hdr[0]  + (1 - hdr0[hedkey[2]])*hdr[1]
        except:
...
# Load mask
if mask != 'none':
    amask = fits.open(mask)[0].data
else:
    amask = np.zeros(spec.shape[1::])
...
linesort = np.array(lines_lambda[:]).argsort()
linestmp = np.take(linestmp,linesort)
...
for index, pix0 in np.ndenumerate(spec[0, :, :]):
    ...
    if amask[y,x] == 0:
        ...
        if linesb[0] == 0:
            vfit_tmp = fitmultiline(wl,vspec,
                       clines, linesig,
                       emi_line = emi_line,
                       abs_line = abs_line,
        ...
        vfit   = np.array(vfit_tmp[0])
        if mask != 'none':
            tabfit.remove_column(tname)

        tabfit.add_column(Column(data=[
            ','.join(map(str,np.asarray(vfit_tmp[0]).ravel())),
            ','.join(vfit_tmp[1]),
            ','.join(map(str,np.asarray(vfit_tmp[2]).ravel())),
            ','.join(vfit_tmp[3]),
            ','.join(map(str,np.asarray(vfit_tmp[4]).ravel())),
            ','.join(map(str,np.asarray(vfit_tmp[5]).ravel())),
            ], name=tname))
...
if mask != 'none':
    for line in linestmp:
        ...
        vnamespectmp = np.loadtxt(ndattmp,dtype='str', usecols=([0]))
        vdatatmp  = np.loadtxt(ndattmp, usecols=np.arange(1,len_tab))

        if os.path.exists(ndat):

            vnamespec = np.loadtxt(ndat,dtype='str', usecols=([0]))
            vdata = np.loadtxt(ndat, usecols=np.arange(1,len_tab))
            ...
            for  tname in deb_tname:
                ...
                tfit  = np.ones((len_tab-1))*np.nan
                if any(np.array(vnamespectmp)==tname):
