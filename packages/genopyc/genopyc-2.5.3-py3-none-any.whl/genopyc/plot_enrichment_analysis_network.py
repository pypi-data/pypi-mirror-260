...
def plot_enrichment_analysis_network(
    ...
    if df.shape[0] == 0:
        ...
        return
    def labelling_without_overlapping(x, y, list_of_annotations, ax, fontsize ,verbose=False, **kwargs):
        class Point:
    ...
    for i, (s, v) in enumerate(zip(df.source.value_counts().index, df.source.value_counts())):
        ...
        if data.shape[0] == 0:
            ...
            if labelling:
                labelling_without_overlapping(xses, yses, number_lab, ax, fontsize = fontsize_ann ,**kwargs)
