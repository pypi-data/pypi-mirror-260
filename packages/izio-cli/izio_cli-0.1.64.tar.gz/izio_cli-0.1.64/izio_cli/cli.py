...
from izio_cli.helper.strings_transformers import to_pascal_case, to_snake_case, to_pascal_case_with_underscore
...
def new_project(
    ...
    if not projectName:
        confirm, pName = getProjectName()
        if not confirm or not pName:
            confirm, pName = getProjectName()
            if not confirm:
                raise Exit(code=1)
            if not pName:
    ...
    if type == "flutter":
        ...
        if entity:
            new_entity(f"{path}{os.sep}{projectName}", solution=projectName)
