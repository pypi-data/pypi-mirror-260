...
def new_project(
    ...
    if type == "flutter":
        ...
        if entity:
            new_entity(f"{path}", projectName=projectName)
...
def new_entity(
    ...
    projectName: str = Option("", "--project", "-o", help="Project name"),
):
    """
    ...
    if not projectName:
        projectName = inquirer.text("Enter the solution name")
        if not projectName:
            raise ValueError("Solution name is required")

    if not path:
        path = f"{os.getcwd()}{os.sep}{projectName}" if projectName not in os.getcwd() else os.getcwd()

    create_entity(path, name, console, projectName)
