...
def _is_1128_watermark(text: bytes):
    return bool(
        re.search(
            rb"Downloaded from https?://(.*) on .+ by .*", text
        )
    )
...
def _is_1212_watermark(text: bytes):
    return bool(
        re.search(
            rb"Downloaded from https?://(.*) by .+ on .* \d{4}", text
        )
    )
...
def _is_1353_watermark(text: bytes):
    return bool(
        re.search(rb"^\[.*]\s+Project MUSE\s+\(.*\)\s*.*$", text)
    )
...
pdf_processors = {
    ...
    "10.1128": WatermarkEraser1(
        is_watermark_predicate=_is_1128_watermark,
