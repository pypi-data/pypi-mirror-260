...
__global__ void update_best_candidate_cuda(const float *split_scores, int n_candidates, int *best_idx, float *best_score) {
...
void evaluate_splits(dataSet *dataset, const TreeNodeGPU *node, candidatesData *candidata, ensembleMetaData *metadata, splitDataGPU* split_data, const int threads_per_block, const int parent_n_samples){
     if (metadata->split_score_func == Cosine){
        ...
        split_cosine_score_kernel<<<n_blocks, tpb>>>(node, split_data->split_scores, candidata->candidate_indices, candidata->candidate_values, candidata->candidate_categories, candidata->candidate_numeric, candidata->n_candidates, split_data->left_sum, split_data->right_sum, split_data->left_count, split_data->right_count, split_data->left_dot, split_data->right_dot, split_data->left_norms, split_data->right_norms, metadata->grow_policy, metadata->min_data_in_leaf);
        ...
        cudaDeviceSynchronize();
        get_grid_dimensions(candidata->n_candidates, n_blocks, tpb);
        split_l2_score_kernel<<<n_blocks, tpb>>>(node, split_data->split_scores, candidata->candidate_indices, candidata->candidate_values, candidata->candidate_categories, candidata->candidate_numeric, candidata->n_candidates, split_data->left_sum, split_data->right_sum, split_data->left_count, split_data->right_count, metadata->grow_policy, metadata->min_data_in_leaf);
     }
...
void evaluate_oblivious_splits_cuda(dataSet *dataset, thrust::device_vector<TreeNodeGPU *> nodes, const int depth, candidatesData *candidata, ensembleMetaData *metadata, splitDataGPU *split_data){
    ...
    for (int i = 0; i < n_nodes; ++i){
        if (metadata->split_score_func == Cosine){
            ...
            split_score_cosine_cuda<<<candidata->n_candidates, threads_per_block, shared_mem>>>(dataset->obs, dataset->categorical_obs, dataset->build_grads, dataset->norm_grads, nodes[i], candidata->candidate_indices, candidata->candidate_values, candidata->candidate_categories, candidata->candidate_numeric, metadata->min_data_in_leaf, split_data->oblivious_split_scores + candidata->n_candidates*i, dataset->n_samples, metadata->grow_policy);
        } else if (metadata->split_score_func == L2){
            size_t shared_mem = sizeof(float)*2*(metadata->output_dim + 1)*threads_per_block;
            split_score_l2_cuda<<<candidata->n_candidates, threads_per_block, shared_mem>>>(dataset->obs, dataset->categorical_obs, dataset->build_grads, nodes[i], candidata->candidate_indices, candidata->candidate_values, candidata->candidate_categories, candidata->candidate_numeric, metadata->min_data_in_leaf, split_data->oblivious_split_scores + candidata->n_candidates*i, dataset->n_samples, metadata->grow_policy);
        }
    }
    cudaDeviceSynchronize();
    cudaError_t err = cudaGetLastError();
...
__global__ void split_score_cosine_cuda(const float* __restrict__ obs, const char* __restrict__ categorical_obs, const float* __restrict__ grads, const float* __restrict__ grads_norm, const TreeNodeGPU* __restrict__ node, const int* __restrict__ candidate_indices, const float* __restrict__ candidate_values, const char* __restrict__ candidate_categories, const bool* __restrict__ candidate_numeric, const int min_data_in_leaf, float* __restrict__ split_scores, const int global_n_samples, const growPolicy grow_policy){
    ...
    if (threadIdx.x == 0){
        ...
        l_mean_norm = sqrtf(l_mean_norm / (l_count[0]*l_count[0]));
        r_mean_norm = sqrtf(r_mean_norm / (r_count[0]*r_count[0]));
        l_dot_sum[0] = l_dot_sum[0] / l_count[0];
        r_dot_sum[0] = r_dot_sum[0] / r_count[0];
        ...
        if (node->depth > 0 && grow_policy == GREEDY)
            split_scores[cand_idx] = left_cosine + right_cosine - node->score;
        else
            split_scores[cand_idx] = left_cosine + right_cosine;
    }  
}
...
__global__ void split_score_l2_cuda(const float* __restrict__ obs, const char* __restrict__ categorical_obs, const float* __restrict__ grads, const TreeNodeGPU* __restrict__ node, const int* __restrict__ candidate_indices, const float* __restrict__ candidate_values,  const char* __restrict__ candidate_categories, const bool* __restrict__ candidate_numeric, const int min_data_in_leaf, float* __restrict__ split_scores, const int global_n_samples, const growPolicy grow_policy){
    ...
    if (threadIdx.x == 0) {
        ...
        for (int d = 0; d < n_cols; ++d){
            left_mean[d] = left_mean[d] / l_count[0];
            l_mean_norm += left_mean[d] * left_mean[d];
            right_mean[d] = right_mean[d] / r_count[0];
            r_mean_norm += right_mean[d] * right_mean[d];
        }
        if (node->depth > 0 && grow_policy == GREEDY)
            split_scores[cand_idx] = l_count[0]*l_mean_norm + r_count[0]*r_mean_norm - node->score;
        else
            split_scores[cand_idx] = l_count[0]*l_mean_norm + r_count[0]*r_mean_norm;
    }  
}
...
__global__ void split_cosine_score_kernel(const TreeNodeGPU* __restrict__ node, float* __restrict__ split_scores, const int* __restrict__ candidate_indices, const float* __restrict__ candidate_values,  const char* __restrict__ candidate_categories, const bool* __restrict__ candidate_numeric, const int n_candidates, float* __restrict__ lsum, float* __restrict__ rsum, float* __restrict__ lcount, float* __restrict__ rcount, float* __restrict__ ldot, float* __restrict__ rdot, float* __restrict__ lnorms, float* __restrict__ rnorms, const growPolicy grow_policy, const int min_data_in_leaf){
    ...
    if (cand_idx < n_candidates){
        ...
        if (lcount[cand_idx] < static_cast<float>(min_data_in_leaf) || rcount[cand_idx] < static_cast<float>(min_data_in_leaf)){
            split_scores[cand_idx] = -INFINITY;
            return;
        } else if (lcount[cand_idx]== 0 || rcount[cand_idx] == 0){
            split_scores[cand_idx] = 0;
            return;
        } 
        ...
        l_mean_norm = sqrtf(l_mean_norm / (lvalue * lvalue));
        r_mean_norm = sqrtf(r_mean_norm / (rvalue * rvalue));
        ...
        if (node->depth > 0 && grow_policy == GREEDY)
            split_scores[cand_idx] = lcos + rcos - node->score;
        else
            split_scores[cand_idx] = lcos + rcos;
    }
}

__global__ void split_l2_score_kernel(const TreeNodeGPU* __restrict__ node, float* __restrict__ split_scores, const int* __restrict__ candidate_indices, const float* __restrict__ candidate_values,  const char* __restrict__ candidate_categories, const bool* __restrict__ candidate_numeric, const int n_candidates, float* __restrict__ lsum, float* __restrict__ rsum, float* __restrict__ lcount, float* __restrict__ rcount, const growPolicy grow_policy, const int min_data_in_leaf){
    int cand_idx = blockIdx.x*blockDim.x + threadIdx.x;
    int n_cols = __ldg(&node->output_dim);
    int cand_row = cand_idx*n_cols;
    float lvalue, rvalue;
    if (cand_idx < n_candidates){
        ...
        lvalue = __ldg(&lcount[cand_idx]);
        rvalue = __ldg(&rcount[cand_idx]);
        l_mean_norm = l_mean_norm / lvalue; // n_count * l2 norm 
        r_mean_norm = r_mean_norm / rvalue; // n_count * l2 norm 

        if (node->depth > 0 && grow_policy == GREEDY)
            split_scores[cand_idx] = l_mean_norm + r_mean_norm - node->score;
        else
            split_scores[cand_idx] = l_mean_norm + r_mean_norm;
    }
}
...
// __global__ void get_partial_mean(const float* __restrict__ input, const int lda, float* __restrict__ per_col_results, const TreeNodeGPU* __restrict__ node)
// {
//     extern __shared__ float sdata[];

//     float x = 0.0f;
//     // Accumulate per thread partial sum
//     for(int i=threadIdx.x; i < node->n_samples; i += blockDim.x) {
//         int sample_idx = node->sample_indices[i]; // Access the spec
//         x += input[sample_idx * lda + blockIdx.x];
//     }
...
//     // thread 0 writes the final result
//     if(threadIdx.x == 0) {
//         per_col_results[blockIdx.x] = sdata[0] / static_cast<float>(node->n_samples);
//     }
// }
...
__global__ void node_l2_kernel(TreeNodeGPU *node, const float *mean){
    ...
    if (idx == 0){
        ...
        node->score = mean_squared_norm*static_cast<float>(node->n_samples); 
    }
}
...
void allocate_child_tree_nodes(dataSet *dataset, TreeNodeGPU* parent_node, TreeNodeGPU* host_parent, TreeNodeGPU** left_child, TreeNodeGPU** right_child, candidatesData *candidata, splitDataGPU *split_data){
    ...
    update_child_nodes_kernel<<<depth, n_threads>>>(parent_node, *left_child, *right_child, split_data->tree_counters, candidata->candidate_indices, candidata->candidate_values, candidata->candidate_numeric, candidata->candidate_categories, split_data->best_idx);
    cudaDeviceSynchronize();
}
...
__global__ void update_child_nodes_kernel(const TreeNodeGPU* __restrict__ parent_node, TreeNodeGPU* __restrict__ left_child, TreeNodeGPU* __restrict__ right_child, 
                                          ...
                                          const char* __restrict__ candidate_categories,  const int* __restrict__ best_idx){
    int idx = blockIdx.x * blockDim.x + threadIdx.x;
    ...
    if (idx == 0){
        ...
        left_child->score = -INFINITY;
        right_child->score = -INFINITY;
...
void fit_tree_greedy_cuda(dataSet *dataset, ensembleData *edata, ensembleMetaData *metadata, candidatesData *candidata, splitDataGPU *split_data){
    ...
    while (crnt_node_ptr_idx > 0)  {
        ...
        if (host_status == 0){
            ...
            cudaDeviceSynchronize();
            evaluate_splits(dataset, crnt_node, candidata, metadata, split_data, threads_per_block, host_node.n_samples);
        }
