...
class LimeBase:
    ...
    def __init__(self, kernel_fn: callable, verbose: bool = False, random_state: int | None = None) -> None:
        """Init function
    ...
    @staticmethod
    def generate_lars_path(weighted_data: np.ndarray, weighted_labels: np.ndarray) -> tuple[np.ndarray, np.ndarray]:
    ...
    def feature_selection(
        ...
        error = "Invalid feature selection method."
    ...
    def explain_instance_with_data(
        ...
        """
        # Compute the weights for the neighborhood samples.
        weights = self.kernel_fn(distances)

        # Get the target label's index.
        labels_column = neighborhood_labels[:, label]

        # Get the features used for the explanation.
        ...
        elif isinstance(model_regressor, LogisticRegression):
            labels_column = np.argmax(neighborhood_labels, axis=1)
            model_regressor = LogisticRegression(fit_intercept=True, random_state=self.random_state)

        # Weigh the neighborhood samples and learn the linear model.
        ...
        # Handle Ridge, Logistic (binary and multi-class) regression models
        if isinstance(easy_model, LogisticRegression):
            if easy_model.coef_.shape[0] == 1:  # Binary classification
                # Use the negative coefficients for class 0; for class 1, use the positive coefficients
                coefs = -easy_model.coef_[0] if label == 0 else easy_model.coef_[0]
            else:  # Multi-class classification
                coefs = easy_model.coef_[label]
        else:  # Other regression models (like Ridge)
            coefs = easy_model.coef_

        # Sort by decreasing absolute value of coefficients
        exp = sorted(zip(used_features, coefs), key=lambda x: np.abs(x[1]), reverse=True)
