...
from typing import Any, Dict, MutableMapping, Optional, Set, Tuple, cast
from zipfile import ZipFile  # noqa

from hangar_sdk.serverless.compat import (
    pip_import_string,
    pip_no_compile_c_env_vars,
    pip_no_compile_c_shim,
)
from hangar_sdk.serverless.utils import UI  # noqa
...
class BaseLambdaDeploymentPackager(object):
    ...
    def _deployment_package_filename(
        ...
        python_version: str,
        prefix: str = "",
        req_file_name: str = "requirements.txt",
    ) -> str:
        requirements_filename = self._get_requirements_filename(
            project_dir, req_file_name
        )
        ...
        for rootdir, dirs, filenames in self._osutils.walk(
            ...
            for filename in filenames:
                ...
                zip_path = self._osutils.joinpath(
                    os.path.relpath(rootdir, self._osutils.abspath(project_dir)),
                    filename,
                )
...
class LayerDeploymentPackager(BaseLambdaDeploymentPackager):
    ...
    def create_deployment_package(
        ...
        package_filename = self.deployment_package_filename(
            project_dir, python_version, req_file_name=requirements_filename
        )
        self._create_output_dir_if_needed(package_filename)
        if self._osutils.file_exists(package_filename):
    ...
    def _deployment_package_filename(
        ...
        if not self._osutils.file_exists(requirements_filename):
            ...
            contents = cast(
                bytes,
                self._osutils.get_file_contents(requirements_filename, binary=True).encode("utf-8"),
            )
        h = hashlib.md5(contents)
