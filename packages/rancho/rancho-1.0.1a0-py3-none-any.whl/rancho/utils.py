...
def draw_player(player_pos, screen, jet_image):
    """
    Function to draw the player on the screen
    """
    screen.blit(jet_image, (player_pos[0], player_pos[1]))

def draw_enemies(enemy_list, screen, jet_image_enemy):
    """
...
def draw_bullets(bullet_list, screen):
    """
...
def play():
    global player_pos, enemy_list, bullet_list, score, celebration_timer, SCRIPT_DIR
    pygame.init()
    ...
    while True:
        ...
        if celebration_timer > 0:
            ...
            draw_enemies(enemy_list, screen=screen, jet_image_enemy=jet_image_enemy)
            draw_bullets(bullet_list, screen=screen)

            if collision_check(player_pos, enemy_list):
                draw_text(screen, "Game Over Rancho!", (255, 0, 0), SCREEN_WIDTH // 2, SCREEN_HEIGHT // 2)
            ...
            draw_player(player_pos, screen=screen, jet_image=jet_image)

            if score == 2 and celebration_timer == 0:
