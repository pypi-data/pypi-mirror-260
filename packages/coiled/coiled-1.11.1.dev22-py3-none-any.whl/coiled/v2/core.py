...
class CloudV2(OldCloud, Generic[IsAsynchronous]):
    ...
    @track_context
    async def _create_custom_certificate(self, subdomain: str, account: Optional[str] = None):
        account = account or self.default_account
        response = await self._do_request(
            "POST",
            self.server + f"/api/v2/clusters/account/{account}/https-certificate",
            json={"subdomain": subdomain},
        )
        if response.status >= 400:
            await handle_api_exception(response)

    async def _check_custom_certificate(self, subdomain: str, account: Optional[str] = None):
        response = await self._do_request(
            "GET",
            self.server + f"/api/v2/clusters/account/{account}/https-certificate/{subdomain}",
        )
        if response.status >= 400:
            await handle_api_exception(response)
    ...
    async def _create_cluster(
        ...
        *,
        ...
        await self._verify_account(account)
