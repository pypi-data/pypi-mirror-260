    ...
    def send_message(self, message: str, path: str, method: str) -> bool:
        ...
        try:
            ...
            if SETTINGS.DD_ENV in ["localhost"]:
                self.logger.debug(f"Sending message in DD_ENV {SETTINGS.DD_ENV}:")
                self.logger.debug(f"Queue_url: {self.queue_url}")
                self.logger.debug(f"Message: {message_body}")
                return True
            ...
            self.logger.debug(f"Executed queue send_message with path: {path}")
            return True
