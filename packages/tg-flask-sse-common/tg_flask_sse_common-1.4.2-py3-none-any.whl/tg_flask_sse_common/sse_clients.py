...
class SseClients(object):
    ...
    def release_sse(self, channel, reason):
        """
        释放连接对象
        :param channel: 连接id
        :param reason: 关闭原因
        ...
        if channel in self.sse_global_clients:
            ...
            for sse_connect in sse_connect_list:
                sse_connect.update({
                    self.Field.LATER_RELEASE: True,
                    self.Field.RELEASE_REASON: reason
                })
    ...
    def listen_message(self, channel):
        ...
        print("sse_connect_list1111 : ", sse_connect_list, threading.current_thread().name)
        ...
        if not latest_sse_connect:
            ...
            self.release_sse(channel=channel, reason=SseListenRsp.ALL_DISCONNECT)
            yield None
            return
        ...
        print("sse_connect_list2222 : ", sse_connect_list, run_rsp, threading.current_thread().name)
        ...
        if run_rsp == SseListenRsp.TIMEOUT_CONNECT:
            ...
            self.release_sse(channel=channel, reason=SseListenRsp.TIMEOUT_CONNECT)
            ...
            # 无需关闭连接，因为是同一节点的重复连接，在连接建立时设置了关闭标志位
            ...
            self.release_sse(channel=channel, reason=SseListenRsp.D_REPEAT_CONNECT)
            ...
            self.release_sse(channel=channel, reason=SseListenRsp.END_CONNECT)
        else:
            print({
                'title': '未知情况，关闭轮训',
                'channel': channel,
            ...
            self.release_sse(channel=channel, reason=run_rsp)
        yield None
        return
