...
from .sse_constant import SseClientConfig, SseSystemEventType, SseBusinessEventType
...
class SseClients(object):
    ...
    def __init__(self, sse_clients_config, business=SseBusinessEventType.DEFAULT):
    ...
    def listen_message(self, channel):
        ...
        while (datetime.now() - connect_time).seconds < self.max_connect_time:
            ...
            # 非当前设置的业务类型的消息，不处理
            if event not in [
                SseSystemEventType.ERROR,
                SseSystemEventType.REDIS,
                SseSystemEventType.CONNECT,
                SseSystemEventType.END,
                SseBusinessEventType.DEFAULT,
                self.business
            ]:
                time.sleep(self.listen_interval)
                continue
