...
class SseRedisPubSub(object):
    ...
    def get_pub_message_stat(self, day=datetime.now().strftime('%Y%m%d')):
        """
        获取指定日期推送消息统计
        """
        return self.sse_event_message_cache.get_pub_all_by_day(day)

    def get_sub_message_stat(self, day=datetime.now().strftime('%Y%m%d')):
        """
        获取指定日期接收消息统计
        """
        return self.sse_event_message_cache.get_sub_all_by_day(day)
