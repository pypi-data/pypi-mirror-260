...
class SseRedisPubSub(object):
    ...
    def get_stat(self, day=datetime.now().strftime('%Y%m%d')):
        """
        获取指定日期连接信息
        """
        # 某天连接统计
        connect_stat = self.sse_connect_stats_cache.get_all_by_day(day)
        # 某天推送消息统计
        pub_message_stat = self.sse_event_message_cache.get_pub_all_by_day(day)
        # 某天接收消息统计
        sub_message_stat = self.sse_event_message_cache.get_sub_all_by_day(day)

        return connect_stat, pub_message_stat, sub_message_stat
