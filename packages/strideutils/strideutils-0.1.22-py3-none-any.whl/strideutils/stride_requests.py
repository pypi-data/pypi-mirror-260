...
retries = Retry(
    total=5, backoff_factor=0.5, status_forcelist=[400, 403, 404, 500, 502, 503, 504]
)
...
def get_host_zone_json(
    zone_id: str, cast_types: bool = True, **kwargs
) -> Dict[str, Any]:
    """
    ...
    endpoint = (
        f"{config.stride.api_endpoint}/Stride-Labs/stride/stakeibc/host_zone/{zone_id}"
    )
...
@lru_cache
def get_balance(
    address: str, block_height: int = 0, api_endpoint: str = config.stride.library_api
) -> Dict[str, int]:
    """
...
@lru_cache
def get_supply(
    token: str, block_height: int = 0, api_endpoint: str = config.stride.api_endpoint
) -> int:
    """
...
@lru_cache
def get_block_time(
    height: int, rpc_endpoint: str = config.stride.library_rpc
) -> datetime.datetime:
    """
...
def get_txs(
    event_filters: List[str], api_endpoint: str = config.stride.api_endpoint
) -> Dict:
    """
    ...
    params: List[Tuple[str, str]] = [
        ("events", event_filter) for event_filter in event_filters
    ]
...
def generate_vesting_account(
    ...
    start_time = timezone.localize(
        datetime.datetime.strptime(start_time_in_local_tz, "%Y-%m-%d %H:%M")
    )
...
def get_block_height_in_past(
    ...
    if prev_time is not None:
        ...
        ts = (
            datetime.datetime.now() - cast(datetime.datetime, prev_time)
        ).total_seconds()
    ...
    while abs(time_estimate_error) > max_acceptable_error_seconds:
        ...
        backwards_looking_time = get_block_time(
            backwards_looking_height, rpc_endpoint=config.stride.library_rpc
        )
        time_estimate_error = (
            current_time - backwards_looking_time - time_in_past
        ).total_seconds()
...
def get_delegated_tokens_on_address(
    ...
    host_zone_balance = query_list_with_pagination(
        stake_endpoint, rel_key="delegation_responses"
    )
    stake_amounts: Dict[str, int] = {}
    for hzb in host_zone_balance:
        stake_amounts[hzb["delegation"]["validator_address"]] = int(
            hzb["balance"]["amount"]
        )
...
def get_unbonding_tokens_on_address(
    ...
    unbondings = query_list_with_pagination(
        unbonding_endpoint, rel_key="unbonding_responses"
    )
...
def get_host_zone_delegations(
    host_zone_id: str, ground_truth_included: bool = True
) -> pd.DataFrame:
    """
    ...
    if ground_truth_included:
        ...
        stake_amounts = {}
        for hzb in host_zone_balance:
            stake_amounts[hzb["delegation"]["validator_address"]] = float(
                hzb["balance"]["amount"]
            )
...
def get_day_epoch() -> int:
    ...
    day_epoch = int(
        epochs_df[epochs_df["identifier"] == "day"]["current_epoch"].values[0]
    )
    return day_epoch


def query_wasm_contract(
    contract_address: str, query_msg: Dict, api_endpoint: str, block_height: int = 0
) -> Dict:
    """
    ...
    query_msg_encoded = base64.b64encode(query_msg_string.encode("utf-8")).decode(
        "utf-8"
    )

    query_path = COSMWASM_CONTRACT_QUERY.format(
        contract_address=contract_address, query=query_msg_encoded
    )
