...
class BaseDownloader(object):
    ...
    def download_tasks(self, tasks, executor):
        ...
        for task_info in tasks:
            ...
            for file_info in task_info["files"]:
                if self.regex and not re.search(self.regex, file_info["relative_path"]):
                    continue

                file_info["output_dir"] = self.ensure_writable_destination(file_info, task_info)
    ...
    def ensure_writable_destination(self,file_info,task_info):
        """
        ...
        If the destination is not writable, we try to use a fallback directory. If that fails, we use the temp folder.
        """

        output_path = file_info["output_dir"]
        if os.name == "posix":
            if re.match(r"^[a-zA-Z]:", output_path):
                self.destination =  os.path.expanduser(os.path.join("~", FALLBACK_DOWNLOADS_FOLDER))

        if self.destination:
            output_path =  os.path.join(self.destination, task_info["job_id"])
