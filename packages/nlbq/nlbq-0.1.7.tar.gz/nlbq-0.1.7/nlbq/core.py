...
def strip_code_block_markdown(text):
    """Removes code block markdown from the given text."""
...
class NLBQ:
    ...
    async def text_to_bq(self, question: str) -> tuple:
        ...
        resp = await client.chat.completions.create(
            model=self.model, messages=prompt_messages, temperature=0
        )

        usage = resp.usage
        statement = resp.choices[0].message.content.strip()
        statement = strip_code_block_markdown(statement)
        cost = calculate_cost(usage.prompt_tokens, usage.completion_tokens, self.model)
