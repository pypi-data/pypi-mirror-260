...
interscript.stdlib.define_map("alalc-ell-Grek-Latn-2010")
def _stage_main(s):
    ...
    s = re.compile("υ(?=[Ιιί])", re.MULTILINE).sub("u", s)
    s = re.compile("Γ(?=[γΓξΞχΧ])", re.MULTILINE).sub("N", s)
    s = re.compile("γ(?=[γΓξΞχΧ])", re.MULTILINE).sub("n", s)
    s = re.compile("(?<=Κ)Γ(?=Ν)", re.MULTILINE).sub("N", s)
    s = re.compile("(?<=κ)γ(?=ν)", re.MULTILINE).sub("n", s)
    s = re.compile("(?<!"+interscript.stdlib.aliases["boundary"]+")Γ(?![κΚ]"+interscript.stdlib.aliases["boundary"]+")(?=[κΚ])", re.MULTILINE).sub("N", s)
    s = re.compile("(?<!"+interscript.stdlib.aliases["boundary"]+")γ(?![κΚ]"+interscript.stdlib.aliases["boundary"]+")(?=[κΚ])", re.MULTILINE).sub("n", s)
