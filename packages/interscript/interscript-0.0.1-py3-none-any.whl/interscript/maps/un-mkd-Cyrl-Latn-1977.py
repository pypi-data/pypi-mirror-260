...
interscript.stdlib.define_map("un-mkd-Cyrl-Latn-1977")
def _stage_main(s):
    s = re.compile("Ѓ(?=[ЕИеи])", re.MULTILINE).sub("G", s)
    s = re.compile("ѓ(?=[ЕИеи])", re.MULTILINE).sub("g", s)
    s = re.compile("Ќ(?=[ЕИеи])", re.MULTILINE).sub("K", s)
    s = re.compile("ќ(?=[ЕИеи])", re.MULTILINE).sub("k", s)
    s = interscript.stdlib.parallel_replace_tree(s, _PTREE_1466053637379202785)
