...
class EditorRestoreFile:
    ...
    def del_automation_restore_state(self, integrations_list: list[str]) -> dict:
        ...
        for config_dict in editable_part:
            try:
                if config_dict[self.configuration.first_key][self.configuration.identifiers_key][self.configuration.attr_id] not in integrations_list:
    ...
    def del_script_restore_state(self, scripts_list: list[str]) -> dict:
        ...
        for config_dict in editable_part:
            if config_dict[self.configuration.state][self.configuration.id_key] not in edit_scripts_list:
                new_cfg_list.append(config_dict)
