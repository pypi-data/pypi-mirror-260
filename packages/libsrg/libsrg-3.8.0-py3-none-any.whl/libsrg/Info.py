...
class Info:
    def __init__(self, hostname: str = None):
        ...
        rhow = Runner(f"test -d /sys/firmware/efi", userat=self.userat,rethrow=True,timeout=10,throw=True)
        self.uefi = "UEFI" if rhow.success else bios

        r2 = Runner("grep 'exclude=kernel' /etc/dnf/dnf.conf", userat=self.userat,rethrow=True,timeout=10,throw=True)
        ret = r2.ret
        if ret == 0:
