...
def import_by_config(config: Dict[str, Any]) -> Any:
    """根据配置文件初始化对象
    ...
    # params = {
    #    k: (import_by_config(v) if isinstance(v, Mapping) and "mod_path" in v else v)
    ...
    def register_providers(self, provider_configs: Dict[str, ProviderConfig]) -> None:
        ...
        for name, provider_config in provider_configs.items():
            try:
                ...
                provider = import_by_config(_provider_config)
            except ValidationError:
...
if __name__ == "__main__":
    ...
    d = import_by_config(config={"mod_path": "multiprocessing.dummy.Pool"})
    print(d)
    print(d())
