...
def get_environment(
    ...
    if environment_name is None:
        with Progress(
            ...
            environments = get_launchflow_client_legacy().environments.list(
                project.name
            )
        ...
        if prompt_for_creation and selected_environment == len(environment_names) - 1:
            environment = create_environment(
                environment_name=None, project=project, environment_type=None
            )
        else:
    ...
    try:
        ...
        if e.status_code == 404 and prompt_for_creation:
            ...
            if answer:
                ...
                environment = create_environment(
                    environment_name, project, environment_type=None
                )
            else:
                raise e
        else:
    ...
    with Progress(
        ...
        def create_env():
            if "gcp" in project.configured_cloud_providers:
                return get_launchflow_client_legacy().environments.create_gcp(
                    project.name, environment_name, environment_type
                )
            elif "aws" in project.configured_cloud_providers:
                return get_launchflow_client_legacy().environments.create_aws(
                    project.name, environment_name, environment_type
                )
            else:
        ...
        while True:
            try:
                environment = await asyncio.wait_for(
                    asyncio.shield(coro), timeout=time_between_task_seconds
                )
                break
