...
class ArgMatcher:
    ...
    def __init__(self, command: cli.Command) -> list[Any]:
        ...
        if command.is_class:
            self.func = command.command_like.run
        else:
    ...
    def __validate_arguments__(self):
        for parameter in self.parameters:
            if parameter.name == "self" and not self.command.is_class:
                raise CommandSignatureError("command function cannot contain argument named \"self\" unless part of a class")
            if str(parameter).startswith("**"):
                raise CommandSignatureError("command functions currently cannot contain **kwargs")
            if str(parameter).startswith("*"):
                self.__vargs__ = True
                if len(self.parameters) > 1:
                    raise CommandSignatureError("vargs can only be used as a standalone argument in command function.")
