...
from .common_utils import compute_md5, replace_bentoservice_model_server_image, get_bentoservice_profile_name
    ...
    def containerize(args):
        ...
        if bentoservice_profile:
            if bentoservice_profile not in bentoservice_profiles_supported.keys():
                raise Exception('Unsupported bentoservice profile "{bentoservice_profile}" in bentoservice found. Supported: {", ".join(bentoservice_profiles_supported.keys())}')
            
            print(f'Using bentoservice profile: {bentoservice_profile}')

            if bentoservice_profile == 'SinaraOnnxBentoService':
                replace_bentoservice_model_server_image(f"{bentoservice_cache_dir}/Dockerfile", "buslovaev/sinara-onnx-model-server")
