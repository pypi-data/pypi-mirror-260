    ...
    def get_secret(self, secret_name, secret_scope=None, dbutils=None):
        ...
        with tracer.start_as_current_span("get_secret"):
            try:
                ...
                logger.info(f"Pre-check secret from environment variable for key: {env_secret_name}")

                # First try to retrieve the secret from an environment variable
                secret_value = os.environ.get(env_secret_name)

                if secret_value is not None:
                    logger.info(f"Pre-check Retrieved secret from environment variable for key: {env_secret_name}")
                    return secret_value
                else:
                    logger.info(f"Pre-check Failed to retrieve secret from environment variable for key: {env_secret_name}")
