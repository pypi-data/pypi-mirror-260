...
class CaerpPaymentHistory(ModelBase):
    __tablename__ = "caerp_payment_history"
    ...
    caerp_payment_archive_seal = relationship(
        "CaerpPaymentArchiveSeal",
        primaryjoin="CaerpPaymentHistory.id=="
        "CaerpPaymentArchiveSeal.caerp_payment_history_id",
        uselist=False,
        back_populates="caerp_payment_history"
    )
...
class CaerpPaymentArchiveSeal(ModelBase):
    __tablename__ = "caerp_payment_archive_seal"
    __table_args__ = default_table_args
    id = Column(Integer, primary_key=True)
    created_at = Column(DateTime, default=datetime.datetime.now)
    archive_type = Column(String(20), default='local')
    remote_identification_key = Column(Text(), nullable=False)
    caerp_payment_history_id = Column(ForeignKey('caerp_payment_history.id'))

    caerp_payment_history = relationship(
        CaerpPaymentHistory,
        primaryjoin="CaerpPaymentHistory.id=="
        "CaerpPaymentArchiveSeal.caerp_payment_history_id",
        back_populates="caerp_payment_archive_seal"
    )
