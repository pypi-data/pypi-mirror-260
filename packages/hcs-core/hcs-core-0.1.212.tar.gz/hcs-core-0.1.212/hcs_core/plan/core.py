...
def _ensure_provider(provider_id: str, module_name: str, state: dict):
...
def _get_handler(res: dict, state: dict):
    kind = _get_kind(res)
    ...
    def _get_resource_handler(provider_path):
        ...
        _ensure_provider(provider_id, provider_module_name, state)
...
def get_deployment_data(data: dict, additional_context: dict = None, resource_name: str = None):
    ...
    for k in blueprint["resource"]:
        ...
        output[k] = res_state
        if res_state:
            handler = _get_handler(res_node, state)
