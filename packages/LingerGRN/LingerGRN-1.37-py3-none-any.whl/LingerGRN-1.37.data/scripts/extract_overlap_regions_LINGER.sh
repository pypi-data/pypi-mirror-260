...
if [ $genome = "hg38"  ]; 
then 
bedtools intersect -a Region.bed -b  $GRNdir/hg38_hg19_pair.bed -wa -wb |awk '{print $7"\t"$8"\t"$9"\t"$1"\t"$2"\t"$3}'> $outdir/match_hg19_peak.bed # the first 3 are the hg19 region origianl ; the last 3 is  the query data region
fi
if [ $genome = "hg19"  ]
then
bedtools intersect -a Region.bed -b  $GRNdir/hg19_hg38_pair.bed -wa -wb |awk '{print $7"\t"$8"\t"$9"\t"$1"\t"$2"\t"$3}'> $outdir/match_hg19_peak.bed # the first 3 are the hg19 region origianl ; the last 3 is  the query data region
fi
...
bedtools intersect -a "$GRNdir/${genome}_Peaks_chr$i.bed" -b Region.bed -wa -wb > $outdir/"Region_overlap_chr$i.bed"
done
i=X
bedtools intersect -a "$GRNdir/${genome}_Peaks_chr$i.bed" -b Region.bed -wa -wb > $outdir/"Region_overlap_chr$i.bed"