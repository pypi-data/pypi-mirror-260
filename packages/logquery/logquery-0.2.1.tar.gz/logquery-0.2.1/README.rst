...
::
    ...
    shell = bash
    cmds =
        ${buildout:bin-directory}/zopepy -m pip install --upgrade --quiet $(curl -OJLs $(curl -s https://pypi.org/pypi/logquery/json | grep -Eo '"url":"([^"]+)"' | cut -d'"' -f4 | grep '.tar.gz' | tail -n 1) -w "%{filename_effective}")
