...
class Worker_WriteProject(QtCore.QThread):
    ...
    because we don't want the GUI to be freezed while writing the project.
    """
...
class Worker_Optimize(QtCore.QThread):
    ...
    def __init__(
        self, parent, opt, constraints, collection, is_multi, n_iter, n_proc, iter_mehtod, timeout
    ):
        super().__init__(parent)
        self.opt = opt
    ...
    def run(self):
        ...
        Time_start = time()
        results = Optimization.multi_start_optimization(
            self.opt,
