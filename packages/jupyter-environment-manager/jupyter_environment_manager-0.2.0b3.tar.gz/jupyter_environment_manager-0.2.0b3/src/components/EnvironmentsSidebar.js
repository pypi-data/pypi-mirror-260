...
export default class EnvironmentsSidebar extends React.Component {
  ...
  constructor(props) {
    ...
    this.state = {
      ...
      expandedEnv: '', // typeof string, contains id for the last environment which has been expanded.
  ...
  toggleExpandedEnv = _id => {
    if (_id === this.state.expandedEnv) {
      this.setState({ expandedEnv: '' });
      return;
    }
    this.setState({ expandedEnv: _id });
  };
  ...
  handleInstallProgressStream = async (environmentId, userId) => {
    const requestUrl = `${API_URL}/api/environments/install-status/${environmentId}/${userId}`;
  ...
  handleImageUpload = e => {
    ...
    if (e.target.files[0].size > UPLOAD_SIZE_LIMIT * 1024) {
      ...
      >
        ...
        >
          ...
          <DialogContentText id="uploadImg-dialog-description">
            To ensure responsiveness, upload an image &lt; 500KB
          </DialogContentText>
        </DialogContent>
        ...
        .map((env, index) => {
          ...
          return (
            <EnvironmentTile
              ...
              key={index}
              expandedEnv={this.state.expandedEnv}
