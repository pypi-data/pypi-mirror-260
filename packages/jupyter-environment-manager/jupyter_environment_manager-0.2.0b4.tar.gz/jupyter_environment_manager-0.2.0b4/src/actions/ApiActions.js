...
switch (true) {
  ...
  case path.includes('lab.xanadu.qbraid'):
    accountURL = 'https://account.qbraid.com/auth/login/partner/xanadu'; // account url for xandu
    apiURL = 'https://api.qbraid.com';
    apiUserPool = 'xanadu';
    break;
  default:
...
export default function () {
  ...
  this.query = () => {
    if (!this.agent) {
      ...
      this.agent.use(req => {
        if (req.url[0] === '/') {
          req.url = `${this.alt.apiBaseUrl}/api${req.url}`;
        }
