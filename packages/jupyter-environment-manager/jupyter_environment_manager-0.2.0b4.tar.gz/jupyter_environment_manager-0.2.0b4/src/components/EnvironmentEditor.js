...
export default class EnvironmentEditor extends Component {
  ...
  handleUninstall = () => {
    ...
    this.context
      ...
      .then(() => {
        ...
        // this.setState({ uninstalling: false }, this.props.close);
      })
      .catch(err => {
        console.log(err);
        alert('This environment can not be uninstalled.');
      });
  };
  ...
  handleSubmit = () => {
    ...
    // Enable underlying section if description is mandatory for a env to save
    // if (!description) {
    //   return this._descriptionInput.focus();
    // }
  ...
  fetchFromPyPi = async value => {
    await fetch(`https://pypi.org/pypi/${value}/json`)
      ...
      .then(data => {
        this.setState({
          searchResults: data,
          // selectedPkgVersion: Object.keys(data.releases)[
          //   Object.keys(data.releases).length - 1
          // ],
      ...
      .then(_data => {
        ...
        if (_.isEmpty(filteredPkg)) {
          this.setState({
            selectedPkgVersion: this.state.availableVersions[0]
          });
        } else {
          this.setState({
  ...
  handleAddPackageToList = async value => {
    if (!_.isEmpty(this.state.selectedPkg)) {
      ...
      if (isExisting && selectedVersion > isExisting.version) {
      ...
      } else if (isExisting && isExisting.version > selectedVersion) {
      ...
      } else if (isExisting && selectedVersion === isExisting.version) {
        ...
        <div className="env-pane-content">
          {this.state.isOwner ? (
            this.state.isCustom ? (
              this.state.isInstalled ? (
                  ...
                  <Grid container>
                    <Grid item xs={12}>
                      <p className="env-pane-prompt">
                        Sharing environments is only available within the
                        cloud-based qBraid Lab platform. Launch an instance at{' '}
                        <a
                ...
                <Grid container>
                  <Grid item xs={12}>
                    <p className="env-pane-prompt">
                      To share an environment, it must be installed. To install
                      this environment, click <b>Install</b> in the panel
                      drop-down.
                    </p>
                  </Grid>
                </Grid>
            ...
            <Grid container>
              <Grid item xs={12}>
                <p className="env-pane-prompt">
                  To share an environment, you must be the environment owner. To
                  create a shareable custom environment, click{' '}
                  <b>+ Add {'>'} + Create Environment</b> in the environments
                  sidebar
                </p>
              </Grid>
  ...
  renderUpdatePane = () => {
    ...
    if (this.state.isCustom) {
      ...
      const sharedWith =
        this.props.env.visibility === 'shared'
          ? this.props.env.readAccessUsers.length
          : 0;
