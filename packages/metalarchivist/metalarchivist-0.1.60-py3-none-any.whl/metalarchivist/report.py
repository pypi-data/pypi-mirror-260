...
def get_albums(range_start: datetime | None = None, range_stop: datetime | None = None, verbose=False, wait=3., retries=3) -> list[dict]:
    if range_start:
        release_page = Album.get_range(range_start, range_stop, verbose=verbose, wait=wait, retries=retries)
    else:
        release_page = Album.get_upcoming(verbose=verbose, wait=wait, retries=retries)
