...
class Command(BaseCommand):
    help = (
        "Helps initializing Minio buckets by creating them and setting their policies."
    )

    def add_arguments(self, parser):
        parser.add_argument(
            "--silenced", action="store_true", default=False, help="No console messages"
        )
    ...
    def handle(self, *args, **options):
        ...
        for bucket in [*public_buckets, *private_buckets]:
            ...
            if m.is_bucket_public:  # Based on settings.py configuration
                ...
                (
                    self.stdout.write(
                        f"Bucket ({m.bucket}) policy has been set to public",
                        ending="\n",
                    )
                    if not silenced
                    else None
        ...
        for policy_tuple in get_setting("MINIO_POLICY_HOOKS", []):
            ...
            (
                self.stdout.write(
                    f"Bucket ({m.bucket}) policy has been set via policy hook",
                    ending="\n",
                )
                if not silenced
                else None
