...
def _build_test_graph4():
    ...
    graph = make_graph(
        ...
        outputs=[make_value_info("out", make_tensor_type_proto(1, [1, 8 * 64 * 64]))],
        initializer=[
            ...
            from_array(np.array([1, 4 * 64 * 64]).astype("int64"), "p"),
            from_array(np.array([1, 4 * 64 * 64]).astype("int64"), "q"),
        ],
    )
    model = make_model(graph, opset_imports=[make_operatorsetid("", 19)])
    return model
