...
class Transcoder(SyncModel):
    ...
    def apply(self, pattern=[], force=False, shuffle=False, dryrun=False):
        ...
        def guess_duration(stream):
            if 'duration' in stream:
                return float(stream['duration'])
            tags = stream.get('tags', {})
            d = tags.get('DURATION', tags.get('duration'))
            if d:
                d = d[:15]
                d = datetime.strptime(d,'%H:%M:%S.%f') - datetime.strptime('00:00:00','%H:%M:%S')
                return d.total_seconds()
            
            _bytes = tags.get('NUMBER_OF_BYTES')
            _pbs = tags.get('BPS')
            if _bytes and _pbs:
                return float(_bytes) / float(_pbs) * 8
        ...
        def convert(path, force=False, dryrun=False):
            ...
            for stream in stream_info['streams']:
                duration = max(duration, guess_duration(stream))
                if stream['codec_type'] in ('video',):
                    if stream['codec_name'] not in ('h264',):
                        pass
