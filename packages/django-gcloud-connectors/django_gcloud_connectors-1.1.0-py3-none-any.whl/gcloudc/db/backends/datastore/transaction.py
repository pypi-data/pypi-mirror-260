...
def in_atomic_block(using="default"):
    txn = current_transaction(using=using)
    if not txn:
        return False
...
class Transaction(object):
    ...
    def put(self, entity):
        putter = (
            self._datastore_transaction.put if self._datastore_transaction
...
def _rpc(using):
    """
        ...
        this effectively returns the current transaction or a new
    ...
    current = current_transaction(using)
    # current_transaction returns None if there is no "real" transaction active.
    # We want to preserve that behaviour since current_transaction is used in
    # other places, but we want to make sure we return the correct Transaction
    # instance here, since NoTransaction uses a different connection to support
    # non_atomic block
    if not current:
        for txn in reversed(_STORAGE.transaction_stack.get(using, [])):
            if isinstance(txn, NoTransaction):
                current = txn
                break

    return current or RootTransaction(connections[using].connection)
...
def _init_storage():
    ...
    if not hasattr(_STORAGE, "non_atomic_connections"):
        _STORAGE.non_atomic_connections = {}
...
class AtomicDecorator(context_decorator.ContextDecorator):
    ...
    def _do_enter(cls, state, decorator_args):
        ...
        elif in_atomic_block(using=using):
        ...
        try:
            if new_transaction._datastore_transaction:
                # Using the context manager APIs here, instead of calling
                # `new_transaction._datastore_transaction.begin()` because of
                # [this bug](https://github.com/googleapis/python-datastore/issues/447).
                # Using the manager APIs for now to make sure the batch is added to the stack. Using the ctx manager
                # APIs should less likely require to be updated here or break once the bug is fixed in the SDK.
                new_transaction._datastore_transaction.__enter__()
    ...
    def _do_exit(cls, state, decorator_args, exception):
        ...
        exception = exception or (True if connection.needs_rollback else None)

        try:
            if transaction._datastore_transaction:
                transaction._datastore_transaction.__exit__(exception, None, None)

                # TODO: this could in principle catch exceptions non related to the datastore
                # within the atomic block, but I don't think there's an easy way to distinguish them
                if original_exception and issubclass(original_exception, Aborted):
                    raise TransactionFailedError()

                if not exception and isinstance(transaction, (IndependentTransaction, NormalTransaction)):
...
def _local_non_atomic_connection(using):
    """Returns a thread-local copy of the "normal" connection for use in non_atomic.

    In order to perform non-atomic actions when a transaction has already been started
    we need a new connection.

    See tests.system.test_transaction.test_failure_with_contention in the python
    datastore SDK for reference.

    Because this connection is never used for transactions, it can be reused safely
    in multiple non_atomic blocks.
    IMPORTANT: do not use this connection for any transactional work
    """
    if using not in _STORAGE.non_atomic_connections:
        connection = connections[using]
        _STORAGE.non_atomic_connections[using] = connection.copy()

    return _STORAGE.non_atomic_connections[using]
...
class NonAtomicDecorator(AtomicDecorator):
    ...
    def _do_enter(cls, state, decorator_args):
        ...
        state.using = using = decorator_args.get("using", "default") or "default"
        connection = _local_non_atomic_connection(using)
        ...
        # For non_atomic blocks we pass nothing as the transaction,
        # since we want operations to be called immediately, on the connection
        new_transaction = NoTransaction(connection, None)
    ...
    def _do_exit(cls, state, decorator_args, exception):
        ...
        try:
            # TODO: remove? Not sure why this assertion is here
