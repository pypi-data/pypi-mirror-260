...
def check_unique_markers_in_cache(model, entities, ns):
    """
    Compare entities agaist the cache.

    This is necessary to enforce unique constraint inside an atomic block
    """
    for entity, _ in entities:
        unique_marker_key_values = unique_identifiers_from_entity(model, entity, ignore_pk=True)
        for named_key in unique_marker_key_values:
            maybe_cached = caching.get_from_cache(named_key, ns)
            if maybe_cached and entity.key != maybe_cached.key:
                # Same cache key, but different object, raise
                table_name = named_key.split("|")[0]
                unique_fields = named_key.split("|")[1:]
                raise IntegrityError(CONSTRAINT_VIOLATION_MSG.format(table_name, unique_fields))
