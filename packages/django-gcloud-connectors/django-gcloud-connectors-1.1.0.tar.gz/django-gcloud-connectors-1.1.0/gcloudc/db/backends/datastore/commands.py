...
class SelectCommand(object):
    ...
    def _exclude_pk(self, columns) -> Optional[List[str]]:
        if columns is None:
            return None
    ...
    def _build_query(self):
        ...
        model_indexed_fields = indexed_fields_on_model(self.query.model)
        if projection and model_indexed_fields:
            ...
            if missing_index_fields:
                raise DatabaseError(
                    ...
                    f"is indexed by defining the field in the meta of `{self.query.model}`."
                )
