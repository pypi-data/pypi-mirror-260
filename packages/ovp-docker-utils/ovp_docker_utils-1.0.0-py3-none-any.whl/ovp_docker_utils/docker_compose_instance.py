...
class DockerComposeServiceInstance(BaseModel):
    ...
    loaded_docker_compose: dict = {}

    @model_validator(mode='after')
    def validate_constraints(self) -> 'DockerComposeServiceInstance':
        with open(self.docker_compose_src_on_pc, "r") as f:
            self.loaded_docker_compose = yaml.load(f, yaml.BaseLoader)
        return self
    
    @property
    def service_name(self):
        return list(self.loaded_docker_compose["services"].keys())[0]
    
    @property
    def docker_repository_name(self):
        return self.loaded_docker_compose["services"][self.service_name]["image"].split(":")[0]
    
    @property
    def container_name(self):
        return self.loaded_docker_compose["services"][self.service_name]["container_name"]
    
    @property
    def log_driver(self):
        if "logging" not in self.loaded_docker_compose["services"][self.service_name]:
            return None
        if "driver" not in self.loaded_docker_compose["services"][self.service_name]["logging"]:
            return None
        return self.loaded_docker_compose["services"][self.service_name]["logging"]["driver"]
