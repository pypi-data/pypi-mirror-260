...
function __vfsupport_check_venv --description "Check if given argument resolves to a virtualenv"
    ...
    # Following line seems to break regular, non-path venv name arguments:
...
function __vf_activate --description "Activate a virtualenv"
    ...
    if not test -d $_VF_EXTRA_PATH
        echo (set_color red)"There is no /bin directory inside this environment, likely rendering it unusable." $VIRTUAL_ENV(set_color normal)
    end
...
function __vf_new --description "Create a new virtualenv"
    ...
    # JM: Experiment to replace Virtualenv's activate.fish, ensuring activation is done our way
    echo "vf activate $envname" > $VIRTUALFISH_HOME/$envname/bin/activate.fish
...
function __vf_rm --description "Delete one or more virtual environments"
    ...
    for venv in $argv
        ...
        if ! set -f venv_path (__vfsupport_check_venv $venv)
            echo "$venv is not a virtualenv inside VIRTUALFISH_HOME, or a path to a directory containing a virtualenv."
            return 2
        end
        echo "Removing $venv_path"
        emit virtualenv_will_remove
        emit virtualenv_will_remove:$venv_path
        if command -q trash
            command trash $venv_path
        else
            # The `rm -rf` below is too destructive. Should change it to something like:
            command mv $venv_path "$venv_path"-to-be-deleted
            # command rm -rf $VIRTUALFISH_HOME/$venv
        end
        emit virtualenv_did_remove
        emit virtualenv_did_remove:$venv_path
    end
end
...
function __vf_upgrade --description "Upgrade virtualenv(s) to newer Python version"
    ...
    if test $status -ne 0
        echo "Specified (or default) Python interpreter does not appear to be valid."
        return -1
    end
    # Set envs to upgrade: all, a list of virtualenvs, or current environment
    if set -q _flag_all
        set venv_list (vf ls)
        ## Append list of Pipx virtual environments, if any, to the list
        for pipx_venv in (command ls -1 ~/.local/pipx/venvs/)
            # Exceptions list: Pin Ansible; Ruff doesn't like being upgraded this way.
            if begin not contains ansible $pipx_venv; not contains ruff $pipx_venv; end
                set pipx_venv_list $pipx_venv_list "~/.local/pipx/venvs/$pipx_venv"
            end
        end
        set venv_list $venv_list $pipx_venv_list
    ...
    for venv in $venv_list
        set -l packages
        # If venv arg contains a slash, assume full path is already present…
        # if string match "*/*" $venv
        #     set venv_path "$venv"
        # … otherwise assume arg is just the venv name, so create the path:
        # else
        #     set venv_path "$VIRTUALFISH_HOME/$venv"
        # end
        ...
        if begin; set -q _flag_rebuild; or test -n "$packages"; end
            ...
            # Install via lock file if found; otherwise Pip-install packages
            if begin; set -q PROJECT_HOME; and test -f "$PROJECT_HOME/$venv/pdm.lock"; end
                set install_cmd "pdm install"
            else if begin; set -q PROJECT_HOME; and test -f "$PROJECT_HOME/$venv/poetry.lock"; end
                set install_cmd "poetry install"
            else
                # If broken env, use above package list. Otherwise use `pip freeze`.
                if test -z "$packages"
                    set packages ($venv_path/bin/python -m pip freeze)
                end
                set install_cmd "if test -n '$packages'; $venv_path/bin/python -m pip install -U $packages; end"
            end
            if set -q VIRTUAL_ENV
                vf deactivate
            end
            ...
            if test "$old_py_mv" -ne "$new_py_mv"
                ...
                for filename in pyvenv.cfg pipx_metadata.json
                    if test -f "$venv_path/$filename"
                        command sed -i '' -e "s/$old_py_fv/$new_py_fv/g" "$venv_path/$filename"
                    end
                end
                ...
                # end
                # if test -f "$venv_path/pipx_metadata.json"
                #     command sed -i '' -e "s/$old_py_fv/$new_py_fv/g" "$venv_path/pipx_metadata.json"
                # end
