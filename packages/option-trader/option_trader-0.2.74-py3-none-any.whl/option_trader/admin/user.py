...
class user():  
    ...
    def syncup_brokerage_accounts(self):
        ...
        if 'IB' in brokerage:
            ...
            summary_dict = load_ib_account_summary(Live=live)
            ...
            position_df = refresh_ib_accounts_positions(live)            
            ...
            openOrder_df = refresh_ib_open_orders(live)
            for account_name in account_name_list:
                a = self.get_account(account_name)
            ...
            v, p = refresh_ib_accounts_values_protfolio(live)
            for account_name in account_name_list:
                a = self.get_account(account_name)
                if p.shape[0] > 0 and p[p['accountName']==account_name].shape[0] > 0:               
                    a.save_ib_account_portfolio(p[p['accountName']==account_name])
                else:          
                    a.drop_ib_account_portfolio()                        

                if account_name in v:      
                    a.save_ib_account_values(v[account_name])
                else:
                    a.drop_ib_account_values()

            exec_defails_df = load_ib_execution_details(live)
            for account_name in account_name_list:
                a = self.get_account(account_name)
                if exec_defails_df.shape[0] > 0 and exec_defails_df[exec_defails_df['acctNumber']==account_name].shape[0] > 0:                    
                    a.save_ib_execution_details(exec_defails_df[exec_defails_df['acctNumber']==account_name])               
                else:
                    a.drop_ib_execution_details()       
                   
            completed_order_df = load_ib_completedOrders(live)
            for account_name in account_name_list:
                a = self.get_account(account_name)
                if completed_order_df.shape[0] > 0 and completed_order_df[completed_order_df['account']==account_name].shape[0]:                    
                    a.save_ib_completed_orders(completed_order_df[completed_order_df['account']==account_name]) 
                else:
                    a.drop_ib_completed_orders() 
