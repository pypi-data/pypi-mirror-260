...
class account():
    ...
    def close_ib_position(self, srow, ppdf):     
        ...
        place_ib_order(srow, live, bracket=False, orderType='MKT', orderAction='Close')
        if len(srow.ib_errorString) > 0:
    ...
    def drop_ib_account_values(self):  
        if app_settings.DATABASES == 'sqlite3':                 
            try:                  
                cursor = self.db_conn.cursor()                                         
                cursor.execute("DROP TABLE IF EXISTS ib_account_values")
                self.db_conn.commit()         
            except Exception as e:
                self.logger.exception(e)   
                raise e
    ...
    def drop_ib_execution_details(self):
        if app_settings.DATABASES == 'sqlite3':                 
            try:
                cursor = self.db_conn.cursor()                                         
                cursor.execute("DROP TABLE IF EXISTS ib_execution_details")
                self.db_conn.commit()
            except Exception as e:
                self.logger.exception(e)   
                raise e
    ...
    def drop_ib_completed_orders(self):
        if app_settings.DATABASES == 'sqlite3':                 
            try:
                cursor = self.db_conn.cursor()                                         
                cursor.execute("DROP TABLE IF EXISTS ib_completed_order")
                self.db_conn.commit()
            except Exception as e:
                self.logger.exception(e)   
                raise e
    ...
    def create_position_summary(self, row):
        ...
        if 'IB' in self.brokerage:
            live = False if 'PAPER' in self.brokerage else True
            x = place_ib_order(row, live, bracket=False)
    ...
    def create_stockSummary(self, symbol):
        ...
        if 'IB' in self.brokerage:
            ...
            x = place_ib_order(srow, live, bracket=False)
