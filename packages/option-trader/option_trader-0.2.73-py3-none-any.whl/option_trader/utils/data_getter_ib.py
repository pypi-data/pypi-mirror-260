...
class IBClient:    
    ...
    def __init__(self, 
        ...
        '''
        self.TWS = True
        self.host = ib_settings.LocalHostIP
        self.port = ib_settings.IBConfig.TWS_live.port if Live else ib_settings.IBConfig.TWS_paper.port
        self.clientID = ib_settings.IBConfig.TWS_live.clientId if Live else ib_settings.IBConfig.TWS_paper.clientId
        print('IB on %s %s TWS %s Live %s' % (self.host, self.port, str(self.TWS), str(self.Live)))   

        return
        '''    
        if Live:
            if self.check_port(ib_settings.LocalHostIP, ib_settings.IBConfig.TWS_live.port):
                self.TWS = True           
            elif self.check_port(ib_settings.LocalHostIP, ib_settings.IBConfig.Gateway_live.port):
                self.TWS = False
            elif self.check_port(ib_settings.DockerHostIP, ib_settings.IBConfig.TWS_live.port):
                self.TWS = True
            elif self.check_port(ib_settings.DockerHostIP, ib_settings.IBConfig.Gateway_live.port):
                self.TWS = False                                     
            else:
                raise IBError(0, 'IB live server not started!')
        else:
            if self.check_port(ib_settings.LocalHostIP, ib_settings.IBConfig.TWS_paper.port):
                self.TWS = True          
            elif self.check_port(ib_settings.LocalHostIP, ib_settings.IBConfig.Gateway_paper.port):
                self.TWS = False   
            elif self.check_port(ib_settings.DockerHostIP, ib_settings.IBConfig.TWS_paper.port):
                self.TWS = True
            elif self.check_port(ib_settings.DockerHostIP, ib_settings.IBConfig.Gateway_paper.port):
                self.TWS = False                                     
            else:
                raise IBError(0, 'IB paper server not started!!')            

        if self.TWS:
            self.clientID = ib_settings.IBConfig.TWS_live.clientId if Live else ib_settings.IBConfig.TWS_paper.clientId
        else:
            self.clientID = ib_settings.IBConfig.Gateway_live.clientId if Live else ib_settings.IBConfig.Gateway_paper.clientId
                
        self.logger.info('IB on %s %s TWS %s LIVE %s' % (self.host, self.port, str(self.TWS), str(self.Live)))   
        print('IB on %s %s TWS %s Live %s' % (self.host, self.port, str(self.TWS), str(self.Live)))   
    ...
    def get_option_contId(self, symbol, exp_date, strike, otype):
        ...
        with contract_details(contract) as app:
            app.connect(self.host, self.port, self.clientID)                
    ...
    def get_account_list(self):
        with ib_app_base() as app:
            print('host %s port %d clientId %d' %(self.host, self.port, self.clientID) )
            app.connect(self.host, self.port, self.clientID)                
            print('connected!')            
            t = Timer(2, app.stop)
            t. start()            
...
if __name__ == '__main__':
    ...
    with IBClient(Live=False) as ibClient:
        try:
    ...
    with IBClient(Live=True) as ibClient:
        try:
            print(ibClient.get_account_list())
        except IBError as error:
            print(str(error))
...
#self.placeOrder(reqId, stock_contract, put_order) 
    ...
    with IBClient(True) as ibClient:
        print(ibClient.place_simple_order(stock_contract, stock_order))
