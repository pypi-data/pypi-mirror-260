...
if [[ "$OS" = "Darwin" ]]; then
    ...
    # Copy /app folder from package directory to working directory
    script.append(f"""
if [ -d "./{model_name}/app" ]; then
    echo "Copying /app folder from {model_name} to current working directory..."
    cp -r ./{model_name}/app/* ./
else
    echo "/app folder does not exist in {model_name}."
fi
    """.strip())
