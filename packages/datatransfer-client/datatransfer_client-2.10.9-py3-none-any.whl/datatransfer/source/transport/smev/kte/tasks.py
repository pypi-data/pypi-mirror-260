...
def change_schedule():
    ...
    # Запрос для задачи с проверкой, что его crontab больше не используется для
    # других периодических задач
    task_query = PeriodicTask.objects.select_related('crontab').filter(
        name=KTE_PERIODIC_TASK_NAME
    ).annotate(
        can_delete_crontab=Exists(
            PeriodicTask.objects.exclude(name=KTE_PERIODIC_TASK_NAME).filter(
                crontab_id=OuterRef('crontab_id')),
            negated=True
        )
    )

    for task in task_query:
        if task.can_delete_crontab:
            task.crontab.delete()
    task_query.delete()
