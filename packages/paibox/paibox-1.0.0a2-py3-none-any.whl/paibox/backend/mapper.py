...
class Mapper:
    ...
    def routing_restructure(self) -> None:
        """Network restructure.
        
        NOTE: The neurons in each core block must be grouped first to determine \
            the #N of cores required, and then coordinates can be assigned.
        """

        def _lcn_ex_adjustment() -> None:
        ...
        def _routing_optim() -> None:
            for cb in filter(
                lambda cb: len(cb.source) == 1 and len(cb.dest) > 1, self.core_blocks
            ):
                # Get the expected grouping results but do not change the data inside.
                nseg_of_cb = cb.group_neurons(
                    optim_target=_BACKEND_CONTEXT.cflags["grouping_optim_target"],
                    dry_run=True,
                )

                n_core_required = len(nseg_of_cb)
    ...
    def config_export(self) -> GraphInfo:
        ...
        _graph_info = GraphInfo(
            ...
            n_core_occupied=self.n_core_required,
