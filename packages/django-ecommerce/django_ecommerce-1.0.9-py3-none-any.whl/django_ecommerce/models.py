...
from django_cloud_thumbnails.utils import crop
...
class Category(models.Model):
    ...
    seo_image_cropped = models.ImageField(storage=CloudStorage, blank=True, null=True, editable=False)
    __original_seo_image = None
    __original_seo_image_cropping = None

    def __init__(self, *args, **kwargs):
        super().__init__(*args, **kwargs)
        self.__original_seo_image = self.seo_image
        self.__original_seo_image_cropping = self.seo_image_cropping

    def delete(self, using=None, keep_parents=False):
        if self.seo_image:
            self.seo_image.delete(save=False)

        if self.seo_image_cropped:
            self.seo_image_cropped.delete(save=False)

        super().delete(using=using, keep_parents=keep_parents)

    def save(self, *args, **kwargs):
        if self.__original_seo_image and self.__original_seo_image != self.seo_image:
            self.__original_seo_image.delete(save=False)

        if self.__original_seo_image_cropping != self.seo_image_cropping:
            if self.seo_image_cropped:
                self.seo_image_cropped.delete(save=False)

            if self.seo_image:
                self.seo_image_cropped.save(
                    self.seo_image.name,
                    crop(self.seo_image, self.seo_image_cropping),
                    False
                )
...
class ProductImage(models.Model):
    ...
    cropped = models.ImageField(storage=CloudStorage, blank=True, null=True, editable=False)
    position = models.PositiveIntegerField(default=0, blank=False, null=False)
    __original_image = None
    __original_cropping = None

    def __init__(self, *args, **kwargs):
        super().__init__(*args, **kwargs)
        self.__original_image = self.image
        self.__original_cropping = self.cropping
    ...
    def delete(self, using=None, keep_parents=False):
        ...
        if self.cropped:
            self.cropped.delete(save=False)

        super().delete(using=using, keep_parents=keep_parents)

    def save(self, *args, **kwargs):
        if self.__original_image and self.__original_image != self.image:
            self.__original_image.delete(save=False)

        if self.__original_cropping != self.cropping:
            if self.cropped:
                self.cropped.delete(save=False)
