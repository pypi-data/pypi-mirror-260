...
class MolecularDynamic_builder(PartitionManager):
    ...
    def get_molecular_graph_tracking(self, sigma:float=None, id_filter:bool=True, pattern:dict=None, 
                                  ...
                                  prevent_repeating:bool=True, verbose:bool=True ) -> list:
        """
        """
        ...
        for container_i, container in enumerate(self.containers):
            if verbose: print(f'(%) >> Config {container_i} ({float(container_i)/len(self.containers)*100}% progress)')
            container.AtomPositionManager.search_molecular_subgraph( pattern=pattern, verbose=True )
    ...
    def handleMDAnalysis(self, values:list, file_location:str=None):
        ...
        for plot in values:
            ...
            if plot.upper() == 'MOLECULE_FORMATION_TRACKING':
                ...
                MDA_data['molecule_formation_tracking'] = self.get_molecular_graph_tracking( sigma=values[plot].get('sigma', None), pattern=self.str_to_connectivity(values[plot].get('topology', None)) )
