...
class MolecularDynamic_builder(PartitionManager):
    ...
    def parallel_search(self, container, container_i, pattern, verbose):
        """
        ...
        """
        container = self.containers[container_i]
        if verbose:
    ...
    def get_molecular_graph_tracking(self, sigma:float=None, id_filter:bool=True, pattern:dict=None, 
                                  ...
                                  prevent_repeating:bool=True, 
                                  parallel:bool=True, verbose:bool=True ) -> list:
        """
        """

        if verbose: print(f'(%) Looking specific pattern : {pattern}')
        sms = []
        container_list = []

        if parallel:
            with ProcessPoolExecutor() as executor:
                futures = [executor.submit(self.parallel_search, container, container_i, pattern, verbose)
                           for container_i, container in enumerate(self.containers)]
            ...
            for container_i, container in enumerate(self.containers):
                if verbose: print(f'(%) >> Config {container_i} ({float(container_i)/len(self.containers)*100}%)')
                sms = container.AtomPositionManager.search_molecular_subgraph( pattern=pattern, verbose=True )
                if sms != []:
                    container_list.append(container_i)
        ...
        count_species = []
        for container_i, container in enumerate(self.containers):
            if verbose: print(f'(%) >> Frame analized {container_i}')
        ...
        for i, n in enumerate(count_species[1:]):
            ...
            for j, m in enumerate(n):
                if len(m) < molecule_max_size and not m in count_species[i]:
                    if verbose: print('MOL', j)    
