...
class SCC_raw(SCC_product):
    ...
    def _get_info(
        ...
        ini_date, station, ini_hour = measurementID[:8], measurementID[8:11], measurementID[11:]
    ...
    def _subtrack_dark_measurement(self, data: xr.Dataset, fill_value = 9.969209968386869e+36) -> xr.Dataset:
        ...
        if 'Background_Profile' not in data:
            logger.warning('Background_Profile not found in data. Dark measurement subtraction will not be performed.')
            return data
        channel_IDs = [channel_ for channel_ in self.channels_ID if data['DAQ_Range'].sel(channels=channel_).values != fill_value]
        ...
        for channel_ in channel_IDs:
            ...
            data['Raw_Lidar_Data_dc'][:,idx_, :] = data['Raw_Lidar_Data'].sel(channels=channel_) - data['Background_Profile'].sel(channels=channel_).mean('time_bck')            
        return data
    ...
    def _subtract_background(self, data: xr.Dataset) -> xr.Dataset:
        ...
        data[f'{raw_string}_bckg'] = data[raw_string].copy(data=np.nan*np.ones(data[raw_string].shape))

        for channel_idx, channel_ in enumerate(self.channels_ID):
            if data['Background_Mode'].sel(channels=channel_).item() == 0:
                low_background = int(data['Background_Low'].sel(channels=channel_).item())
                high_background = int(data['Background_High'].sel(channels=channel_).item())
            else:
                low_background = int(data["Background_Low"].sel(channels=channel_).item()/self.range_resolution)
                high_background = int(data["Background_High"].sel(channels=channel_).item()/self.range_resolution)
            bckgrd_ = data[raw_string].sel(channels=channel_, points=slice(low_background, high_background)).mean("points")
            data[f'{raw_string}_bckg'][:,channel_idx, :] = data[raw_string].sel(channels=channel_) - bckgrd_
        return data
    ...
    def _zerobin_correction(self, data: xr.Dataset) -> xr.Dataset:
        ...
        if raw_string not in data:
            raise ValueError('Background subtraction must be performed before zero bin correction.')
        ...
        for channel_idx, channel_ in enumerate(data.channels.values):
            ...
            shifted_data[:, channel_idx, :(topbin- zerobin_)] = data[raw_string].sel(channels=channel_, points=slice(zerobin_, topbin))
    ...
    def plot(
        ...
        **kwargs,
        ...
        if kwargs.get('preprocess', True):        
            ...
            data = self.open_dataset()
        data["points"] = self.range_resolution * np.arange(1,data.dims['points']+1)/1e3
        figs, outputpaths = [], []
        for channel_ in data.channels.values:
            ...
            raw_colors = color_list(
                data.dims["time"], cmap='Greys'
            )
            for time_idx, time_ in enumerate(data['Raw_Data_Start_Time'].values.flatten()):
                try:
                    if 'Raw_Lidar_Data_dc_bckg_zb' in data:
                        signal_to_rcs(data.sel(time=time_idx, channels=channel_)[ "Raw_Lidar_Data_dc_bckg_zb" ], data['points']*1e3).plot( x="points", ax=ax[0], color=colors[time_idx], linewidth=0.25)
                        data.sel(time=time_idx, channels=channel_)[ "Raw_Lidar_Data_dc_bckg_zb" ].plot( x="points", ax=ax[1], color=colors[time_idx], linewidth=0.25)
                        data.sel(time=time_idx, channels=channel_)[ "Raw_Lidar_Data" ].plot( x="points", ax=ax[1], color=raw_colors[time_idx], linewidth=0.25)
                    elif 'Raw_Lidar_Data_bckg_zb' in data:
                        signal_to_rcs(data.sel(time=time_idx, channels=channel_)[ "Raw_Lidar_Data_bckg_zb" ], data['points']*1e3).plot( x="points", ax=ax[0], color=colors[time_idx], linewidth=0.25)
                        data.sel(time=time_idx, channels=channel_)[ "Raw_Lidar_Data_bckg_zb" ].plot( x="points", ax=ax[1], color=colors[time_idx], linewidth=0.25)
                        data.sel(time=time_idx, channels=channel_)[ "Raw_Lidar_Data" ].plot( x="points", ax=ax[1], color=raw_colors[time_idx], linewidth=0.25)
                    else:                        
                        data.sel(time=time_idx, channels=channel_)[ "Raw_Lidar_Data" ].plot( x="points", ax=ax[1], color=raw_colors[time_idx], linewidth=0.25)
                except:
            ...
            fig.suptitle(f"{self.station} | {string_ID} | {channel_} | {self.measurementID}")
            ...
            try:
                ...
                print(e)
                logger.warning(f"Error in tight_layout for {self.path.name} | {string_ID}")
                continue
            if savefig:
                # save
                if isinstance(output_dir, Path):
                    ...
                    output_dir = Path.cwd()
                outputpath = output_dir.joinpath(self.path.name.replace(".nc", f"_{string_ID}.png"))
        ...
        for idx, channel_ in enumerate(raw_data.channel):
            ...
            for idx, time_ in enumerate(raw_data.time.values):
                ...
                (1e6 * normalized_rcs.sel(time=time_, channel=channel_)).plot(
                    x="altitude",
                    ax=ax,
                    label=r"$\beta_{RCS}^{att}$" + f"[{channelID_}] | {time_.astype(str).split('.')[0]}",
                    color=colors[idx],
                    linewidth=1.0,
                )
                (
