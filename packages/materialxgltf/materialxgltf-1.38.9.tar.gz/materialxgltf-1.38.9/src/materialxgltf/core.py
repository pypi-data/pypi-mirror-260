...
class MTLX2GLTFWriter:
    ...
    def stringToScalar(self, value, type) -> None:
        ...
        if type in ['integer', 'matrix33', 'matrix44', 'vector2', 'vector3', 'vector4', 'float', 'color3', 'color4']:
    ...
    def graphToJson(self, graph, json, materials, textures, images, samplers):
        ...
        for node in graph.getNodes():
            ...
            for output in nodedef.getOutputs():
                # ignore if ouput in nodeoutputs
                exists = False
                for nodeoutput in nodeoutputs:
                    if nodeoutput.getName() == output.getName():
                        exists = True
                if not exists:
                    outputItem = {}
                    outputItem['name'] = output.getName()
                    outputItem['type'] = output.getType()
