...
def prv_pool_picture(latest_date,style_date,latest_HB1001_spjg):
    ...
    # ws["A32".format(2+3*len(jjdm_list))].options(pd.DataFrame, header=1, index=True, expand='table').value = summary
    ws["A{}".format(3*len(barra_exp))].options(pd.DataFrame, header=1, index=True, expand='table').value = barra_exp
    ws["O{}".format(3*len(barra_exp))].options(pd.DataFrame, header=1, index=True, expand='table').value = barra_ret
...
def quant_pool_picture(end_date):
    ...
    pool_info=pd.read_excel(r"E:\GitFolder\docs\To虚怀\业绩统计(fof&池）\基金池\数据源\基金池202401\量化基金池202401.xlsx"
                            ,sheet_name='量化池列表',header=1)
    index_enhance=pool_info[pool_info['三级策略'].isin(['1000指数增强','500指数增强','全市场指数增强'])][['基金代码','代表产品','三级策略']]
    market_netuarl=pool_info[pool_info['二级策略']=='市场中性'][['基金代码','代表产品']]


    #get funds ret
    start_date=str(int(end_date)-20000)
    sql=("select hb1z,tjrq,jjdm from st_hedge.t_st_sm_zhb where jjdm in {0} and tjrq>='{1}' and tjrq<='{2}' and hb1z!=99999"
    ...
    #get index ret
    sql=("select hb1z,tjrq,zqdm from st_market.t_st_zs_zhb where zqdm in ('000300','000905','000852','932000') and tjrq>='{0}' and tjrq<='{1}'"
         .format(start_date,end_date))
    index_ret=hbdb.db2df(sql,db='alluser').pivot_table('hb1z','tjrq','zqdm')
    index_ret.index=index_ret.index.astype(str)
    index_ret.columns=['300', '500', '1000', '2000']


    list_500=['300', '1000', '2000']
    list_1000=['300', '500', '2000']
    list_all=['300', '500', '1000', '2000']
    ...
    corr_500=index_enhance_nav.sub(index_enhance_nav['500'],
                                   axis=0).drop('500',axis=1).rolling(52,26).corr().loc[(slice(None),list(
    ...
    corr_1000=index_enhance_nav.sub(index_enhance_nav['1000'],
                                   axis=0).drop('1000',axis=1).rolling(52,26).corr().loc[(slice(None),list(
    ...
    all_corr=index_enhance_nav.rolling(52,26).corr().loc[(slice(None),list(
    ...
    corr_500=pd.merge(corr_500,index_enhance[['基金代码','代表产品']],how='left',
                      left_on='level_1',right_on='基金代码').drop(['基金代码','level_1'],axis=1)
    corr_1000=pd.merge(corr_1000,index_enhance[['基金代码','代表产品']],how='left'
                       ,left_on='level_1',right_on='基金代码').drop(['基金代码','level_1'],axis=1)
    all_corr=pd.merge(all_corr,index_enhance[['基金代码','代表产品']],how='left',
                      left_on='level_1',right_on='基金代码').drop(['基金代码','level_1'],axis=1)
    netuarl_corr=pd.merge(netuarl_corr,market_netuarl[['基金代码','代表产品']],how='left',
                          left_on='level_1',right_on='基金代码').drop(['基金代码','level_1'],axis=1)


    pool_500_history=corr_500[corr_500['tjrq']>=last_year].groupby('tjrq').mean()
    pool_1000_history=corr_1000[corr_1000['tjrq']>=last_year].groupby('tjrq').mean()
    pool_all_history=all_corr[all_corr['tjrq']>=last_year].groupby('tjrq').mean()
    pool_netural_history = netuarl_corr[netuarl_corr['tjrq'] >= last_year].groupby('tjrq').mean()


    corr_500=corr_500[corr_500['tjrq']==latest_date].drop('tjrq',axis=1).set_index('代表产品')
    corr_1000=corr_1000[corr_1000['tjrq']==latest_date].drop('tjrq',axis=1).set_index('代表产品')
    all_corr=all_corr[all_corr['tjrq']==latest_date].drop('tjrq',axis=1).set_index('代表产品')
    netuarl_corr=netuarl_corr[netuarl_corr['tjrq']==latest_date].drop('tjrq',axis=1).set_index('代表产品')
    ...
    ws = wb.sheets['个基宽基相关性']
    ws.range("A2:Z100").clear_contents
    ws["A2"].options(pd.DataFrame, header=1, index=True, expand='table').value = corr_500
    ws["F2"].options(pd.DataFrame, header=1, index=True, expand='table').value = corr_1000
    ws["K2"].options(pd.DataFrame, header=1, index=True, expand='table').value = all_corr
    ws["Q2"].options(pd.DataFrame, header=1, index=True, expand='table').value = netuarl_corr
...
if __name__ == '__main__':
    ...
    quant_pool_picture('20240223')

    # sql="select hb1n,zsdm from st_hedge.t_st_sm_zhmzsnhb where zsdm in ('HB0011','HB0018','HB001d','HB0014','HB0015','HB0012') and tjnf='2023'"
    ...
    # mutual_pool_picture('公募核心池','20230630', '20230831', '20240222',fool_version=True)
