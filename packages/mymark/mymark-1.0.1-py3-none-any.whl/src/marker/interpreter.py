...
class Interpreter:
    def __init__(
        ...
        self.mark_scheme = get_file_contents(mark_scheme_path)
    ...
    def get_gpt_response(self, query: str) -> str:
        ...
        response = self.gpt_adapter.get_response(
            adapter.GPTRequest(query, temperature=0, model=adapter.GPTModel.GPT3_5TURBO_UNLIMITED)
        )
        if self.debug_mode:
