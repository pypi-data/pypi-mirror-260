...
def use(smart_search: str, interactive: bool):
    ...
    if use_util.looks_like_id(smart_search):
        ret = provider.list(search=f"id $eq {smart_search}")
    else:
        ret = provider.list(search=f"name $like '{smart_search}'")
        if not ret:
            ret = provider.list(search=f"description $like '{smart_search}")
