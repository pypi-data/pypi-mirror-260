...
def validate_acs(
    ...
    if not acs_config or not acs_config.get("index_name") or not acs_connection_id:
        activity_logger.info(
            "[Validate Deployments]: ACS config or ACS connection id is missiong. Skip ACS validation."
        )
        return
    ...
    for retry_counter in range(MAX_RETRIES + 1):
        try:
            ...
            if retry_counter < MAX_RETRIES:
                retry_counter += 1
                activity_logger.info(f"Failed to validate ACS. {retry_counter}th retry after {SLEEP_DURATION} seconds.")
                time.sleep(SLEEP_DURATION)
            else:
                activity_logger.info(
                    f"ValidationFailed: Failed to reach Azure Cognitive Search account.exception: {traceback.format_exc()}"
                )
                raise ex
    ...
    if acs_config["index_name"] in index_names:
        ...
        raise ValueError(
            f"ACS index with name '{acs_config['index_name']}' does not exists in the ACS account. ACS account has reached the quota limit for the number of indexes."
        )
