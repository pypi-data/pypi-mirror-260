...
class EasyFTP:
    ...
    def chmod(self, remote_path: str, perm: int, mode: Literal["oct", "bin", "int"] = "int") -> None:
        ...
        else: raise FTPError(f"permission mode unsupported : {mode}. mode can be 'int', 'oct', or 'bin'.")
...
class Filter:
    def __init__(self, include : str = None, exclude : str = None) -> None:
        pass

if __name__ == "__main__":
    with EasyFTP() as session:
        session.connect("ftp.dlptest.com", 21, "dlpuser", "rNrKYTX9g7z3RgJRmxWuGHbeu")
        print(session.ls())