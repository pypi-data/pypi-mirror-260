...
async def new_ds(
    ...
    try:
        ...
        if datasource_exists:
            new = [asdict(index) for index in ds.get("params", {}).get("indices_list", [])]
            existing = existing_ds.get("indices", [])
            ...
            if len(existing) != len(new) or any([(d, d2) for d, d2 in zip(new, existing) if d != d2]):
                click.secho(f"NEW INDICES: {new}")
                click.secho(f"EXISTING INDICES: {existing}")
                new_indices = ds.get("params", {}).get("indices") or "0"
        if new_description or new_schema or new_ttl or (new_indices is not None) and (not fork_downstream or not fork):
            click.secho(f"NEW INDICES: {new_indices}")
...
def format_indices(file_parts: List[str], node: Dict[str, Any]) -> List[str]:
    if "indices" in node and node["indices"]:
        ...
        file_parts.append("INDICES >")
        file_parts.append(DATAFILE_NEW_LINE)
        file_parts.append(f"{DATAFILE_NEW_LINE}".join(map(lambda index: f"    {index.to_datafile()}", indices)))
        file_parts.append(DATAFILE_NEW_LINE)
        file_parts.append(DATAFILE_NEW_LINE)
