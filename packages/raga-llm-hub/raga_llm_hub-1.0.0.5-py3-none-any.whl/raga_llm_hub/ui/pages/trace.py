...
def trace_page_creator(context: dict):
    trace_page = Blueprint('trace_page', __name__,
                           template_folder='templates')

    @trace_page.route("/trace/<data_point_id>")
    def page(data_point_id):
        results = context.get('results')
        result = list(filter(lambda x : x.get('data_point_id') == data_point_id, group_by_data_point_id(results)))[0]
        if result is None:
            abort(404)
        ...
        try:
            return render_template('trace.html', result=result, tests=result.get('dynamic'))
