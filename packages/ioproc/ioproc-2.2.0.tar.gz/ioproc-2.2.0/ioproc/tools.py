...
def setupFolderStructure(workflowName="", path=None):
    ...
    if path2userConfig.exists():
        ...
        try:
            ...
            with path2userConfig.open("w") as opf:
                for iline in src:
                    in_action_folder_declaration = ('action_folder' in iline) or in_action_folder_declaration
    ...
    if not folderScript.exists():
        with (IOPROCINSTALLROOT/'templatefiles'/'create_folder_structure.py').open('r') as ipf:
                src = ipf.read()
