...
@ioproc.command(help="recreate the general.py action file in the current work directory. Overwrite without prompting.")
@click.option("--path", "-p", default=None, help="path to folder where to create general.py")
def recreate_default_actions(path):
    
    path = pt.Path.cwd()/'general.py' if path is None else pt.Path(path)

    if path.exists():
        click.confirm('There is already an general.py file existing. Do you want to overwrite and continue?', abort=True)
...
@ioproc.command(help="recreate the noioproc.py file in the current work directory. Overwrite without prompting.")
@click.option("--path", "-p", default=None, help="path to folder where to create noioproc.py")
def create_noioproc(path):
    
    path = pt.Path.cwd()/'noioproc.py' if path is None else pt.Path(path)

    if path.exists():
        click.confirm('There is already an noioproc.py file existing. Do you want to overwrite and continue?', abort=True)
...
@ioproc.command(help="Display the code snipped that needs to be added to ioproc action files to make them useable without ioproc. Requires the noioproc.py file.")
def show_noioproc():
    log.info('add this to your actions file:')
    print("# this is a safe guard to make the below actions useable without an ioproc installation.")
    print("try:")
    ...
    print("    print('ioproc is not installed and the integration into ioproc workflows is disabled.')")


@ioproc.command(help="create an empty user.yaml file")
@click.option("--path", "-p", default=None, help="path to folder where to create the user.yaml")
def create_workflow_file(path):
    ...
    if path is not None:
        ...
        if path.exists():
            click.confirm('There is already an user.yaml file existing. Do you want to overwrite and continue?', abort=True)
...
@ioproc.command(help="list all actions known to this ioproc workflow or display the details for a specific action in a given project. For details specify the requested action as project.action e.g. general.readExcel")
@click.option("--useryaml", "-u", default=None, help="path to user.yaml, defaults to './user.yaml'")
...
def actions(useryaml, details):
    ...
    if len(details) > 0:
        for i in details:
            d = i.split('.')
            if len(d) > 2:
                log.error(f'the requested action "{i}" is not in the correct format. See help for details.')
                return
            project, action_name = d
            if project not in actionMgr:
                log.error(f'the requested project "{project}" is not available to ioproc. Known projects are: {", ".join(actionMgr.keys())}.')
                return
            if action_name not in actionMgr[project]:
                log.error(f'the requested action "{action_name}" is not available to ioproc. Known actions in project "{project}" are: {", ".join(actionMgr[project].keys())}.')
                return
    ...
    try:
        ...
        log.info('paths ioproc looks for actions:')
        for ipath in config["user"]["action_folder"]:
            log.info(f'  - {ipath} :: {"valid" if pt.Path(ipath).exists() else "invalid"}')
        log.info('-------------------------------')
        if len(actionMgr) == 0:
...
def _execute(
    ...
    try:
        data = _execute_workflow(userConfigPath, metaformat, overridedata, return_data, log, dlog)
    finally:
        os.chdir(old_pwd.as_posix())
...
def _execute_workflow(userConfigPath, metaformat, overridedata, return_data, log, dlog):
    ...
    if 'metadata_files' in config['user']:
        for itag, imeta in config['user']['metadata_files'].items():
            ...
            elif IOPROVENANCE_INSTALLED and imeta['flavor'] in ioprocmeta.available_standard_formats:
                m = ioprocmeta.available_standard_formats[imeta['flavor']].read_json(pt.Path(imeta['path']))
            else:
                print(ioprocmeta.available_standard_formats)
                raise IOError(f'metadata file "{imeta["path"]}" is of unknown format "{imeta["flavor"]}"')
