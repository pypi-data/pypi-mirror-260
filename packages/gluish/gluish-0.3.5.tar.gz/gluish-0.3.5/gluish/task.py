...
class BaseTask(luigi.Task):
    ...
    def path(self, filename=None, ext='tsv', digest=False, shard=False, encoding='utf-8', use_legacy_bool=False, use_legacy_sep=False):
        """
        ...
        if filename is None:
            ...
            for name, param in self.get_params():
                ...
                # some cosmetic changes: e.g. we had:
                #     "date-2024-02-19-format-solr5vu3-style-default-with_fullrecord-True.zst"
                # but it would a bit more straight forward to have a filename like:
                #     "date-2024-02-19-format-solr5vu3-style-default-with-fullrecord-true.zst"
                # few of the following lines are related to this

                if isinstance(val, datetime.datetime):
                    val = val.strftime('%Y-%m-%dT%H%M%S')
                elif isinstance(val, datetime.date):
                    val = val.strftime('%Y-%m-%d')
                elif isinstance(val, bool) and not use_legacy_bool:
                    val = "true" if val else "false" # use lowercase
                elif isinstance(val, str) and not use_legacy_sep:
                    val = val.replace("_", "-") #

                if isinstance(name, str) and not use_legacy_sep:
                    name = name.replace("_", "-")
