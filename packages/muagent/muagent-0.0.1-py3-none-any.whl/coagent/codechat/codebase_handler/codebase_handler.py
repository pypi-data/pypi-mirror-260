...
class CodeBaseHandler:
    ...
    def search_vertices(self, vertice_type="class") -> List[str]:
        '''
        ...
        if self.nh:
            ...
            vertices = [v.as_node().get_id() for v in vertices["v"] if vertice_type in v.as_node().tags()]
            # for v in vertices["v"]:
            #     logger.debug(f"{v.as_node().get_id()}, {v.as_node().tags()}")
        else:
            if vertice_type == "class":
                vertices = [class_name for code, structure in self.graph.items() for class_name in structure['class_name_list']]
            elif vertice_type == "method":
                vertices = [
                    methods_name
