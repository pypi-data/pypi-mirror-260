...
test_fractional_aufbau = [
    ...
    (
        "cc-pvdz",
        "test/no.xyz",
        {"nocc_a": 6.5, "nocc_b": 6.5, "charge": 2},
        {
...
@pytest.mark.parametrize("basis,mol,kwargs,expected", test_fractional_aufbau)
@pytest.mark.parametrize("factory", [Basis, DenseLinalgFactory])
def test_occ_fractional_aufbau(basis, mol, kwargs, expected, factory):
    """Test fractional occupation model for gobasis, nocc_a/nocc_b, and ncore
    arguments as input."""
...
test_wrong_arguments = [
    ...
    (
        ...
        "test/water.xyz",
        10,  # nel, needed for lf tests
        {"nocc_a": 5.5, "nocc_b": 4.5, "unrestricted": False},
        ConsistencyError,
...
@pytest.mark.parametrize("basis,mol,nel,kwargs,error", test_wrong_arguments)
def test_occ_fractional_arguments_gobasis(basis, mol, nel, kwargs, error):
    """Test fractional occupation model for gobasis, nocc_a/nocc_b, and ncore
    arguments as input.
    ...
    with pytest.raises(error):
        assert FractionalOccModel(gobasis, **kwargs)


@pytest.mark.parametrize("basis,mol,nel,kwargs,error", test_wrong_arguments)
def test_occ_fractional_arguments_lf(basis, mol, nel, kwargs, error):
    """Test fractional occupation model for gobasis, nocc_a/nocc_b, and ncore
    arguments as input."""
    fn_xyz = context.get_fn(mol)
    gobasis = get_gobasis(basis, fn_xyz, print_basis=False)
...
@pytest.mark.parametrize("factory", [Basis, DenseLinalgFactory])
def test_occ_fractional_kwargs(basis, mol, kwargs, factory):
    """Test fractional occupation model for gobasis, nocc_a/nocc_b, and ncore arguments as
    input.
    """
    fn_xyz = context.get_fn(mol)
    gobasis = get_gobasis(basis, fn_xyz, print_basis=False)
