...
def check_get_fock_matrix(basis_str, xyz, nocc):
    ...
    # gobasis instance
    obasis = get_gobasis(basis_str, xyz, print_basis=False)
    lf = DenseLinalgFactory(obasis.nbasis)

    # Compute Gaussian integrals
    one = compute_kinetic(obasis)
    one.iadd(compute_nuclear(obasis))
    ...
    fockref._array = np.einsum("abc->ac", tmp._array[:, :nocc, :])
    fockref.iadd(one)
    ...
    assert np.allclose(fock._array, fockref._array)
...
def check_get_diag_fock_matrix(basis_str, xyz, nocc):
    ...
    # Done use contract function here:
    fockref._array = np.einsum("ab->a", tmp._array[:, :nocc])
    fockref._array[:] += one._array.diagonal()
