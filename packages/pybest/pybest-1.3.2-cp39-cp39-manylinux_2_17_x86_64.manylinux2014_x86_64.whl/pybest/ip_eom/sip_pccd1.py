...
class RIPpCCD1(RSIPCC1):
    ...
    @timer.with_section("Hdiag IPpCCD1 sf")
    def h_diag_sf(self):
        ...
        h_diag.assign(x1imdiag, end0=no)
        #
        # rijb
        #
        if self.nhole >= 2:
    ...
    def get_2_hole_terms_h_diag_sf(self, h_diag):
        ...
        **Arguments:**
        ...
        # x6(j,b,j,b)
        #
        x6jbld.iadd_expand_four_to_three("bcbc->abc", h_ijb, 1.0)
        #
        # x8(i,b,i,b)
        #
        x8ibld.iadd_expand_four_to_three("acac->abc", h_ijb, 1.0)
        #
        # x9(i,i)
        #
        h_ijb.iadd_expand_two_to_three("aa->abc", x9il, 1.0)
        #
        # x10(b,b)
        #
        h_ijb.iadd_expand_two_to_three("cc->abc", x10bc, 1.0)
        #
        # x11(j,j)
        #
        h_ijb.iadd_expand_two_to_three("bb->abc", x9il, 1.0, end0=no, end1=no)
        #
        # x12(i,j,i,j)
        #
        x12ijlm.iadd_expand_four_to_three("abab->abc", h_ijb, 1.0)
        #
        # x13(i,j,b,j) dij
        #
        tmp = loovv.contract("aabb,ab->ab", cia, out=None)
        h_ijb.iadd_expand_two_to_three("ab->aab", tmp, -1.0)
        #
        # assign to h_diag output
        #
        h_diag.assign(h_ijb, begin0=no)

    @timer.with_section("Hdiag IPpCCD1")
    def h_diag(self):
    ...
    def get_2_hole_terms_h_diag(self, h_diag):
        ...
        **Arguments:**
        ...
        #
        lij = x1im.new()
        goooo.contract("abab->ab", out=lij, clear=True, select="einsum")
        goooo.contract("abab->ab", out=lij, factor=-1.0, select="einsum")
        gov = goovv.contract("aabb->ab", out=None, clear=True, select="einsum")
        lov = govov.contract("abab->ab", out=None, clear=True, select="einsum")
        govvo.contract("abba->ab", out=lov, factor=-1.0, select="einsum")
        x6ij = x6ijlm.contract(
            "abab->ab", out=None, clear=True, select="einsum"
        )
        gjjbb = goovv.contract(
            "aabb->ab", out=None, factor=1.0, select="einsum"
        )
        gjbjb = govov.contract(
            "abab->ab", out=None, factor=1.0, select="einsum"
        )
        #
        # H_ijb,ijb
        #
        ...
        # lij
        #
        h_ijb.iadd_expand_two_to_three("ab->abc", lij, 0.25)
        #
        # lov(j,b)
        #
        h_ijb.iadd_expand_two_to_three("bc->abc", lov, -1.0)
        #
        # gov(j,b) cjb
        #
        cia_ = cia.copy()
        ...
        # H_iJB,iJB
        # -fim(i,i)
        h_iJB.iadd_expand_two_to_three("aa->abc", fock, -1.0, end0=no, end1=no)
        #
        # x4bd(b,b)
        #
        ...
        # x6ijlm(i,j,i,j)
        #
        h_iJB.iadd_expand_two_to_three("ab->abc", x6ij, 1.0)
        #
        # gjjbb
        #
        h_iJB.iadd_expand_two_to_three("bc->abc", gjjbb, 1.0)
        #
        # gjjbb cjb / giibb cia
        #
        cia_ = cia.copy()
        cia_.imul(gjjbb)
        h_iJB.iadd_expand_two_to_three("bc->abc", cia_, 1.0)
        h_iJB.iadd_expand_two_to_three("ac->abc", cia_, 1.0)
        # -gjjbb cjb dij
        h_iJB.iadd_expand_two_to_three("ab->aab", cia_, -1.0)
        #
        # -gibib
        #
        h_iJB.iadd_expand_two_to_three("ac->abc", gjbjb, -1.0)
        #
        # assign using mask
        #
    ...
    @timer.with_section("SubH IPpCCD1 sf")
    def subspace_sf(self, b_vector):
        """
    ...
    def get_2_hole_terms_sf(self, b_1, b_vector, s_1, sigma):
        """Determine all contributions containing two hole operators for
        the spin-free representation:
        ...
        **Arguments:**
        ...
        # (13) - xklcbj rlkc dij
        # (b) = lklcb rlkc
        tmp = loovv.contract("abcd,bac->d", b_3, out=None, select=self.tco)
        cia_ = cia.new()
        cia.contract("ab,b->ab", tmp, cia_)
        s_3.iadd_expand_two_to_three("ab->aab", cia_, -1.0)
        #
        # Assign to sigma vector
        #
        sigma.assign(s_3, begin0=no)

    @timer.with_section("SubH IPpCCD1")
    def subspace(self, b_vector):
        """
    ...
    def get_2_hole_terms(self, b_1, b_vector, s_1, sigma):
        ...
        **Arguments:**
        ...
        s_3 = self.lf.create_three_index(no, no, nv)
        # input
        b_3aa = self.lf.create_three_index(no, no, nv)
        b_3bb = self.lf.create_three_index(no, no, nv)
        #
        # Finale index of b_3aa in b_vector
        #
        end = no + (no - 1) * no * nv // 2
        #
        # assign rijb
        ...
        sigma.assign(s_3.array[self.get_mask(0)], begin0=no, end0=end)
        #
        # R_iJB
        #
    ...
    def get_2_hole_r_i_terms(self, b_3aa, b_3bb, s_1):
    ...
    def get_2_hole_r_3ss_terms(self, b_1, b_3aa, b_3bb, s_3):
        """Determine all contributions containing two particle operators:
            * R_ijb (same spin - ss)
        ...
        **Arguments:**
        ...
        # P(ij) <bD|jL> rild cjb
        tmp = gvvoo.contract("abcd,edb->eca", b_3aa, **tco)
        tmp.contract("abc,bc->abc", cia, s_3)
        #
        # (11) (DjLb) riLD
        # P(ij) <bd||jl> riLD
        tmp = gvvoo.contract("abcd,edb->eca", b_3bb, **tco)
        # (ijb)
        s_3.iadd(tmp)
        # ex term
        gvvoo.contract("abcd,ecb->eda", b_3bb, tmp, factor=-1.0, **tco)
        # (ijb) cjb
        tmp.contract("abc,bc->abc", cia, s_3)
        #
        # P(ij)
    ...
    def get_2_hole_r_3os_terms(self, b_1, b_3aa, b_3bb, s_3):
        """Determine all contributions containing two particle operators:
        ...
        **Arguments:**
        ...
        # <jL|bD> rild
        # (ijb)
        tmp = goovv.contract("abcd,ebd->eac", b_3aa, **tco)
        s_3.iadd(tmp)
        # <jl|db> rild (ex. part of <jl||bd> rild)
        # (ijb)
        goovv.contract("abcd,ebc->ead", b_3aa, tmp, factor=-1.0, **tco)
        # (ijb) cjb
        tmp.contract("abc,bc->abc", cia, s_3)
        #
        # (18) (jDbL) riLD
        ...
        # <jl|bd> riLD
        # (ijb)
        tmp = goovv.contract("abcd,ebd->eac", b_3bb, **tco)
        # (ijb) cjb
        tmp.contract("abc,bc->abc", cia, s_3)
        #
        # (19) (idkb) rkJD
        # -<ib|kd> rkJD
        tmp = govov.contract("abcd,ced->aeb", b_3bb, out=None, **tco)
        s_3.iadd(tmp, -1.0)
        # <ib|dk> cib rkJD
        # <ib|dk> rkJD
        tmp = govvo.contract("abcd,dec->aeb", b_3bb, out=None, **tco)
        # (ijb) cib
        tmp.contract("abc,ac->abc", cia, s_3)
        #
        # (20) rkld dij
        # (b) = 0.5 <bd||kl> rkld
        tmp = gvvoo.contract("abcd,cdb->a", b_3aa, factor=0.5, **tco)
        gvvoo.contract("abcd,dcb->a", b_3aa, tmp, factor=-0.5, **tco)
        #
        # (21) rkLD dij
        # (b) = <bd|kl> rkLD
        gvvoo.contract("abcd,cdb->a", b_3bb, tmp, **tco)
        # - b cib
        cia_ = cia.new()
        cia.contract("ab,b->ab", tmp, cia_)
        s_3.iadd_expand_two_to_three("ab->aab", cia_, -1.0)

    @timer.with_section("SetH IPpCCD1")
    ...
    def set_hamiltonian_2_hole(self, fock, mo2):
        ...
        **Arguments:**
        ...
        # -<ij||mb>
        gooov.contract("abcd->abdc", out=x2ijbm, factor=-1.0, select="einsum")
        gooov.contract("abcd->badc", out=x2ijbm, factor=1.0, select="einsum")
        # -<ij|mb> cjb
        ...
        # x5ijbm
        #
        x5ijbm = self.init_cache("x5ijbm", no, no, nv, no)
        # -<ij|mb>
        gooov.contract(
            "abcd->abdc", out=x5ijbm, factor=-1.0, clear=True, select="einsum"
        )
        # -<ij|mb> cjb
        ...
        # -fbm cib dij
        tibm = self.lf.create_three_index(no, nv, no)
        cia.contract("ab,bc->abc", fock, tibm, factor=-1.0, **vo2)
        # -<bm|ee> cie dij
        mo2.contract("abcc,dc->dab", cia, tibm, factor=-1.0, **vovv)
        # dij tibm
        x5ijbm.iadd_expand_three_to_four("abc->aabc", tibm, factor=1.0)
        #
        # x6ijlm
        #
        x6ijlm = self.init_cache("x6ijlm", no, no, no, no)
        # gijlm
        mo2.contract(
            "abcd->abcd",
            out=x6ijlm,
        ...
        # glmcc cic dij
        tmpilm = self.lf.create_three_index(no, no, no)
        mo2.contract("abcc,dc->dab", cia, tmpilm, **oovv)
        # dij tilm
        x6ijlm.iadd_expand_three_to_four("abc->aabc", tmpilm, factor=1.0)
    ...
    def set_sf_terms(self, fock, mo2):
        ...
        **Arguments:**
        ...
        # looov
        #
        looov = self.init_cache("looov", no, no, no, nv)
        mo2.contract(
            "abcd->abcd",
            out=looov,
            factor=2.0,
        ...
        govvo = self.denself.create_four_index(no, nv, nv, no)
        mo2.contract(
            "abcd->abcd",
            out=govvo,
            factor=1.0,
        ...
        # x6jbld
        #
        x6jbld = self.init_cache("x6jbld", no, nv, no, nv)
        # Ljdbl
        govvo.contract(
            "abcd->acdb", out=x6jbld, factor=2.0, clear=True, select="einsum"
        )
        govov.contract("abcd->abcd", out=x6jbld, factor=-1.0, select="einsum")
        # Ljlbd cjb / jdbl jbdl
        ...
        # x7jbld
        #
        x7jbld = self.init_cache("x7jbld", no, nv, no, nv)
        # -gjdbl
        govvo.contract(
            "abcd->acdb", out=x7jbld, factor=-1.0, clear=True, select="einsum"
        )
        # -<jl||bd> cjb / jdbl jbdl
        ...
        # x8ibld
        #
        x8ibld = self.init_cache("x8ibld", no, nv, no, nv)
        # -gibld
        govov.contract(
            "abcd->abcd", out=x8ibld, factor=-1.0, clear=True, select="einsum"
        )
        # gibdl cib
        ...
        del govov
        del govvo
        loovv = self.init_cache("loovv", no, no, nv, nv)
        mo2.contract(
            "abcd->abcd",
            out=loovv,
            factor=2.0,
        ...
        # x9il = x11jl
        #
        x9il = self.init_cache("x9il", no, no)
        # -fil
        x9il.iadd(fock, factor=-1.0, end2=no, end3=no)
        # -<il|cc> cic
        mo2.contract("abcc,ac->ab", cia, x9il, factor=-1.0, **oovv, select=opt)
        #
        # x10bc
        #
        x10bc = self.init_cache("x10bc", nv, nv)
        # fbc
        x10bc.iadd(fock, 1.0, begin2=no, begin3=no)
        # -gbckk ckb
        mo2.contract(
        ...
        # x12ijlm
        #
        x12ijlm = self.init_cache("x12ijlm", no, no, no, no)
        # gijlm
        mo2.contract(
            "abcd->abcd",
            out=x12ijlm,
        ...
        mo2.contract("abcc,dc->dab", cia, tmpilm, **oovv)
        # dij tilm
        x12ijlm.iadd_expand_three_to_four("abc->aabc", tmpilm, factor=1.0)

    def set_sd_terms(self, mo2):
