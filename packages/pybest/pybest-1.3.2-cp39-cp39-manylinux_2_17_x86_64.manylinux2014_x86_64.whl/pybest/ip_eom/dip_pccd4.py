...
class RDIPpCCD4(RDIPCC4):
    ...
    @timer.with_section("Hdiag DIPpCCD4")
    def compute_h_diag(self, *args):
        ...
        # x1im(k,k)
        #
        h_ijkc.iadd_expand_one_to_four("2", x1ii, 1.0)
        #
        # 1/3 x1cd(c,c)
        #
        h_ijkc.iadd_expand_one_to_four("3", x1cc, 1 / 3)
        #
        # 0.5 lij
        #
        h_ijkc.iadd_expand_two_to_four("ab->abcd", lij, 0.5)
        #
        # x2kckc
        #
        h_ijkc.iadd_expand_two_to_four("cd->abcd", x2kckc, 1.0)
        #
        # Permutations
        #
        # create copy first
    ...
    @timer.with_section("SubH DIPpCCD4")
    def build_subspace_hamiltonian(self, b_vector, h_diag, *args):
        ...
        **Arguments:**
        ...
        #
        # output
        sigma = self.denself.create_four_index(no, no, no, nv)
        to_s = {"out": sigma, "select": self.tco, "clear": False}
        # input
        b_v = self.denself.create_four_index(no, no, no, nv)
        #
        # reshape b_vector
        #
        b_v.clear()
    ...
    @timer.with_section("SetH DIPpCCD4")
    def set_hamiltonian(self, mo1, mo2):
        ...
        **Arguments:**
        ...
        # x2kcmd
        #
        x2kcmd = self.init_cache("x2kcmd", no, nv, no, nv)
        # <kc|dm> ckc
        mo2.contract("abcd,ab->abdc", cia, x2kcmd, **ovvo)
        # -<kd|mc>
        mo2.contract(
            "abcd->abcd", x2kcmd, factor=-1.0, **ovov, select="einsum"
        )
        if self.dump_cache:
            self.cache.dump("x2kcmd")
        #
