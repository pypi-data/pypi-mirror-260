...
from pybest.linalg import DenseLinalgFactory, FourIndex, TwoIndex
...
class REOMCC(ABC):
    ...
    def __init__(self, lf, occ_model):
        ...
        **Arguments:**
        ...
        if self._nocc >= self._nbasis:
            raise ValueError(
                f"Impossible number of occupied ({self._nocc}) and virtual "
                f"({self._nvirt}) orbitals!"
            )
    ...
    @property
    def tco(self):
        """The tensor contraction engine used factory"""
        return self._tco
    ...
    @property
    def nvirt(self):
        """The total number of virtual orbitals"""
        return self._nvirt
    ...
    def print_info(self, **kwargs):
        ...
        if log.do_medium:
            ...
            log(f"Number of frozen occupied orbitals: {self.ncore}")
            log(f"Number of active occupied orbitals: {self.nacto}")
            log(
                f"Total number of electrons:          {self.nacto*2+self.ncore*2}"
            )
            log(f"Number of active electrons:         {self.nacto*2}")
            log(f"Number of active virtual orbitals:  {self.nactv}")
    ...
    def get_index_d(self, index):
        ...
        """
        triu = np.triu_indices(self.nacto * self.nactv)
        row = triu[0][index]
        col = triu[1][index]
    ...
    def set_seniority_0(self, other, value=0.0):
    ...
    def print_results(self, threshold):
        ...
        if log.do_medium:
            ...
            for ind, e_val in enumerate(e_vals):
                ...
                #
                log(
                    f"Excitation energy:      {e_val.real: e} [au]  /  "
    ...
    @timer.with_section("EOM-CC-Diagonalize")
    ...
    @timer.with_section("REOMCC")
    def __call__(self, *args, **kwargs):
        """Solve the non-symmetric eigenvalue problem in EOM-pCCD
