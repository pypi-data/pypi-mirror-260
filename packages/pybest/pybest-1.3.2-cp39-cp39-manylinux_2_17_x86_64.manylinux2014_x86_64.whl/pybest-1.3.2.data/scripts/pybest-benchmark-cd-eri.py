...
def compute_cholesky_decomposed_eri(
    obasis, precision, nthreads, disable_thread_control=False
):
    ...
    gobasis_module.compute_cholesky_eri(obasis, precision)


def run_nthreads(
    obasis, nthreads=1, precision=1e-12, ntries=1, disable_thread_control=False
):
    ...
    for _ in range(ntries):
        ...
        #    args=(obasis, precision, nthreads),
        #    kwargs={"disable_thread_control": disable_thread_control},
        # )
        # p.start()
        # p.join()
        compute_cholesky_decomposed_eri(obasis, precision, nthreads)
        end = time.time()
...
def main():
    ...
    obasis = get_gobasis(args.basis, mol)
