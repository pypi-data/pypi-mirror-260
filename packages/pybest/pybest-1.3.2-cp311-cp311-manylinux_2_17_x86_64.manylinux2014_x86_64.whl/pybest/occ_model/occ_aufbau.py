...
class AufbauOccModel(OccupationModel):
    ...
    # NOTE: Replace this init after grace period is expired with
    # def __init__(self, gobasis, **kwargs):
    def __init__(self, *noccs, **kwargs):
        ...
        **Keyword arguments:**
        ...
        ncore:
            (int) number of core orbitals. The number of core orbitals is equal
            for alpha and beta orbitals.
        """
        # Workaround to distinguish between noccs numbers (old version) and
        # gobasis/lf instance (new version)
        ...
        for arg in noccs:
            if isinstance(arg, (Basis, LinalgFactory)):
                gobasis = arg
        # end
        # if not found gobasis, do old stuff:
        # TBD: start
        if gobasis is None:
            OccupationModel.__init__(self, gobasis, **kwargs)
            for nocc in noccs:
                if nocc < 0:
    ...
    def print_info(self):
        ...
        def resolve(list_):
            return " ".join(f"{str(s):<5s}" for s in list_)
