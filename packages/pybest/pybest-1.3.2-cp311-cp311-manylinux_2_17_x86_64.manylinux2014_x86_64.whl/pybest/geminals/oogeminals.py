...
from pybest.orbital_utils import compute_unitary_matrix
...
class OOGeminals(GeminalsBase, ABC):
    ...
    #
    @timer.with_section("OOpCCD grad")
    def compute_orbital_gradient(self, ps2c=False):
        ...
        **Optional arguments:**
        ...
        #
        gpqrq.contract(
            "abc,ab->ac", twodmpqpq, gradient, factor=8.0, select="einsum"
        )
        #
    ...
    def compute_orbital_hessian(self, lshift=1e-8, positive=False, ps2c=False):
        ...
        **Optional arguments:**
        ...
        vpq.contract("ab,ab->a", twodmpqpq, two_c, select="einsum")
        vpp.contract("ab,ab->a", two_dm_av, two_ca, select="einsum")
    ...
    def get_exact_hessian(self, mo1, mo2):
        ...
        **Arguments**
        ...
        # ab
        mo2.contract(
            "abcd,cd->acbd", dm2pqpq, hessian, factor=4.0, select="einsum"
        )
        mo2.contract(
            "abcd,ab->acdb", dm2pqpq, hessian, factor=-4.0, select="einsum"
        )
        mo2.contract(
            "abcd,cd->acdb", dm2pqpq, hessian, factor=-4.0, select="einsum"
        )
        mo2.contract(
            "abcd,ab->acbd", dm2pqpq, hessian, factor=4.0, select="einsum"
        )
        # (2)
        # ab
        mo2.contract(
            "abcd,cb->acdb", dm2pqpq, hessian, factor=4.0, select="einsum"
        )
        mo2.contract(
            "abcd,ad->acbd", dm2pqpq, hessian, factor=-4.0, select="einsum"
        )
        mo2.contract(
            "abcd,cb->acbd", dm2pqpq, hessian, factor=-4.0, select="einsum"
        )
        mo2.contract(
            "abcd,ad->acdb", dm2pqpq, hessian, factor=4.0, select="einsum"
        )
        # (3)
        mo2.contract(
            "abcd,bd->abcd", dm2av, hessian, factor=4.0, select="einsum"
        )
        mo2.contract(
            "abcd,ad->abcd", dm2av, hessian, factor=-4.0, select="einsum"
        )
        mo2.contract(
            "abcd,bd->abdc", dm2av, hessian, factor=-4.0, select="einsum"
        )
        mo2.contract(
            "abcd,ac->abcd", dm2av, hessian, factor=4.0, select="einsum"
        )
        mo2.contract(
            "abcd,bc->abdc", dm2av, hessian, factor=4.0, select="einsum"
        )
        mo2.contract(
            "abcd,ac->abdc", dm2av, hessian, factor=-4.0, select="einsum"
        )
        mo2.contract(
            "abcd,bc->abcd", dm2av, hessian, factor=-4.0, select="einsum"
        )
        mo2.contract(
            "abcd,ad->abdc", dm2av, hessian, factor=4.0, select="einsum"
        )
        # Apq,qw (pw) (qv) (-qw) (-pv)
        mo1.contract("ab,b->ab", dm1, ind2, factor=4.0, select="einsum")
        # ab
        mo2.contract("abcb,cb->ac", dm2pqpq, ind2, factor=2.0, select="einsum")
        mo2.contract("abcb,ab->ac", dm2pqpq, ind2, factor=2.0, select="einsum")
        mo2.contract("abcc,bc->ba", dm2av, ind2, factor=2.0, select="einsum")
        mo2.contract("abcc,bc->ab", dm2av, ind2, factor=2.0, select="einsum")
        # Apq,pw (pq,pw) (-pq,vp)
        mo1.contract("ab,c->cab", dm1, ind30, factor=4, select="einsum")
        # ab
        mo2.contract(
            "abcb,db->dac", dm2pqpq, ind30, factor=4.0, select="einsum"
        )
        mo2.contract("abcc,dc->dab", dm2av, ind30, factor=4.0, select="einsum")
        # Apq,vq (pq,vq) (-pq,qw)
        mo1.contract("ab,c->acb", dm1, ind31, factor=4, select="einsum")
        # ab
        mo2.contract(
            "abcb,db->adc", dm2pqpq, ind31, factor=4.0, select="einsum"
        )
        mo2.contract("abcc,dc->adb", dm2av, ind31, factor=4.0, select="einsum")
        ...
        # aa
        mo2.contract(
            "abcd,cd->acbd", dm2pqpqex, hessian, factor=4.0, select="einsum"
        )
        mo2.contract(
            "abcd,ab->acdb", dm2pqpqex, hessian, factor=-4.0, select="einsum"
        )
        mo2.contract(
            "abcd,cd->acdb", dm2pqpqex, hessian, factor=-4.0, select="einsum"
        )
        mo2.contract(
            "abcd,ab->acbd", dm2pqpqex, hessian, factor=4.0, select="einsum"
        )
        # (2)
        # aa
        mo2.contract(
            "abcd,cb->acdb", dm2pqpqex, hessian, factor=4.0, select="einsum"
        )
        mo2.contract(
            "abcd,ad->acbd", dm2pqpqex, hessian, factor=-4.0, select="einsum"
        )
        mo2.contract(
            "abcd,cb->acbd", dm2pqpqex, hessian, factor=-4.0, select="einsum"
        )
        mo2.contract(
            "abcd,ad->acdb", dm2pqpqex, hessian, factor=4.0, select="einsum"
        )
        # Apq,qw (pw) (qv) (-qw) (-pv)
        # aa
        mo2.contract(
            "abcb,cb->ac", dm2pqpqex, ind2, factor=2.0, select="einsum"
        )
        mo2.contract(
            "abcb,ab->ac", dm2pqpqex, ind2, factor=2.0, select="einsum"
        )
        # Apq,pw (pq,pw) (-pq,vp)
        # aa
        mo2.contract(
            "abcb,db->dac", dm2pqpqex, ind30, factor=4.0, select="einsum"
        )
        # Apq,vp (pq,vq) (-pq,qw)
        # aa
        mo2.contract(
            "abcb,db->adc", dm2pqpqex, ind31, factor=4.0, select="einsum"
        )
        ...
        #
        # Apq,vq
        hessian.iadd_expand_two_to_four("ac->abcb", ind2, -1.0)
        # Apq,qw
        hessian.iadd_expand_two_to_four("0-3", ind2, 1.0)
        # Apq,vp
        hessian.iadd_expand_two_to_four("1-2", ind2, 1.0)
        # Apq,pw
        hessian.iadd_expand_two_to_four("bc->abac", ind2, -1.0)
        # Apq,vw (pq,pw) (-pq,vp)
        hessian.iadd_expand_three_to_four("abc->abac", ind30, 1.0)
        hessian.iadd_expand_three_to_four("1-2-1-2", ind30, -1.0)
        # Apq,vw (pq,vq) (-pq,qw)
        hessian.iadd_expand_three_to_four("abc->abcb", ind31, 1.0)
    ...
    def sort_natural_orbitals(self, orb):
        ...
        **Arguments:**
        ...
        order = onedm.sort_indices(begin0=nc)
        orderref = np.arange(na)
        if not (order == orderref).all():
            orb.permute_orbitals(order, begin0=nc)
    ...
    def print_options_scf(
        ...
        **Arguments:**
        ...
        if log.do_medium:
            ...
            log(f"Sorting natural orbitals:      {sort}")
            if stepsearch["method"] == "trust-region":
