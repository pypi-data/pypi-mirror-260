...
@pytest.mark.parametrize("mol, basis, expected", test_cases)
def test_go_basis_desc(mol, basis, expected):
    fn = context.get_fn(f"test/{mol}.xyz")
    mol = IOData.from_file(fn)
    obasis = get_gobasis(f"{basis}", fn, print_basis=False)
    assert (obasis.shell2atom == expected["shell2atom"]).all()
    assert (obasis.nprim == expected["nprim"]).all()
    assert (obasis.shell_types == expected["shell_types"]).all()
    np.testing.assert_almost_equal(obasis.alpha, expected["alpha"])
    np.testing.assert_almost_equal(obasis.contraction, expected["contraction"])
    assert obasis.nbasis == expected["nbasis"]
