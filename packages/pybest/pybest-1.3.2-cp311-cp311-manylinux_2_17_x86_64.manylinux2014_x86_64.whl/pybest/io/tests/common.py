...
def compute_mulliken_charges(obasis, lf, numbers, dm):
    """Computes Mulliken charges for some molecule with basis obasis."""
...
def compute_hf_energy(mol):
    ...
    kin = compute_kinetic(mol.gobasis)
    na = compute_nuclear(mol.gobasis)
    er = compute_eri(mol.gobasis)
    external = {"nn": compute_nuclear_repulsion(mol.gobasis)}
...
class Molecule:
    ...
    def __init__(self, molfile, basis, lf_cls, **kwargs):
        ...
        self.gobasis = get_gobasis(basis, fn, print_basis=False)
        #
        # Define Occupation model, expansion coefficients and overlap
        #
        self.lf = lf_cls(self.gobasis.nbasis)
        self.occ_model = AufbauOccModel(self.gobasis, **kwargs)
        orb = [
            self.lf.create_orbital() for i in range(len(self.occ_model.nbasis))
        ]
        self.orb_a = orb[0]
        self.orb_b = orb[-1]
        self.olp = compute_overlap(self.gobasis)
        #
        # Construct Hamiltonian
        #
        kin = compute_kinetic(self.gobasis)
        na = compute_nuclear(self.gobasis)
        if isinstance(self.lf, CholeskyLinalgFactory):
            er = compute_cholesky_eri(self.gobasis, threshold=1e-8)
        elif isinstance(self.lf, DenseLinalgFactory):
            er = compute_eri(self.gobasis)
        external = compute_nuclear_repulsion(self.gobasis)
...
def get_orbital_data(mol_file, orb_file, basis_file):
    ...
    gobasis = get_gobasis(fn_bas, fn_mol, print_basis=False)
    ...
    return IOData(**{"orb_a": orb_a, "gobasis": gobasis})
