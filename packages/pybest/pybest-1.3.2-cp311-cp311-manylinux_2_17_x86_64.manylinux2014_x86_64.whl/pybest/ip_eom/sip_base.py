...
class RSIPCC(RXIPCC):
    ...
    def get_mask(self, select):
        ...
        **Arguments:**
        ...
        ind0, ind1 = np.indices((no, nv))
        mask = np.ones((no, no, nv), dtype=bool)
        mask[ind0, ind0, ind1] = False
        mask[np.triu_indices(no, k=select)] = False
        return mask
    ...
    def get_index_ijb(self, index):
        ...
        ** Arguments: **
        ...
        no, nv = self.occ_model.nacto[0], self.occ_model.nactv[0]
        if self.spin_free:
            i, j, b = np.unravel_index(index, (no, no, nv))
            return i, j, b, False
        end_aa = (no - 1) * no * nv // 2
        same_spin = True
        # Full index:
        if index < end_aa:
            ...
            # return iJB block
            #
            i, j, b = np.unravel_index(index - end_aa, (no, no, nv))
            same_spin = False
...
class RSIPCC1(RSIPCC):
    ...
    def dimension(self):
        ...
        if self.spin_free:
            return no + no * no * nv
        return no + (no - 1) * no * nv // 2 + no * no * nv
    ...
    def print_ci_vector(self, ci_dict):
        ...
        **Arguments:**
        ...
        for ind, ci in ci_dict.items():
            if (ind - no) >= 0:
                i, j, b, same_spin = self.get_index_ijb(ind - no)
                state = "r_ijb" if same_spin else "r_iJB"
                log(
                    f"{state:>17}:   ({i+nc+1: 3},{j+nc+1: 3},"
                    f"{b+nco+1: 3})   {ci: 1.5f}"
                )
            else:
                log(f"{'r_i':>17}:           ({ind+nc+1: 3})   " f"{ci: 1.5f}")
    ...
    def print_weights(self, e_vec_j):
        ...
        **Arguments:**
        ...
        """
        no, nv = self.occ_model.nacto[0], self.occ_model.nactv[0]
        w_i = np.dot(e_vec_j[:no], e_vec_j[:no])
        w_ijb = 0
        w_iJB = 0
        log(f"{'weight(r_i)':>17}: {w_i: 1.5f}")
        if self.nhole >= 2:
            if self.spin_free:
                w_ijb = np.dot(e_vec_j[no:], e_vec_j[no:])
                log(f"{'weight(r_ijb)':>22}: {w_ijb: 1.5f}")
            else:
                end = (no - 1) * no * nv // 2 + no
                w_ijb = np.dot(e_vec_j[no:end], e_vec_j[no:end])
    ...
    def _check_multiplicity(self, e_vec_j, w_i, w_ijb, w_iJB):
        ...
        check = np.any(abs(e_vec_j[:no]) > 1e-5) and w_i > 1e-5
    ...
    def print_ci_vector(self, ci_dict):
        ...
        **Arguments:**
        ...
        for ind, ci in ci_dict.items():
            ...
            log(
                f"{'r_ijB':>17}:   ({i+nc+1: 3},{j+nc+1: 3},{b+nco+1: 3})"
                f"   {ci: 1.5f}"
            )
