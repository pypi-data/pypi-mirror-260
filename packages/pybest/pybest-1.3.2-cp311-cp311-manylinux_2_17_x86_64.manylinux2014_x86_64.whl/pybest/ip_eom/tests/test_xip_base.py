...
from pybest.linalg import DenseLinalgFactory, DenseOneIndex
...
def test_build_guess_vectors(guess_input):
    ...
    occ_model = AufbauOccModel(lf, nel=20)
    # Initialize empty class (we cannot use abc class)
...
def test_resolve_t(cls, kwargs, t_args):
    ...
    occ_model = AufbauOccModel(lf, nel=4 * 2)
    # Initialize empty class (we cannot use abc class)
...
def test_resolve_t_error(cls, kwargs, t_args, error):
...
def test_ip_pccd_unmask_raise_error(cls, args, raised_error, no_1m):
