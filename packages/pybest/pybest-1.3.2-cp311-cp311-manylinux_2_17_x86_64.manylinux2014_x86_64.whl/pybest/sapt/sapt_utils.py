...
# pybest imports
import pybest.gbasis.gobasis as gobasis_module
from pybest import io, log, timer
...
def transform_integrals_SAPT(
    ...
    out: Optional[List[FourIndex]] = None,
) -> Union[List[FourIndex], FourIndex]:
...
def prepare_cp_monomers(
    fn_geo: str,
) -> Tuple[io.IOData, io.IOData, io.IOData, List, List]:
    """
    Prepares CP-corrected (dimer centered) molecules via IOData.prepare_ghosts
    **Arguments**
    fn_geo
    ...
    mol_dim = io.IOData.from_file(fn_dim)
    mol_mon_A = io.IOData.from_file(fn_mon_A)
    mol_mon_B = io.IOData.from_file(fn_mon_B)

    ix_map_A, ix_map_B = io.IOData.prepare_ghosts(fn_mon_A, fn_mon_B, fn_dim)
...
def prepare_cp_molecules(
...
) -> Tuple[io.IOData, io.IOData, io.IOData]:
    """
    ...
    **Arguments:**
    ...
    gbasis_dim = gobasis_module.get_gobasis(basisname=basis, coords=mol_dim)
    gbasis_mon_a = gobasis_module.get_gobasis(
        basisname=basis, coords=mol_dim, dummy=dummy_mon_a
    )
    gbasis_mon_b = gobasis_module.get_gobasis(
    ...
    # dimer
    olp = gobasis_module.compute_overlap(gbasis_dim)
    kin = gobasis_module.compute_kinetic(gbasis_dim)
    na = gobasis_module.compute_nuclear(gbasis_dim)

    # ERI AO in DCBS are the same for monA, monB & dimer
    if fourindex_type.lower() == "dense":
        er = gobasis_module.compute_eri(gbasis_dim)

    # otherwise do cholesky eri
    elif fourindex_type.lower() == "cholesky":
        er = gobasis_module.compute_cholesky_eri(
    ...
    # mon a
    olp_a = gobasis_module.compute_overlap(gbasis_mon_a)
    kin_a = gobasis_module.compute_kinetic(gbasis_mon_a)
    na_a = gobasis_module.compute_nuclear(gbasis_mon_a)

    # mon b
    olp_b = gobasis_module.compute_overlap(gbasis_mon_b)
    kin_b = gobasis_module.compute_kinetic(gbasis_mon_b)
    na_b = gobasis_module.compute_nuclear(gbasis_mon_b)
...
def prepare_cp_hf(
    basis: str = "cc-pvdz",
    fn_geo: str = None,
    occupations: Union[Tuple[Dict, Dict, Dict], None] = None,
    fourindex_type: str = "Dense",
    er_threshold: float = 1e-10,
    updated_geometry: np.ndarray = None,
    solver: str = "ediis2",
    solver_opts: Union[Dict[str, Any], None] = None,
):
    """
    ...
    external_dim = gobasis_module.compute_nuclear_repulsion(dimer.gobasis)
    external_monA = gobasis_module.compute_nuclear_repulsion(monA.gobasis)
    external_monB = gobasis_module.compute_nuclear_repulsion(monB.gobasis)

    if solver:
        # compute dimer rhf
        occ_model_dim = AufbauOccModel(dimer.gobasis, **kwargs_dim)
        ...
        occ_model_monA = AufbauOccModel(monA.gobasis, **kwargs_monA)
        ...
        occ_model_monB = AufbauOccModel(monB.gobasis, **kwargs_monB)
    ...
    log(f"{raw_fn_geo:9s} {basis:14s} {str(nbasis):14s}")
    log(" ")
    log(f"{'':9s} {'NO OCC':14s} {'NO VIRT':14s}")
    log(f"{'DIMER':9s} {str(occ_dim):14s} {str(nvirt_dim):14s}")
    log(f"{'MONOMER A':9s} {str(occ_monA):14s} {str(nvirt_monA):14s}")
    log(f"{'MONOMER B':9s} {str(occ_monB):14s} {str(nvirt_monB):14s}")
    log(" ")
    ...
    log(
        f"{en_int:12.10f} {en_int_mhatree:14.8f} {en_int_kcal:14.8f}"
        f"{en_int_invcm:14.8f} {en_int_kelivn:14.8f}"
    )
