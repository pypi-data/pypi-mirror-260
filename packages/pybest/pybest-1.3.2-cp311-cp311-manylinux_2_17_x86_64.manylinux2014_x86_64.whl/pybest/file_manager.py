...
def handle_dir_args(new_dir: Union[str, Path]) -> Path:
    """helper function that does some common logic for checking `new_dir`

    Args:
        new_dir (Union[str, Path]): new dir candidate
...
class FileManager:
    ...
    * results dir, where the requested results are written
    ...
    @result_dir.setter
    def result_dir(self, new_dir: Union[str, Path]) -> None:
    ...
    @temp_dir.setter
    def temp_dir(self, new_dir: Union[str, Path]) -> None:
    ...
    def create_dir(pathlib_dir: Path) -> None:
        ...
        if not pathlib_dir.exists() and not pathlib_dir.is_dir():
            raise DirectoryError(
                f"Cannot create directory {str(pathlib_dir)}!"
            )
