...
def cholesky_td_routine(subscripts, operands_, arr, factor, save_memory=False):
    ...
    * save_memory : boolean. If True, a partial for-loop implementation is
    ...
    if subscripts == "abab->ab":
        ...
        tmp = td(op0, operands_[2], axes=([2, 0]))
        # xa,xbd->adb
        arr[:, :, :] += factor * td(tmp, op1, axes=([0, 0])).transpose(2, 1)
    elif subscripts == "abcd,d->abc":
        # first do xbd,d->xb
        tmp = td(op1, operands_[2], axes=([2, 0]))
        # xac,xb->abc
        arr[:, :, :] += factor * td(op0, tmp, axes=([0, 0])).transpose(2, 1)
        ...
        parts = int(op0.shape[0] / dim + 1)
        # either sum over [e] axis or partition [x] into smaller junks
        # the second option requires larger intermediates
        if not save_memory and (parts < dim):
            for i in range(parts):
                # for each part, we need to determine the range [i*dim:(i+1)*dim]
                end = (i + 1) * dim if (i + 1) < parts else None
                # generate a slice instance
                slice_ = tuple([slice(i * dim, end), slice(None), slice(None)])
                tmp = td(op0[slice_], operands_[2], axes=([2, 0]))
                arr[:] += factor * td(
                    tmp, op1[slice_], axes=([0, 2], [0, 2])
                ).transpose(0, 2, 1)
            return
        ...
        if not save_memory and (parts < dim):
            for i in range(parts):
                ...
                # generate a slice instance
                slice_ = tuple([slice(i * dim, end), slice(None), slice(None)])
                tmp = td(op0[slice_], operands_[2], axes=([2, 1]))
                arr[:] += factor * td(
                    tmp, op1[slice_], axes=([0, 2], [0, 2])
                ).transpose(0, 2, 1)
            return
        ...
        for i in range(op0.shape[1]):
            tmp = td(op1, op0[:, i, :], axes=([0, 0]))
            arr[:, i, :, :] += factor * td(
                operands_[2], tmp, axes=([1, 3], [2, 1])
            )
    elif subscripts == "abcd,cfed->afeb":
        for i in range(op0.shape[1]):
        ...
        for i in range(op0.shape[1]):
            tmp = td(op1, op0[:, i, :], axes=([0, 0]))
            arr[:, i, :, :] += factor * td(
                operands_[2], tmp, axes=([1, 3], [1, 2])
            )
    elif subscripts == "abcd,ecdf->eabf":
        for i in range(op0.shape[1]):
            tmp = td(op0[:, i, :], op1, axes=([0, 0]))
            arr[:, i, :, :] += factor * td(
                operands_[2], tmp, axes=([1, 2], [0, 2])
            ).transpose((0, 2, 1))
    elif subscripts == "abcd,efcd->efab":
        for i in range(op0.shape[1]):
            tmp = td(op1, op0[:, i, :], axes=([0, 0]))
            arr[:, :, i, :] += factor * td(
                operands_[2], tmp, axes=([2, 3], [2, 1])
            )
        ...
        for i in range(op0.shape[1]):
            tmp = td(op1, op0[:, i, :], axes=([0, 0]))
            arr[i, :, :, :] += factor * td(
                operands_[2], tmp, axes=([0, 2], [2, 1])
            ).transpose((0, 2, 1))
    elif subscripts == "abcd,cedf->beaf":
        for i in range(op0.shape[1]):
            tmp = td(op1, op0[:, i, :], axes=([0, 0]))
            arr[:, :, i, :] += factor * td(
                tmp, operands_[2], axes=([1, 2], [2, 0])
            )
    elif subscripts == "abcd,edaf->efbc":
        for i in range(op1.shape[1]):
            tmp = td(op0, op1[:, i, :], axes=([0, 0]))
            arr[:, :, i, :] += factor * td(
                operands_[2], tmp, axes=([1, 2], [2, 0])
            )
    elif subscripts in (
        "abcd,cedf->aefb",
        "abcd,cedf->abfe",
...
def cholesky_td_routine_abcd(subscripts, operands, arr, factor, save_memory):
