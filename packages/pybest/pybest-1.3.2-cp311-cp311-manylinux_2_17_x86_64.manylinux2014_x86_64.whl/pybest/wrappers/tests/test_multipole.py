...
from pybest.utils import get_com, unmask, unmask_orb
...
def test_transform_dm(mol_generation_oopccd_ncore, dm1_data):
    ...
    basis_fc = unmask("occ_model", molecule_fc.pccd)
    occ_model_fc = basis_fc
    scale = [1.0, 2.0, -2.0]
    orb = unmask_orb(*args)
    ...
    if isinstance(dm_1, OneIndex):
        dm_1 = unfolding_dm(dm_1)
    dm_1 = transform_dm(dm_1, occ_model_fc, orb)
    assert dm_1.shape == (occ_model_fc.nbasis[0], occ_model_fc.nbasis[0])
    for i in range(len(scale)):
        dm_1.iscale(scale[i])
        #  check if the density matrix is correct only when scaled by 2.0
        if isinstance(dm_1_unmask, OneIndex):
            if occ_model_fc.ncore[0] == 0:
                if scale[i] == 2.0:
                    assert np.allclose(
