...
def test_scf_solvers(name, basis, cls, kw_occ_model, expected):
    ...
    diff_e = expected["energy"]["total"] - scf.e_tot
    threshold = expected["threshold"]

    # FIXME: This should be simplified when we have a way to tell which libint
    # version we are using.
    # distinguish two different cases:
    # 1) CODATA 2014 (print warning)
    # 2) CODATA 2018 (no warning)
    # In case of CODATA 2014, the error should be at most 10*threshold
    # otherwise an error should be raised
    codata2014 = abs(diff_e) > threshold and abs(diff_e) < threshold * 10
    if codata2014:
        warnings.warn(
    ...
    assert abs(expected["energy"]["total"] - scf.e_tot) < threshold * 10
