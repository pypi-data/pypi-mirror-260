...
def dump_cube(filename, data):
    """Write data to file using the cube format. The cube file contains the
    following records (per line):
        * job title
    ...
    **Arguments:**
    ...
    filename
        The filename of the cube file, which is the output file for this
        routine.

    data
        An IOData instance. Must contain:
        * ``orb_a`` (an Orbital instance)
        * ``gobasis`` (a Basis instance) either as separate attribute or stored
          in the ``occ_model`` attribute
        ...
        * ``points`` (a tuple; the number of points in each direction; defaults
          to [80, 80, 80])
    """
    if log.do_medium:
        log(" ")
    ...
    if not any(hasattr(data, attr) for attr in ["gobasis", "occ_model"]):
        raise ArgumentError("Basis information gobasis is missing")
    if not hasattr(data, "indices_mos"):
        raise ArgumentError("List of orbitals to be printed are missing")
    #
    # Check consistency in basis set; gobasis is taken either from gobasis
    # attribute or from occ_model
    if hasattr(data, "occ_model") and hasattr(data, "gobasis"):
        assert (
            data.occ_model.factory == data.gobasis
...
def get_cube_data(data):
    """Return dictionary of all data required to generate cube files:
    * origin (a tuple; the origin of the cube file)
    * step (a tuple; the number of points in each direction; defaults
      to [80, 80, 80])
    * points
    """
...
def get_cube_origin(data, shift=-3.0):
    """Determine origin of grid based on molecular coordinates. If not
    provided, an origin is selected:
    ...
    **Arguments:**
    ...
    data
        (IOData) container containing the coordinates

    shift
        (float) a negative number to shift the origin
    """
    ...
    for coord in gobasis.coordinates:
...
def get_cube_step(data, origin, points, shift=3.0):
    """Determine step size in each grid dimension. If not provided, a step
    size will be provided based on the outer-most coordinates plus a shift
    (defaults to 2 bohr).

    **Arguments:**

    data
        (IOData) container with attribute `gobasis` or `occ_model` (stores an
        instance of gobasis)

    origin
        (list of floats) the origin of the grid

    points
        (list list of ints) number of grid points along each dimension

    shift
        (float) a number to shift the boundary of the cube

    **Returns:**
        (list) of step sizes [dx, dy, dz]
    """
...
def print_cube_header(filename, **cube_data):
    """Prints the header of a cube file

    **Arguments:**

    filename:
        (pathlib.Path(filename) instance) the filename

    **Keyword arguments:**

    data:
        (IOData instance) contains the information of the molecule taken from
        the gobasis or occ_model.gobasis attribute

    origin:
        (tuple) the origin of the grid

    step:
        (tuple) the step taken along each coordinate axis

    points:
        (tuple) the number of points in each direction
    ...
    with open(filename, "w", encoding="utf8") as f:
        ...
        #
        print(
            f"{len(gobasis.atom):>5} {origin[0]:> 12.6f} {origin[1]:> 12.6f} {origin[2]:> 12.6f}",
            file=f,
        )
        #
        ...
        # Atomic number, charge and coordinates; one such record for each atom
        #
        for i, atom in enumerate(gobasis.atom):
            x, y, z = gobasis.coordinates[i]
            print(
...
def dump_orbital_to_cube(filename, mo, **cube_data):
    """Dumps one orbital to a cube file with the name {filename} (contains
    the suffix .cube). This function only calculates the values at each grid
    point and appends them to the file {filename}. Note that no new file is
    created.
    ...
    **Arguments:**
    ...
    mo:
        (int) the number of the MO to be printed (Fortran indexing is assumed)

    **Keyword arguments:**

    data:
        (IOData instance) contains the gobasis and orbitals
