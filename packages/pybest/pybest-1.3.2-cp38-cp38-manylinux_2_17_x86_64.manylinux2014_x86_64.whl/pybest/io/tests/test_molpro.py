...
def test_dump_load_fcidimp_consistency_ao(tmp_dir):
    ...
    mol0.e_core = compute_nuclear_repulsion(obasis)
    mol0.nelec = 10
    mol0.ms2 = 1
    mol0.one = lf.create_two_index()
    mol0.one = compute_kinetic(obasis)
    mol0.one.iadd(compute_nuclear(obasis))
    mol0.two = compute_eri(obasis)
...
def test_dump_load_fcidimp_consistency_separate(tmp_dir):
    ...
    mol0.nelec = 10
    mol0.ms2 = 1
    mol0.kin = compute_kinetic(obasis)
    mol0.ne = compute_nuclear(obasis)
    mol0.eri = compute_eri(obasis)
