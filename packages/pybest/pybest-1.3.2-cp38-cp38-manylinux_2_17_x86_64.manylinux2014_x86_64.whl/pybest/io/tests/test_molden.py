...
def test_load_dump_molden_files(fn, expected):
    ...
    assert mol.gobasis.basisname == mol2.gobasis.basisname
    assert mol.gobasis.alpha == mol2.gobasis.alpha
    assert mol.gobasis.contraction == mol2.gobasis.contraction
    assert mol.gobasis.nbasis == mol2.gobasis.nbasis
    assert mol.gobasis.nprim == mol2.gobasis.nprim
    assert mol.gobasis.nshell == mol2.gobasis.nshell
    assert mol.gobasis.ncenter == mol2.gobasis.ncenter
    assert mol.gobasis.shell2atom == mol2.gobasis.shell2atom
    assert mol.gobasis.shell_types == mol2.gobasis.shell_types
    assert mol.gobasis.atom == mol2.gobasis.atom
    assert mol.gobasis.ncenter == mol2.gobasis.ncenter
    assert (
        abs(
            np.array(mol.gobasis.coordinates)
            - np.array(mol2.gobasis.coordinates)
        )
        < 1e-8
    ).all()
...
test_mol = [
    ...
    ("h2o", "cc-pvdz", ("orb_a", "occ_model", "gobasis"), {}),
    ("h2o", "cc-pvdz", ("orb_a",), {"gobasis": None}),
