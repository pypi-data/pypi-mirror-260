...
class pCCDPT2(Perturbation):
    ...
    def vfunction_0(self):
        ...
        **Arguments:**
        ...
        out = govvo.contract(
            "abcd->acdb", factor=1.0, clear=True, select="einsum"
        )
        return out
    ...
    def update_aux_matrix(self, mo1, mo2, cia):
        ...
        **Arguments:**
        ...
        mo2.contract("abcc->abc", out=tmpabc, factor=1.0, select="einsum")
        #
        # oc_jbc = sum_m <mm|bc> c_jm^bc
        #
        auxmat2 = self.init_aux_matrix("ocjbc")
        tmpabc.contract("abc,cb->ab", cia, tmp, clear=True, **vvo)
        cia.contract("ab,bc->abc", tmp, auxmat2, select="einsum")
        # P_jm
        tmpabc.contract("abc,ca->ab", cia, tmp, clear=True, **vvo)
        cia.contract("ac,bc->abc", tmp, auxmat2, select="einsum")
        #
        # vc_jkb = sum_d <dd|jk> c_jk^bd
        #
        ...
        tmp.contract("ab,ac->abc", cia, auxmat3, select="einsum")
        # P_jm
        tmpabc.contract("abc,ac->ab", cia, tmp, clear=True, **oov)
        tmp.contract("ab,bc->abc", cia, auxmat3, select="einsum")
        #
        ...
        mo2.contract(
            "aabb->ab", out=tmp, factor=1.0, clear=True, select="einsum"
        )
    ...
    def check_result(self, **kwargs):
        ...
        if excludepairs:
            ...
            tmp = self.amplitudes[0].contract(
                "abab->ab", clear=True, select="einsum"
            )
            if tmp.sum() > thresh:
    ...
    def vfunction_psi0(self, *args, **kwargs):
        ...
        **Arguments:**
        ...
        #
        govvo.contract(
            "abcd->acdb", out=out, factor=1.0, clear=True, select="einsum"
        )
        #
        # c_kc <jc||bk>
        #
        ...
        if self.include_fock:
            t_p.contract(
                "ac,bc->abc", fockmod, tmp3ind0, **vv2, select="einsum"
            )
        #
        # delta_jk [ c_jb F_bc ]
        #
        if self.include_fock:
            t_p.contract(
                "ab,bc->abc", fockmod, tmp3ind0, **vv2, select="einsum"
            )
        #
        # delta_jk [ oc_jbc ]
        #
        ...
        #
        out.iadd_expand_three_to_four("abc->abac", tmp3ind0, 1.0)
        #
        # delta_bc [ c_jb F_jk ]
        #
        if self.include_fock:
        ...
        #
        out.iadd_expand_two_to_four("diag", dcjb, 1.0)
        #
        # Single excitations
        #
        #
