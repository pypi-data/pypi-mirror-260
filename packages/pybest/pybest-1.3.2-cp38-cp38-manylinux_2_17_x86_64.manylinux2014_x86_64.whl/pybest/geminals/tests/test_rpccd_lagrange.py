...
from pybest.utils import fda_1order, numpy_seed
...
def test_pccd_lagrange():
    ...
    def fun(x):
        iiaa = geminal_solver.from_cache("gppqq")
        iaia = geminal_solver.from_cache("lpqpq")
        fock = geminal_solver.from_cache("fock")
        one = geminal_solver.from_cache("t")
        coeff = geminal_solver.lagrange_matrix._array

        lagrangian = geminal_solver.compute_total_energy(x.reshape(3, 25))
        lagrangian += np.dot(
            coeff.ravel(order="C"),
            geminal_solver.vector_function_geminal(
                x.ravel(order="C"), iiaa, iaia, one, fock
            ),
        )
        return lagrangian
    ...
    def fun_deriv(x):
        ...
        gradient = geminal_solver.vector_function_lagrange(
            coeff, gmat, iiaa, iaia, one, fock
        )
