    ...
    def log(self, cache):
        ...
        for count, e_orbital in enumerate(orbs.energies):
            ...
            if count == orbs.get_homo_index():
                orb_type = "HOMO"
                e_homo = e_orbital
            if count == orbs.get_lumo_index():
                orb_type = "LUMO"
                e_lumo = e_orbital
            # for all other orbitals we do not print the orbital type:
            log(
                f"\t\t{count:>4} {orb_type:>4}\t{e_orbital:> 17.7f}{e_orbital/electronvolt:> 13.3f}"
            )
        log.hline()
        log.hline()
        log(
            f"{'HOMO-LUMO gap':>20} \t\t{e_homo - e_lumo:> 17.7f}{(e_homo - e_lumo)/electronvolt:> 13.3f}"
        )
        log.hline()
        log.hline()
