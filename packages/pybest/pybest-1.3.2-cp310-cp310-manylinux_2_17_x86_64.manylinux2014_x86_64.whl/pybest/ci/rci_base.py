...
class RCI(ABC):
    ...
    def __init__(self, lf, occ_model):
        ...
        **Arguments:**
        ...
        if self._nvirt <= 0 or self._nvirt > (
            lf.default_nbasis - occ_model.nocc[0]
        ):
            raise ValueError(
                f"Impossible number of virtual orbitals: {self._nvirt}!"
            )

        if self._nactv <= 0 or self._nactv > (
            lf.default_nbasis - occ_model.nocc[0]
        ):
            raise ValueError(
    ...
    @property
    def tco(self):
        """The tensor contraction engine used"""
        return self._tco

    @tco.setter
    def tco(self, new):
        check_options("tco", new, None, "td", "einsum", "opt_einsum", "cupy")
        self._tco = new
    ...
    @property
    def e_ref(self):
        """The total energy of the reference wave function."""
        return self._e_ref
    ...
    @property
    def csf(self):
        """Configuration State Functions. If True, the CSF's are used during
    ...
    def csf(self, new):
        if not isinstance(new, bool):
            raise ArgumentError(
                "Unkown type for keyword csf. Boolean type required."
            )
        self._csf = new
    ...
    def init_cache(self, select, *args, **kwargs):
        ...
        **Keyword Arguments:**
        ...
        nvec
            ...
            class does not support different sizes of arrays. Default is None.
        """
    ...
    #
    def print_info(self):
        """Prints information about the CI calculations."""
        ...
        if log.do_medium:
            ...
            log(f"Number of frozen cores:            {self.ncore}")
            log(f"Number of active occupied:         {self.nacto}")
            log(f"Number of active virtuals:         {self.nactv}")
            ...
            log(f"Tensor contraction engine:         {tco}")
            log.hline("~")
        ...
        *  string - any sequence of 'o' (occupied) and 'v' (virtual)
        ...
        for char in string:
            if char == "o":
                ...
                range_["end" + str(ind)] = self.nacto
            elif char == "v":
                range_["begin" + str(ind)] = self.nacto
                range_["end" + str(ind)] = self.nact
            elif char == "V":
                range_["begin" + str(ind)] = 0
                range_["end" + str(ind)] = self.nactv
            elif char == "n":
                range_["begin" + str(ind)] = 0
                range_["end" + str(ind)] = self.nact
            else:
        ...
        **Keywords:**
        ...
        if self.davidson:
            ...
            if self.ci_method is None:
                e_ci, eigen_vector_real = davidson(self)
            else:
                e_ci, eigen_vector_real = davidson(self.ci_method)
        else:
            hamiltonian = self.calculate_exact_hamiltonian()
            e_ci, eigen_vector_real = np.linalg.eigh(hamiltonian.array)
        #
        # Do Checkpoint
        #
        self.checkpoint.to_file(self.checkpoint_fn)
        self.checkpoint.update("e_ci", e_ci[: self.nroot])
        self.checkpoint.update("civ", eigen_vector_real[:, : self.nroot])
        self.checkpoint.update("e_ref", self.e_ref)
        #
        # Perform a size-consistency correction
        #
        if self.size_consistency_correction:
            self.rci_corrections = RCICorrections(self.nacto)
            self.rci_corrections(
                eigen_vector_real[0][0],
                e_ci[0],
