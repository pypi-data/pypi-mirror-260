...
from pybest.ci.rci_utils import display
from pybest.ci.spin_free_base import SpinFree
...
class RpCCDCID(SpinFree):
    ...
    def compute_h_diag(self, *arg):
        ...
        # Intermediates
        #
        fii = self.lf.create_one_index(self.nacto)
        fock.copy_diagonal(out=fii, end=self.nacto)
        faa = self.lf.create_one_index(self.nactv)
        fock.copy_diagonal(out=faa, begin=self.nacto, end=self.nact)

        g_ijij = self.lf.create_two_index(self.nacto, self.nacto)
        goooo.contract("abab->ab", g_ijij)
        g_ijji = self.lf.create_two_index(self.nacto, self.nacto)
        goooo.contract("abba->ab", g_ijji, factor=-1.0)

        g_abab = self.lf.create_two_index(self.nactv, self.nactv)
        gvvvv.contract("abab->ab", g_abab)
        g_abba = self.lf.create_two_index(self.nactv, self.nactv)
        gvvvv.contract("abba->ab", g_abba, factor=-1.0)

        g_iaia = self.lf.create_two_index(self.nacto, self.nactv)
        govov.contract("abab->ab", g_iaia)
        g_iaai = self.lf.create_two_index(self.nacto, self.nactv)
        govvo.contract("abba->ab", g_iaai, factor=-1.0)

        r_iajb = self.denself.create_four_index(
    ...
    def build_subspace_hamiltonian(self, bvector, hamiltonian, *args):
        ...
        **Arguments:**
        ...
        sigma_p.iadd_expand_three_to_four("abc->abac", tmp_iab, factor=c_0)
        del tmp_iab
        ...
        sigma_p.iadd_expand_three_to_four("abc->abcb", tmp_iaj, factor=c_0)
        del tmp_iaj
        ...
        sigma_p.iadd_expand_two_to_four("diag", tmp, factor=c_0)
        ...
        b_d.contract("abcd,ce->abed", f_oo, out=sigma_p, factor=-1.0, **tco)

        # 14 Pia/jb fac c_icjb
        b_d.contract("abcd,eb->aecd", f_vv, out=sigma_p, **tco)

        # 15) +L_kaci r_kcJB
        b_d.contract("abcd,aebf->fecd", govvo, out=sigma_p, factor=2.0, **tco)

        b_d.contract("abcd,aefb->fecd", govov, out=sigma_p, factor=-1.0, **tco)

        # 16) -<ka|di> r_kbjd
        b_d.contract("abcd,aedf->fecb", govvo, out=sigma_p, factor=-1.0, **tco)

        # 17) -Pia/jb<ka|jd> r_kbid
        b_d.contract("abcd,aefd->cefb", govov, out=sigma_p, factor=-1.0, **tco)
        ...
        sigma_p.iadd_expand_three_to_four("abc->abcb", tmp_iaj)
        del tmp_iaj
        ...
        sigma_p.iadd_expand_three_to_four("abc->abac", tmp_iab)
        del tmp_iab
