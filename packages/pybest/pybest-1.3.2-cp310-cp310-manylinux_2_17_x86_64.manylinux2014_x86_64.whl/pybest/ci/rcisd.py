...
from pybest.ci.csf_cisd import CSFRCISD
from pybest.ci.rci_base import RCI
from pybest.ci.rci_utils import display, display_csf
from pybest.ci.rcid import RCID
from pybest.ci.rcis import RCIS
from pybest.ci.sd_cisd import SDRCISD
from pybest.linalg.cholesky import CholeskyFourIndex
from pybest.log import log


class RCISD(RCIS, RCID, RCI):
    ...
    def __init__(self, lf, occ_model, csf=True):
        ...
        self._csf = csf
        if self.csf:
            self._ci_method = CSFRCISD(self)
        else:
            self._ci_method = SDRCISD(self)
    ...
    def printer(self):
        ...
        end_s = 2 * self.nacto * self.nactv + 1
        alpha = "\u03B1"
        beta = "\u03B2"
        ...
        for ind, val in enumerate(evals):
            ...
            log(
                f"{ind:>3d} {val:> 17.6e} {self.e_ref+val:> 16.8f}"
    ...
    def collect_data(self, index, data, evecsj):
        ...
        *evecsj:
        ...
        if self.csf and self.print_csf:
            if index < end_1:
                i, a = RCIS.get_index_s(self, index - 1)
                i += self.ncore + 1
                a += self.ncore + self.nacto + 1
            else:
                i, a, j, b = RCID.get_index_d(self, index - end_1)
                i, a, j, b = (
                    i + self.ncore,
                    a + self.ncore,
                    j + self.ncore,
        ...
        if not self.csf and not self.print_csf:
            if index > 2 * end_a:
                i, a, j, b = RCID.get_index_d(self, index - 1 - 2 * end_a)
                i, a, j, b = (
                    i + ncore,
                    a + ncore,
                    j + ncore,
                ...
                i, a = RCIS.get_index_s(self, index - 1 - end_a)
