...
from pybest.gbasis.gobasis import compute_nuclear_repulsion, compute_overlap
...
def test_load_mkl_ethanol():
    ...
    assert mol.gobasis.nbasis == 39
    assert mol.gobasis.alpha[0] == 18.731137000
    assert mol.gobasis.alpha[10] == 7.868272400
    assert mol.gobasis.alpha[-3] == 2.825393700
    assert mol.gobasis.shell2atom[:5] == [0, 0, 1, 1, 1]
    assert mol.gobasis.shell_types[:5] == [0, 0, 0, 0, 1]
    assert mol.gobasis.nprim[-5:] == [3, 1, 1, 3, 1]
    ...
    assert abs(compute_nuclear_repulsion(mol.gobasis) - 81.87080034) < 1e-5

    # Check normalization
    olp = compute_overlap(mol.gobasis)
    mol.orb_a.check_normalization(olp, 1e-5)

    # Mulliken charges
    dm_full = mol.lf.create_two_index()
    dm_full = compute_1dm_hf(mol.orb_a)
    dm_full.iscale(2.0)
    charges = compute_mulliken_charges(mol.gobasis, mol.lf, numbers, dm_full)
