...
void compute_point_charges(
  ...
  if (uncontract == true) {
    ...
    uobs1.set_pure(true);
    // reset p shell back to cartesian so that we can compute integrals
  ...
  // Loading the charge-coordinate pairs for libint to put into its engine
