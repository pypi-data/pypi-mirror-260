...
def get_4idx_eri(geometry_str, basis_str):
    ...
    obasis = gobasis_module.get_gobasis(basis_str, mol)
    eri = gobasis_module.compute_eri(obasis)
    return obasis, eri._array
...
def assert_cd_eri(cd_eri, ref_eri, precision):
    ...
    print("")
    print("FROB. NORM: {:.3e}".format(frobenius_norm))
    print("MAX   DIFF: {:.3e}".format(max_diff))
...
def compute_cholesky(*args, **kwargs):
    return gobasis_module.compute_cholesky_eri(*args, **kwargs)


@pytest.mark.skipif(
    not gobasis_module.PYBEST_CHOLESKY_ENABLED,
...
def test_static_file():
    ...
    obs = gobasis_module.get_gobasis("cc-pVDZ", fn, print_basis=False)
...
def test_cd_eri_h2(basis, precision):
    obasis, ref_dense_eri = get_4idx_eri(H2_GEOMETRY, basis)
    cd_eri = compute_cholesky(obasis, threshold=precision)
    frob_norm, max_diff = assert_cd_eri(
...
def test_cd_eri_h2o(basis, precision):
    obasis, ref_dense_eri = get_4idx_eri(H2O_GEOMETRY, basis)
    cd_eri = compute_cholesky(obasis, threshold=precision)
    frob_norm, max_diff = assert_cd_eri(
        cd_eri._array, ref_dense_eri, precision=precision
    )
...
{} 0.000 0.000 0.000
""".strip()
    obasis, ref_dense_eri = get_4idx_eri(geometry.format(atom), basis)
    cd_eri = compute_cholesky(obasis, threshold=precision)
    frob_norm, max_diff = assert_cd_eri(
        cd_eri._array, ref_dense_eri, precision=precision
    )
