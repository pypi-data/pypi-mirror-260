...
from pybest.exceptions import NoSCFConvergence
from pybest.log import log
from pybest.scf.scf_diis import DIISHistory, DIISSCFSolver
from pybest.solvers.quadprog import QPSolver
from pybest.utils import doc_inherit
...
class EDIISSCFSolver(DIISSCFSolver):
    ...
    def __init__(
        ...
        **Optional arguments:**
        ...
        """
        log.cite("the EDIIS method", "kudin2002")
...
class EDIISHistory(DIISHistory):
    ...
    def __init__(self, lf, nvector, ndm, deriv_scale, overlap):
        ...
        **Arguments:**
        ...
        DIISHistory.__init__(
            self, lf, nvector, ndm, deriv_scale, overlap, [self.edots]
        )
        ...
        for i0 in range(self.nused - 1, -1, -1):
            ...
            for i1 in range(i0 + 1):
                ...
                for j in range(self.ndm):
                    self.edots[i0, i1] += state0.focks[j].contract(
                        "ab,ba", state1.dms[j]
                    )
                if i0 != i1:
                    ...
                    for j in range(self.ndm):
                        self.edots[i1, i0] += state1.focks[j].contract(
                            "ab,ba", state0.dms[j]
                        )
    ...
    def solve(self, dms_output, focks_output):
        ...
        qps = QPSolver(
            b, e, np.ones((1, self.nused)), np.array([1.0]), eps=1e-6
        )
        if self.nused < 10:
