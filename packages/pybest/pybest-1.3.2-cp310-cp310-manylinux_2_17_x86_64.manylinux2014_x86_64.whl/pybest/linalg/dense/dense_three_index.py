...
class DenseThreeIndex(ThreeIndex):
    ...
    def assign(
        ...
        **Optional arguments:**
        ...
        """
        check_type("other", other, DenseThreeIndex, np.ndarray, DenseOneIndex)
    ...
    def iadd_expand_one_to_three(
        ...
        """Expand one-index object along two axes and add it to three-index
        object.

        **Arguments:**

        axis
             The expansion is performed for the repeated indices in the
             three-index object
        ...
        **Optional arguments:**
        ...
        begin0, end0, begin1, end1, ...
             Can be used to select a subblock of the three-index and one-
             index object.
             When not given, the full range is used.
        """
        check_options("axis", axis, "a->aab", "a->abc", "b->abc", "c->abc")
        end0, end1, end2 = self.fix_ends(end0, end1, end2)
        end3 = one.fix_ends(end3)[0]
        if axis == "a->aab":
            assert begin0 == begin1
            assert end0 == end1
            assert (end1 - begin1) == (end3 - begin3)
            for i in range(begin1, end1):
                self.array[i, i, begin2:end2] += one.array[begin3 + i] * factor
        elif axis == "a->abc":
            assert begin0 == begin3
            assert end0 == end3
            assert (end0 - begin0) == (end3 - begin3)
            for i in range(begin1, end1):
                for j in range(begin2, end2):
                    self.array[:, i, j] += one.array[begin3:end3] * factor
        elif axis == "b->abc":
            assert begin1 == begin3
            assert end1 == end3
            assert (end1 - begin1) == (end3 - begin3)
            ...
            assert begin2 == begin3
            assert end2 == end3
            assert (end2 - begin2) == (end3 - begin3)
            for i in range(begin0, end0):
                for j in range(begin1, end1):
                    self.array[i, j, :] += one.array[begin3:end3] * factor

    def iadd_expand_two_to_three(
        self,
        axis,
        two,
        factor=1.0,
        ...
        """Expand two-index object along two axes and add it to three-index
        object.
        ...
        **Optional arguments:**
        ...
        abegin0, aend0, abegin1, aend1, abegin2, aend2
             FIXME
        """
        check_options(
            axis,
        ...
        aend0, aend1, aend2 = self.fix_ends(aend0, aend1, aend2)
        if axis == "ac->abc":
            for i in range(abegin1, aend1):
                self.array[:, i, :] += (
                    two.array[begin0:end0, begin1:end1] * factor
                )
        elif axis == "ab->abc":
            for i in range(abegin2, aend2):
                self.array[:, :, i] += (
                    two.array[begin0:end0, begin1:end1] * factor
                )
        elif axis == "bc->abc":
            for i in range(abegin0, aend0):
                self.array[i, :, :] += (
                    two.array[begin0:end0, begin1:end1] * factor
                )
        elif axis == "ab->aab":
            ...
            for i in range(abegin0, aend0):
                self.array[i, i, :] += two.array[i, begin1:end1] * factor
        elif axis == "aa->abc":
            assert two.nbasis == two.nbasis1
            for i in range(abegin1, aend1):
                for j in range(abegin2, aend2):
                    self.array[:, i, j] += (
                        two.array[begin0:end0, begin1:end1].diagonal() * factor
                    )
        elif axis == "cc->abc":
            assert two.nbasis == two.nbasis1
            for i in range(abegin0, aend0):
                for j in range(abegin1, aend1):
                    self.array[i, j, :] += (
                        two.array[begin0:end0, begin1:end1].diagonal() * factor
                    )
        elif axis == "bb->abc":
