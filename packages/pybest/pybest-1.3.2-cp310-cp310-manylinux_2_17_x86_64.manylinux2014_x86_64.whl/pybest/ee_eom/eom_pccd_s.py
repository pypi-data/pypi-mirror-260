...
class REOMpCCDS(REOMpCCDSBase):
    ...
    @timer.with_section("Ham EOM-pCCD+S")
    def build_full_hamiltonian(self):
        ...
        m10ia = self.from_cache("m10ia")
        #
        # H_kc,0
        #
        end_s = self.nacto * self.nactv + 1
        eom_ham.iadd(m10ia.ravel(), begin0=1, end0=end_s, end1=1)
        return eom_ham

    @timer.with_section("Hdiag EOMpCCD+S")
    ...
    @timer.with_section("SubHam EOM-pCCD+S")
    def build_subspace_hamiltonian(self, bvector, hdiag, *args):
        ...
        **Arguments:**
        ...
        #
        # Xkc0 r0
        end_s = self.nactv * self.nacto + 1
        sigma.iadd(
            m10ia.ravel(), factor=bvector.get_element(0), begin0=1, end0=end_s
        )

        return sigma

    @timer.with_section("Cache EOM-pCCD+S")
    def update_hamiltonian(self, mo1, mo2):
        ...
        **Arguments:**
        ...
        #
        aux_mat_ia0 = self.init_cache("m10ia", self.nacto, self.nactv)
        # fia
        aux_mat_ia0.iadd(fock, 1.0, **ov2)
        # fia cia
        aux_mat_ia0.iadd_mult(cia, fock, 1.0, **ov2)
        # <ia|cc> cic
        mo2.contract("abcc,ac->ab", cia, aux_mat_ia0, **ovvv)
        # <ia|ll> cla
        mo2.contract("abcc,cb->ab", cia, aux_mat_ia0, factor=-1.0, **ovoo)
        #
