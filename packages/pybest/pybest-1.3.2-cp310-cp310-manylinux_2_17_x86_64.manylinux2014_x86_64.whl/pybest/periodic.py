...
from pybest.units import amu, angstrom
        ...
        **Returns:** the corresponding :class:`Element` instance
        ...
        if result is None and isinstance(index, str):
            ...
            if result is None and index.isdigit():
                ...
                if result is None:
                    raise KeyError(f"Could not find element {index}.")
        return result
...
def load_periodic():
    ...
    with open(context.get_fn("elements.csv"), "r", encoding="utf8") as f:
        r = csv.reader(f)
