...
def test_get_mask_shape(sz, spin, shift, occ, nbasis, ncore):
    ...
    # check shape
    no, nv = occ_model.nacto[0], occ_model.nactv[0]
    for mask_ in mask:
        assert mask_.shape == (no, no, no, nv), "get_mask has wrong shape"


@pytest.mark.parametrize("sz, spin, shift", test_data_sz)
@pytest.mark.parametrize("occ, nbasis, ncore", test_data_mask)
def test_get_mask_elements(sz, spin, shift, occ, nbasis, ncore):
    ...
    mask = [dipcc.get_mask(spin_) for spin_ in spin]
    # check elements
    no, nv = range(occ_model.nacto[0]), range(occ_model.nactv[0])
    for mask_, spin_ in zip(mask, spin):
        for i, j, k, c in itertools.product(no, no, no, nv):
            if sz == 0:
                if spin_ and i < k:
                    # unique block (iJkc)
...
@pytest.mark.parametrize("sz, spin, shift", test_data_sz)
@pytest.mark.parametrize("occ, nbasis, ncore", test_data_mask)
def test_get_index_of_mask(sz, spin, shift, occ, nbasis, ncore):
        ...
        for j in range(dim1):
            if sz == 0.0:
                if spin[i]:
                    ...
                    assert mask[i][0][j] < no - 1
                    assert mask[i][1][j] < no
                else:
                    # check j<k for same spin, unique block (iJKC)
                    assert mask[i][1][j] < mask[i][2][j]
                    assert mask[i][0][j] < no
                    assert mask[i][1][j] < (no - 1)
                assert mask[i][2][j] < no
                assert mask[i][3][j] < nv
            else:
                ...
                if spin[i]:
                    ...
                    assert mask[i][1][j] < mask[i][2][j]
                assert mask[i][0][j] < (no - 1 - shift[i])
                assert mask[i][1][j] < (no - shift[i])
                assert mask[i][2][j] < no
                assert mask[i][3][j] < nv
...
def test_alpha(cls, expected):
    ...
    occ_model = AufbauOccModel(lf, nel=8)
...
def test_get_index_ijkc(nbasis, nocc, ncore, sz, ind, expected):
...
def test_check_nhole(cls, nbasis, nocc, ncore, sz, nhole, expected):
