...
@pytest.mark.parametrize("basis,mol,kwargs,expected", test_fermi)
@pytest.mark.parametrize("occ_model_class", test_occ_model)
def test_occ_fermi_gobasis(basis, mol, kwargs, expected, occ_model_class):
    """Test Fermi Aufbau model for gobasis, charge, alpha, and ncore arguments
    as input."""
    ...
    occ_model = occ_model_class(gobasis, **kwargs)
...
@pytest.mark.parametrize("basis,mol,kwargs,expected", test_fermi)
@pytest.mark.parametrize("occ_model_class", test_occ_model)
def test_occ_fermi_lf(basis, mol, kwargs, expected, occ_model_class):
...
@pytest.mark.parametrize("basis,mol", test_molecule)
@pytest.mark.parametrize("kwargs", test_wrong_arguments)
@pytest.mark.parametrize("factory", [Basis, DenseLinalgFactory])
def test_occ_fermi_arguments_gobasis(basis, mol, kwargs, factory):
    """Test Fermi Aufbau model for gobasis, nocc_a/nocc_b, and ncore arguments
    as input.
...
def test_occ_fermi_kwargs(
    ...
    mol: str,
    kwargs: Dict[str, Any],
    factory: Union[Basis, DenseLinalgFactory],
):
    """Test Fermi model for gobasis, nocc_a/nocc_b, and ncore arguments as
    input.
    """
    fn_xyz = context.get_fn(mol)
    gobasis = get_gobasis(basis, fn_xyz, print_basis=False)
