...
# cpp defined object
from .pybasis import Basis, from_coordinates
...
def __pybasis_from_hdf5(group: h5py.Group) -> Basis:
    """Method used as a class method, in monkeypatched pybasis.Basis (pybind11 export)
    return an instance of pybasis.Basis object from hdf5 dump.

    Args:
        group (h5py.Group): group in which Basis object information is stored

    Returns:
        Basis: pybasis.Basis object (pybind11 export)
    """
    # pybind11 autoconverts std::vector to list, so we must cast all np.ndarray instances
...
Basis.from_hdf5 = __pybasis_from_hdf5
Basis.to_hdf5 = __pybasis_to_hdf5


# define test runner
from pybest._pytesttester import PytestTester  # noqa
from pybest.gbasis.dkhn import compute_dkh  # noqa

# NOTE: gobasis imports must be after Basis class has been monkeypatched
from pybest.gbasis.gobasis import (  # noqa
...
from pybest.gbasis.gobasis_helper import shell_int2str, shell_str2int  # noqa
