...
from pybest.ci.csf_cid import CSFRCID
...
class CSFRCISD(CSF):
    ...
    def h_diag_singles(self):
        ...
        rci = self.object
        self.dimension = "CIS"
        hdiag = rci.denself.create_one_index(self.dimension)
    ...
    def compute_h_diag(self, *args):
        ...
        hdiag.assign(
            hdiag_s.array[1:], begin0=1, end0=rci.nactv * rci.nacto + 1
        )
        hdiag.iadd(hdiag_d.array[1:], begin0=rci.nactv * rci.nacto + 1)
        return hdiag
    ...
    def build_subspace_hamiltonian(self, bvector, hamiltonian, *args):
        ...
        **Arguments:**
        ...
        gooov = rci.from_cache("gooov")
        govvv = rci.from_cache("govvv")
        #
        # Ranges
        #
        ov = rci.get_range("ov")
        ...
        # Sigma Vectors
        #
        sigma = rci.denself.create_one_index(self.dimension)
        sigma_s = rci.denself.create_two_index(nacto, nactv)
        sigma_ia = rci.denself.create_two_index(nacto, nactv)
        sigma_iab = rci.denself.create_three_index(nacto, nactv, nactv)
        sigma_iaj = rci.denself.create_three_index(nacto, nactv, nacto)
        sigma_iajb_a = rci.denself.create_four_index(
            nacto, nactv, nacto, nactv
        )
        sigma_iajb_b = rci.denself.create_four_index(
            nacto, nactv, nacto, nactv
        )
        #
        # Bvectors
        #
        b_sa = rci.denself.create_two_index(nacto, nactv)
        bvect_ia = rci.denself.create_two_index(nacto, nactv)
        bvect_iaib = rci.denself.create_four_index(nacto, nactv, nacto, nactv)
        #
        # Bvectors assignment
        #
        ...
        tmp.iadd_expand_three_to_four("abc->abac", sigma_iab)
        sigma.iadd(
        ...
        if nacto > 1:
            tmp = rci.denself.create_four_index(nacto, nactv, nacto, nactv)
            tmp.iadd_expand_three_to_four("abc->abcb", sigma_iaj)
            sigma.iadd(
    ...
    def build_subspace_hamiltonian_singles(self, bvector, hamiltonian, *args):
        ...
        **Arguments:**
        ...
        b_sa = rci.lf.create_one_index(self.dimension)
    ...
    def build_subspace_hamiltonian_doubles(self, bvector, hamiltonian, *args):
        ...
        **Arguments:**
        ...
        self.dimension = "CID"
        sigma = rci.lf.create_one_index(self.dimension)
        b_d = rci.lf.create_one_index(self.dimension)
        b_d.set_element(0, bvector.get_element(0))
        b_d.assign(bvector.array[rci.nacto * rci.nactv + 1 :], begin0=1)
