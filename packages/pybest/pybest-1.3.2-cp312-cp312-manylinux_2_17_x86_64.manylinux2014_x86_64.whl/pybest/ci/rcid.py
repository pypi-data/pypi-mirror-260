...
from pybest.utils import check_options
...
class RCID(RCI):
    ...
    def __init__(self, lf, occ_model, csf=True):
        ...
        self._csf = csf
        if self.csf:
            self._ci_method = CSFRCID(self)
        else:
            self._ci_method = SDRCID(self)
    ...
    def collect_data(self, index, data, evecsj):
        ...
        *evecsj:
        ...
        if not self.csf and not self.print_csf:
            end_ab = self.nacto * self.nacto * self.nactv * self.nactv
            end_aa = end_ab + ((self.nacto * (self.nacto - 1)) // 2) * (
                (self.nactv * (self.nactv - 1)) // 2
            )
            if index < end_ab:
