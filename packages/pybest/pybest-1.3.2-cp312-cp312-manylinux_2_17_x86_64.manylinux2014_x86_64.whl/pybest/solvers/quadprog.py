...
r"""A light-weight quadratic programming solver
   ...
   sufficient for the needs in PyBEST.
...
def check_constrained_problem(a, b, r=None, s=None):
    ...
    **Optional arguments:**
    ...
    nx = len(a)
    if nx == 0:
        raise ArgumentError("The array a can not be empty.")
    if a.shape != (nx, nx):
        raise ArgumentError("The array a must be square.")
    if not (a == a.T).all():
        raise ArgumentError("The matrix a must be symmetric.")
    if b.shape != (nx,):
        raise ArgumentError("The size fo a and b do not match.")
    if r is not None:
        nl = len(r)
        if nl > nx:
            raise ConsistencyError(
                "There must not be more constraints than unknowns."
            )
        if r.shape != (nl, nx):
            raise ConsistencyError(
    ...
    **Optional arguments:**
    ...
    if evals is None:
        ...
        if min_radius > radius:
            raise FeasibilityError(
                f"The constraints do not allow a solution closer to the center than "
                f"{min_radius} > radius."
            )
        return x0
    if basis is None:
...
class QPSolver(object):
        ...
        **Optional arguments:**
        ...
        if self.nl > 0:
            if abs(np.dot(self.r, x) - self.s).max() > self.eps:
                raise FeasibilityError(
                    "Point does not satisfy equality constraints."
                )
        if free is None:
            if (x < 0).any():
    ...
    def get_rmsds(self, x):
        ...
        **Returns:**
        ...
        rmsd_frozen = np.sqrt(
            (gradient**2 * (~free) * (gradient < 0)).mean()
        )
    ...
    def compute_cn(self, free):
        ...
        **Arguments:**
        ...
        """
        a_free, b_free, r_free, s_free = self.get_free_problem(free)
        x0, basis, evals, evecs, b_diag = diagonal_form(
            a_free, b_free, r_free, s_free
        )
        if evals is None:
    ...
    def solve_radius(self, free, center, radius):
        """Solve the equations, keeping certain components fixed at zero, with maximum
        radius.
        ...
        **Arguments:**
        ...
        a_free, b_free, r_free, s_free = self.get_free_problem(free)
        x_free = solve_radius(
            a_free, b_free, center[free], radius, r_free, s_free
        )
        return self._free_to_full(x_free)
    ...
    def find_local(self, x, trust_radius, maxiter=None):
        ...
        **Returns:**
        ...
        self.log(guess)
        raise NoConvergence("Local search failed to converge")
