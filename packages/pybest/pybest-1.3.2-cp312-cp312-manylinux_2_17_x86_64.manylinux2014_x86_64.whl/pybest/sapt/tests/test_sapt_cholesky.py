...
import pybest.context as context
...
def test_plain_solver_he2(thresh=1e-6):
    fn_name = "he2.xyz"
    abs_path = context.get_fn("test/" + fn_name)
    dimer, monA, monB = sapt_utils.prepare_cp_hf(
        "cc-pvdz", abs_path, fourindex_type="cholesky"
    )
    en_int = dimer.e_hf - monA.e_hf - monB.e_hf
    assert abs(en_int - 0.000026523798) < thresh
...
def test_diis_solver_he2(thresh=1e-6):
    fn_name = "he2.xyz"
    abs_path = context.get_fn("test/" + fn_name)
    dimer, monA, monB = sapt_utils.prepare_cp_hf(
        "aug-cc-pvdz",
    ...
    en_int = dimer.e_hf - monA.e_hf - monB.e_hf
    assert abs(en_int - 0.00003032524) < thresh
...
def get_sapt0_corrections():
    fn_name = "arhf.xyz"
    arhf_path = context.get_fn("test/" + fn_name)
    dimer, monA, monB = sapt_utils.prepare_cp_hf(
        basis="cc-pvdz",
        fn_geo=arhf_path,
        fourindex_type="cholesky",
