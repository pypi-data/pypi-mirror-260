...
from pingsafe_cli.psgraph.common.runners.runner_registry import OUTPUT_CHOICES, SUMMARY_POSITIONS
...
class ExtArgumentParser(configargparse.ArgumentParser):
    ...
    def add_parser_args(self) -> None:
        ...
        self.add(
            "--support",
            action="store_true",
            help="Enable debug logs and upload the logs to the server. Requires a PingSafe or Prisma Cloud API key.",
            default=None
        )
        self.add(
            "-d",
        ...
        self.add(
            ...
            "--file",
            action="append",
            help="File to scan (can not be used together with --directory). With this option, PingSafe will attempt "
        ...
        self.add(
            "--output-bc-ids",
            action="store_true",
            help="Print PingSafe platform IDs (BC...) instead of PingSafe IDs (CKV...), if the check exists in the platform",
        )
        self.add(
            "--include-all-pingsafe-policies",
            action="store_true",
            help="When running with an API key, PingSafe will omit any policies that do not exist "
                 "in the PingSafe or Prisma Cloud platform, except for local custom policies loaded with the "
                 "--external-check flags. Use this key to include policies that only exist in PingSafe in the scan. "
        ...
        self.add(
            ...
            help="Checks to run; any other checks will be skipped. Enter one or more items separated by commas. "
                 "Each item may be either a PingSafe check ID (CKV_AWS_123), a BC check ID (BC_AWS_GENERAL_123), or "
                 ...
                 "check will still be run (see --include-all-pingsafe-policies for more info).",
            action="append",
            default=None,
            env_var="CKV_CHECK",
        )
        ...
        self.add(
            ...
            help="Exits with a 0 exit codescanner if only the specified items fail. Enter one or more items "
                 "separated by commas. Each item may be either a PingSafe check ID (CKV_AWS_123), a BC "
        ...
        self.add(
            ...
            env_var="BC_API_KEY",
            sanitize=True,
            help="PingSafe API key or Prisma Cloud Access Key (see --prisma-api-url)",
        )
        self.add(
            "--prisma-api-url",
        ...
        self.add(
            "--skip-download",
            help="Do not download any data from PingSafe. This will omit doc links, severities, etc., as well as "
                 "custom policies and suppressions if using an API token. Note: it will prevent BC platform IDs from "
                 "being available in PingSafe.",
            action="store_true",
        )
        self.add(
        ...
        self.add(
            "--skip-fixes",
            help="Do not download fixed resource templates from PingSafe. Only has effect when using the API key.",
            action="store_true",
        )
        self.add(
        ...
        self.add(
            "--config-file",
            help="path to the PingSafe configuration YAML file",
            is_config_file=True,
            default=None,
        )
        self.add(
        ...
        self.add(
            "--create-baseline",
            help="Alongside outputting the findings, save all results to .pingsafe.baseline file"
        ...
        self.add(
            "--baseline",
            help="Use a .pingsafe.baseline file to compare current results with a known baseline. "
