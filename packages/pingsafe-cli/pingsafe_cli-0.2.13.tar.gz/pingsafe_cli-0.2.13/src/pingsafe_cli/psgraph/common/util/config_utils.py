...
def config_file_paths(dir_path: str | Path) -> list[str]:
    return [os.path.join(dir_path, '.pingsafe.yaml'), os.path.join(dir_path, '.pingsafe.yml')]


def get_default_config_paths(argv: list[str]) -> list[str]:
    """
    PingSafe looks for .pingsafe.yml or .pingsafe.yaml file in the directory (--directory) against which it is run.
