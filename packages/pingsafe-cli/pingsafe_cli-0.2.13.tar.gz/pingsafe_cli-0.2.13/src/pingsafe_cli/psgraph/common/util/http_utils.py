...
from pingsafe_cli.psgraph.common.util.consts import DEV_API_GET_HEADERS, DEV_API_POST_HEADERS, PRISMA_API_GET_HEADERS, \
    PRISMA_PLATFORM, PINGSAFE_PLATFORM
...
def get_auth_error_message(status: int, is_prisma: bool, is_s3_upload: bool) -> str:
    platform_type = PRISMA_PLATFORM if is_prisma else PINGSAFE_PLATFORM
...
def get_user_agent_header() -> dict[str, str]:
    return {'User-Agent': f'pingsafe/{pingsafe_version}'}
