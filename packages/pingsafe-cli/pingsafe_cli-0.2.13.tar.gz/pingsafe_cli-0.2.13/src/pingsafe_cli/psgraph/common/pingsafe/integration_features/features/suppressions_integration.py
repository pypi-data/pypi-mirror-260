...
class SuppressionsIntegration(BaseIntegrationFeature):
        ...
        try:
            ...
            for suppression in suppressions:
                if suppression['policyId'] in metadata_integration.bc_to_ckv_id_mapping:
                    suppression['pingsafePolicyId'] = metadata_integration.get_ckv_id_from_bc_id(suppression['policyId'])
                else:
                    suppression['pingsafePolicyId'] = suppression['policyId']  # custom policy

            self._init_repo_regex()
            suppressions = sorted(suppressions, key=lambda s: s['pingsafePolicyId'])

            # group and map by policy ID
            self.suppressions = {policy_id: list(sup) for policy_id, sup in
                                 groupby(suppressions, key=lambda s: s['pingsafePolicyId'])}
    ...
    def _check_suppression(self, record: Record, suppression: dict[str, Any]) -> bool:
        ...
        :return:
        """
        if record.check_id != suppression['pingsafePolicyId']:
            return False
    ...
    def _suppression_valid_for_run(self, suppression: dict[str, Any]) -> bool:
        ...
        - the policy does not have a PingSafe ID and does not have an ID matching a custom policy format
