...
from pingsafe_cli.psgraph.common.output.common import format_string_to_licenses
...
from pingsafe_cli.psgraph.common.output.report import CheckType
from pingsafe_cli.psgraph.common.output.cyclonedx_consts import (
    SCA_CHECKTYPES,
    PURL_TYPE_MAVEN,
...
from pingsafe_cli.psgraph.common.output.record import SCA_PACKAGE_SCAN_CHECK_NAME
from pingsafe_cli.psgraph.common.sca.commons import UNFIXABLE_VERSION, get_fix_version
...
class CycloneDX:
    ...
    def create_bom(self) -> Bom:
        ...
        try:
            version = meta_version("pingsafe")
        except Exception:
            # Unable to determine current version of 'pingsafe'
            version = "UNKNOWN"

        this_tool = Tool(vendor="pingsafe", name="pingsafe", version=version)
    ...
    def create_library_component(self, resource: Record | ExtraResource, check_type: str) -> Component:
        ...
        if check_type is CheckType.SCA_IMAGE:
            ...
            if PINGSAFE_DISPLAY_REGISTRY_URL and registry_url and is_private_registry:
    ...
    def create_iac_vulnerability(self, resource: Record, component: Component) -> Vulnerability:
        ...
        <vulnerability bom-ref="41f657e7-a83b-4535-9b83-541211d02397">
          ...
          <advisories>
            <advisory>
              <url>https://docs.pingsafe.io/docs/s3_16-enable-versioning</url>
            </advisory>
          </advisories>
          <affects>
            <target>
        ...
        vulnerability = Vulnerability(
            ...
            source=VulnerabilitySource(name="pingsafe"),
            ratings=[
                VulnerabilityRating(
                    severity=severity,
                )
    ...
    def get_output(self, output_format: OutputFormat) -> str:
        ...
        schema_version = CYCLONE_SCHEMA_VERSION.get(
            os.getenv("PINGSAFE_CYCLONEDX_SCHEMA_VERSION", ""), DEFAULT_CYCLONE_SCHEMA_VERSION
        )
        output = get_instance(
            bom=self.bom,
