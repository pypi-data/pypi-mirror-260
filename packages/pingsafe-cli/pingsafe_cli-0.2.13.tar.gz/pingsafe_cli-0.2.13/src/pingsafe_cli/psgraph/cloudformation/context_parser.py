...
from pingsafe_cli.psgraph.common.parsers.node import DictNode, StrNode, ListNode
from pingsafe_cli.psgraph.common.typing import _SkippedCheck
...
class ContextParser:
    ...
    def evaluate_default_refs(self) -> None:
        ...
        for ref in refs:
            ...
            if default_value is not None:
                ...
                # Output in PingSafe looks like this:
    ...
    def find_lines(node: Union[ListNode, DictNode], kv: str) -> Generator[int, None, None]:
        # Hack to allow running pingsafe on json templates
    ...
    def collect_skip_comments(entity_code_lines: List[Tuple[int, str]], resource_config: Optional[DictNode] = None) -> List[_SkippedCheck]:
        ...
        if resource_config:
            ...
            if metadata:
                ckv_skip = metadata.get("pingsafe", {}).get("skip", [])
                bc_skip = metadata.get("pingsafe", {}).get("skip", [])
