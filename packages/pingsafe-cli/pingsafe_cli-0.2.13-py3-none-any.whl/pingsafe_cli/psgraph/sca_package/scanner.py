...
from pingsafe_cli.psgraph.common.util.file_utils import compress_file_gzip_base64, decompress_file_gzip_base64
...
class Scanner:
    ...
    async def execute_twistcli_scan(
        ...
        command = f"{Path(pingsafe_dir) / TWISTCLI_FILE_NAME} coderepo scan --address {docker_image_scanning_integration.get_proxy_address()} --token {docker_image_scanning_integration.get_bc_api_key()} --details --output-file \"{output_path}\" {input_path}"
