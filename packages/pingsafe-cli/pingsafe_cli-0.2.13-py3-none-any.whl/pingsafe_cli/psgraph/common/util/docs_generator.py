...
from pingsafe_cli.psgraph.ansible.checks.registry import registry as ansible_registry
from pingsafe_cli.psgraph.argo_workflows.checks.registry import registry as argo_workflows_registry
from pingsafe_cli.psgraph.arm.registry import arm_resource_registry, arm_parameter_registry
from pingsafe_cli.psgraph.azure_pipelines.checks.registry import registry as azure_pipelines_registry
from pingsafe_cli.psgraph.bicep.checks.param.registry import registry as bicep_param_registry
from pingsafe_cli.psgraph.bicep.checks.resource.registry import registry as bicep_resource_registry
from pingsafe_cli.psgraph.bitbucket.registry import registry as bitbucket_configuration_registry
from pingsafe_cli.psgraph.bitbucket_pipelines.registry import registry as bitbucket_pipelines_registry
from pingsafe_cli.psgraph.circleci_pipelines.registry import registry as circleci_pipelines_registry
from pingsafe_cli.psgraph.cloudformation.checks.resource.registry import cfn_registry as cfn_registry
from pingsafe_cli.psgraph.common.checks.base_check_registry import BaseCheckRegistry
from pingsafe_cli.psgraph.common.checks_infra.registry import BaseRegistry as BaseGraphRegistry, get_graph_checks_registry
from pingsafe_cli.psgraph.common.runners.base_runner import strtobool
from pingsafe_cli.psgraph.dockerfile.registry import registry as dockerfile_registry
from pingsafe_cli.psgraph.github.registry import registry as github_configuration_registry
from pingsafe_cli.psgraph.github_actions.checks.registry import registry as github_actions_jobs_registry
from pingsafe_cli.psgraph.gitlab.registry import registry as gitlab_configuration_registry
from pingsafe_cli.psgraph.gitlab_ci.checks.registry import registry as gitlab_ci_jobs_registry
from pingsafe_cli.psgraph.kubernetes.checks.resource.registry import registry as k8_registry
from pingsafe_cli.psgraph.secrets.runner import CHECK_ID_TO_SECRET_TYPE
from pingsafe_cli.psgraph.serverless.registry import sls_registry
from pingsafe_cli.psgraph.terraform.checks.data.registry import data_registry
from pingsafe_cli.psgraph.terraform.checks.module.registry import module_registry
from pingsafe_cli.psgraph.terraform.checks.provider.registry import provider_registry
from pingsafe_cli.psgraph.terraform.checks.resource.registry import resource_registry
from pingsafe_cli.psgraph.openapi.checks.registry import openapi_registry
...
CODE_LINK_BASE = ''
CREATE_MARKDOWN_HYPERLINKS = strtobool(os.getenv("PINGSAFE_CREATE_MARKDOWN_HYPERLINKS", "FALSE"))
SKIP_CHECK_IDS = {
...
def print_checks(frameworks: Optional[List[str]] = None, use_bc_ids: bool = False,
                 include_all_pingsafe_policies: bool = True, filtered_policy_ids: Optional[List[str]] = None) -> None:
    framework_list = frameworks if frameworks else ["all"]
    printable_checks_list = get_checks(framework_list, use_bc_ids=use_bc_ids,
                                       include_all_pingsafe_policies=include_all_pingsafe_policies,
...
def get_check_link(absolute_path: str) -> str:
    ...
    temp = absolute_path.split("pingsafe")
    # this will even work in the likely event that you're running pingsafe from a folder called pingsafe
...
def get_checks(frameworks: Optional[List[str]] = None, use_bc_ids: bool = False,
               include_all_pingsafe_policies: bool = True, filtered_policy_ids: Optional[List[str]] = None) -> List[Tuple[str, str, int, int, str, str]]:
    framework_list = frameworks if frameworks else ["all"]
    printable_checks_list: list[tuple[str, str, str, str, str, str]] = []
    filtered_policy_ids = filtered_policy_ids or []
    runner_filter = RunnerFilter(include_all_pingsafe_policies=include_all_pingsafe_policies, filtered_policy_ids=filtered_policy_ids)
