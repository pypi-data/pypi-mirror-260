...
from pingsafe_cli.psgraph.common.util.http_utils import extract_error_message, get_default_post_headers
...
class FixesIntegration(BaseIntegrationFeature):
    ...
    def fixes_url(self) -> str:
        return f"{self.bc_integration.api_url}/api/v1/fixes/pingsafe"
    ...
    def _get_platform_fixes(self, scan_report: Report) -> None:
        ...
        for file, sorted_failed_checks in groupby(sorted_by_file, key=lambda c: c.file_abs_path):
            ...
            # a mapping of (pingsafe_check_id, resource_id) to the failed check Record object for lookup later
            ...
            for fix in all_fixes:
                ...
                if not ckv_id:
                    logging.debug(f"BC ID {fix['policyId']} has no PingSafe ID - might be a cloned policy")
