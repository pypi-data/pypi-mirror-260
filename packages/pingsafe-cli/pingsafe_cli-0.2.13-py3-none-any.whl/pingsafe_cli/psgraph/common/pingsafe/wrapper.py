...
from pingsafe_cli.psgraph.common.typing import _ReducedScanReport
from pingsafe_cli.psgraph.common.util.file_utils import compress_string_io_tar
...
pingsafe_results_prefix = 'pingsafe_results'
check_reduced_keys = (
...
def reduce_scan_reports(scan_reports: list[Report]) -> dict[str, _ReducedScanReport]:
    """
    Transform pingsafe reports objects into compact dictionaries
    :param scan_reports: List of pingsafe output reports
...
def persist_checks_results(
    ...
    """
    Save reduced scan reports into pingsafe's platform
    ...
    for check_type, reduced_report in reduced_scan_reports.items():
        check_result_object_path = f'{full_repo_object_key}/{pingsafe_results_prefix}/{check_type}/checks_results.json'
...
def persist_run_metadata(
        run_metadata: dict[str, str | list[str]], s3_client: S3Client, bucket: str, full_repo_object_key: str, use_pingsafe_results: bool = True
) -> None:
    object_path = f'{full_repo_object_key}/{pingsafe_results_prefix}/run_metadata.json' if use_pingsafe_results else f'{full_repo_object_key}/run_metadata.json'
    try:
...
def enrich_and_persist_checks_metadata(
    ...
    """
    Save checks metadata into pingsafe's platform
    :return:
    """
    ...
    for scan_report in scan_reports:
        ...
        checks_metadata_object_path = f'{full_repo_object_key}/{pingsafe_results_prefix}/{check_type}/checks_metadata.json'
