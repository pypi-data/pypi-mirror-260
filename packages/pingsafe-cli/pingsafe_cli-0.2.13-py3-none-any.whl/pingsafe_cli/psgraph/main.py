...
import pingsafe_cli.psgraph.logging_init  # noqa  # should be imported before the others to ensure correct logging setup

from pingsafe_cli.psgraph.ansible.runner import Runner as ansible_runner
from pingsafe_cli.psgraph.argo_workflows.runner import Runner as argo_workflows_runner
from pingsafe_cli.psgraph.arm.runner import Runner as arm_runner
from pingsafe_cli.psgraph.azure_pipelines.runner import Runner as azure_pipelines_runner
from pingsafe_cli.psgraph.bitbucket.runner import Runner as bitbucket_configuration_runner
from pingsafe_cli.psgraph.bitbucket_pipelines.runner import Runner as bitbucket_pipelines_runner
from pingsafe_cli.psgraph.cloudformation.runner import Runner as cfn_runner
from pingsafe_cli.psgraph.common.pingsafe.bc_source import SourceTypes, BCSourceType, get_source_type, SourceType
...
from pingsafe_cli.psgraph.common.pingsafe.integration_features.features.repo_config_integration import \
    integration as repo_config_integration
from pingsafe_cli.psgraph.common.pingsafe.integration_features.features.suppressions_integration import \
    integration as suppressions_integration
from pingsafe_cli.psgraph.common.pingsafe.integration_features.features.custom_policies_integration import \
    integration as custom_policies_integration
from pingsafe_cli.psgraph.common.pingsafe.integration_features.integration_feature_registry import integration_feature_registry
from pingsafe_cli.psgraph.common.pingsafe.platform_integration import bc_integration
from pingsafe_cli.psgraph.common.pingsafe.integration_features.features.licensing_integration import integration as licensing_integration
from pingsafe_cli.psgraph.common.pingsafe.severities import BcSeverities
from pingsafe_cli.psgraph.common.goget.github.get_git import GitGetter
from pingsafe_cli.psgraph.common.output.baseline import Baseline
from pingsafe_cli.psgraph.common.pingsafe.check_type import pingsafe_runners
from pingsafe_cli.psgraph.common.runners.runner_registry import RunnerRegistry
from pingsafe_cli.psgraph.common.util import prompt
from pingsafe_cli.psgraph.common.util.banner import banner as pingsafe_banner, tool as pingsafe_tool
from pingsafe_cli.psgraph.common.util.config_utils import get_default_config_paths
from pingsafe_cli.psgraph.common.util.consts import PINGSAFE_RUN_SCA_PACKAGE_SCAN_V2
from pingsafe_cli.psgraph.common.util.docs_generator import print_checks
from pingsafe_cli.psgraph.common.util.ext_argument_parser import ExtArgumentParser
from pingsafe_cli.psgraph.common.util.runner_dependency_handler import RunnerDependencyHandler
from pingsafe_cli.psgraph.common.util.type_forcers import convert_str_to_bool
from pingsafe_cli.psgraph.contributor_metrics import report_contributor_metrics
from pingsafe_cli.psgraph.dockerfile.runner import Runner as dockerfile_runner
from pingsafe_cli.psgraph.github.runner import Runner as github_configuration_runner
from pingsafe_cli.psgraph.github_actions.runner import Runner as github_actions_runner
from pingsafe_cli.psgraph.gitlab.runner import Runner as gitlab_configuration_runner
from pingsafe_cli.psgraph.gitlab_ci.runner import Runner as gitlab_ci_runner
from pingsafe_cli.psgraph.helm.runner import Runner as helm_runner
from pingsafe_cli.psgraph.json_doc.runner import Runner as json_runner
from pingsafe_cli.psgraph.kubernetes.runner import Runner as k8_runner
from pingsafe_cli.psgraph.kustomize.runner import Runner as kustomize_runner
from pingsafe_cli.psgraph.runner_filter import RunnerFilter
from pingsafe_cli.psgraph.sca_image.runner import Runner as sca_image_runner
from pingsafe_cli.psgraph.sca_package.runner import Runner as sca_package_runner
from pingsafe_cli.psgraph.sca_package_2.runner import Runner as sca_package_runner_2
from pingsafe_cli.psgraph.secrets.runner import Runner as secrets_runner
from pingsafe_cli.psgraph.serverless.runner import Runner as sls_runner
from pingsafe_cli.psgraph.terraform.plan_runner import Runner as tf_plan_runner
from pingsafe_cli.psgraph.terraform.runner import Runner as tf_graph_runner
from pingsafe_cli.psgraph.terraform_json.runner import TerraformJsonRunner
from pingsafe_cli.psgraph.version import version
from pingsafe_cli.psgraph.yaml_doc.runner import Runner as yaml_runner
from pingsafe_cli.psgraph.bicep.runner import Runner as bicep_runner
from pingsafe_cli.psgraph.openapi.runner import Runner as openapi_runner
from pingsafe_cli.psgraph.circleci_pipelines.runner import Runner as circleci_pipelines_runner
from pingsafe_cli.psgraph.logging_init import log_stream as logs_stream
...
class PingSafe:
    ...
    def normalize_config(self) -> None:
        ...
        elif not self.config.bc_api_key and not self.config.include_all_pingsafe_policies:
            # makes it easier to pick out policies later if we can just always rely on this flag without other context
            logger.debug('No API key present; setting include_all_pingsafe_policies to True')
            self.config.include_all_pingsafe_policies = True
    ...
    def run(self, banner: str = pingsafe_banner, tool: str = pingsafe_tool, source_type: SourceType | None = None) -> int | None:
        self.run_metadata = {
            "pingsafe_version": version,
            "python_executable": sys.executable,
            "python_version": sys.version,
            "pingsafe_executable": sys.argv[0],
        ...
        try:
            ...
            if self.config.bc_api_key and not self.config.include_all_pingsafe_policies:
                if self.config.skip_download and not self.config.external_checks_dir:
                    print('You are using an API key along with --skip-download but not --include-all-pingsafe-policies or --external-checks-dir. '
                          'With these arguments, PingSafe cannot fetch metadata to determine what is a local PingSafe-only '
                          'policy and what is a platform policy, so no policies will be evaluated. Please re-run PingSafe '
                          'and either remove the --skip-download option, or use the --include-all-pingsafe-policies and / or '
                          '--external-checks-dir options.',
                          file=sys.stderr)
                    self.exit_run()
                elif self.config.skip_download:
                    print('You are using an API key along with --skip-download but not --include-all-pingsafe-policies. '
                          'With these arguments, PingSafe cannot fetch metadata to determine what is a local PingSafe-only '
                          'policy and what is a platform policy, so only local custom policies loaded with --external-checks-dir '
                          'will be evaluated.',
                          file=sys.stderr)
                else:
                    logger.debug('Using API key and not --include-all-pingsafe-policies - only running platform policies '
            ...
            # if a bc_api_key is passed it'll save it.  Otherwise it will check ~/.pingsafe/credentials
            ...
            runner_filter = RunnerFilter(
                ...
                include_all_pingsafe_policies=self.config.include_all_pingsafe_policies,
            ...
            if PINGSAFE_RUN_SCA_PACKAGE_SCAN_V2:
            ...
            if self.config.bc_api_key == '':
                ...
                if not bc_integration.is_token_valid(self.config.bc_api_key):
                    raise Exception('The provided API key does not appear to be a valid PingSafe API key or Prisma Cloud '
                                    'access key and secret key. For Prisma, the value must be in the form '
                                    'ACCESS_KEY::SECRET_KEY. For PingSafe, make sure to copy the token value from when you '
                ...
                try:
                    ...
                    bc_integration.setup_pingsafe_credentials(repo_id=self.config.repo_id,
                    ...
                    if bc_integration.prisma_api_url:
                        ...
                        message = 'An error occurred setting up the PingSafe platform integration. ' \
            ...
            # except Exception:
            #     if not self.config.include_all_pingsafe_policies:
                    # stack trace gets printed in the exception handlers above
                    # include_all_pingsafe_policies will always be set when there is no API key, so we don't need to worry about it here
                    # print('An error occurred getting data from the platform, including policy metadata. Because --include-all-pingsafe-policies '
                    #       'was not used, PingSafe cannot differentiate PingSafe-only policies from platform policies, and no '
                    #       'policies will get evaluated. Please resolve the error above or re-run with the --include-all-pingsafe-policies argument '
            ...
            if self.config.directory:
                ...
                for root_folder in self.config.directory:
                    ...
                    if self.config.create_baseline:
                        ...
                        created_baseline_path = os.path.join(os.path.abspath(root_folder), '.pingsafe.baseline')
    ...
    def print_results(
        ...
        if convert_str_to_bool(os.getenv("PINGSAFE_NO_OUTPUT", "False")):
...
# the flag/arg parsing moved to pingsafe_cli.psgraph/common/util/ext_argument_parser.py
