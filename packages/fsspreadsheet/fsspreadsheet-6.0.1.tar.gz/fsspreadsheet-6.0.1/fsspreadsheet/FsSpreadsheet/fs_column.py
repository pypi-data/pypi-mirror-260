    ...
    def create_at(index: int, cells: FsCellsCollection) -> FsColumn:
        def get_index_by(f: Callable[[Callable[[FsCell], int], IEnumerable_1[FsCell]], FsCell]) -> int:
            if length(FsCellsCollection__GetCellsInColumn_Z524259A4(cells, index)) == 0:
                ...
                def _arrow36(c: FsCell, f: Any=f) -> int:
                    return FsAddress__get_RowNumber(c.Address)

                return FsAddress__get_RowNumber(f(_arrow36)(FsCellsCollection__GetCellsInColumn_Z524259A4(cells, index)).Address)


        def _arrow39(projection: Callable[[FsCell], int]) -> Callable[[IEnumerable_1[FsCell]], FsCell]:
            def _arrow38(source_1: IEnumerable_1[FsCell]) -> FsCell:
                class ObjectExpr37:
                    @property
                    def Compare(self) -> Callable[[int, int], int]:
                        return compare_primitives

                return min_by(projection, source_1, ObjectExpr37())

            return _arrow38

        min_row_index: int = get_index_by(_arrow39) or 0
        def _arrow42(projection_1: Callable[[FsCell], int]) -> Callable[[IEnumerable_1[FsCell]], FsCell]:
            def _arrow41(source_2: IEnumerable_1[FsCell]) -> FsCell:
                class ObjectExpr40:
                    @property
                    def Compare(self) -> Callable[[int, int], int]:
                        return compare_primitives

                return max_by(projection_1, source_2, ObjectExpr40())

            return _arrow41

        max_row_index: int = get_index_by(_arrow42) or 0
