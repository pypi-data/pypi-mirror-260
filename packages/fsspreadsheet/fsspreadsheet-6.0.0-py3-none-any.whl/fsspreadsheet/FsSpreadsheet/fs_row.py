    ...
    def create_at(index: int, cells: FsCellsCollection) -> FsRow:
        def get_index_by(f: Callable[[Callable[[FsCell], int], IEnumerable_1[FsCell]], FsCell]) -> int:
            if length(FsCellsCollection__GetCellsInRow_Z524259A4(cells, index)) == 0:
                ...
                def _arrow28(c: FsCell, f: Any=f) -> int:
                    return FsAddress__get_ColumnNumber(c.Address)

                return FsAddress__get_ColumnNumber(f(_arrow28)(FsCellsCollection__GetCellsInRow_Z524259A4(cells, index)).Address)


        def _arrow31(projection: Callable[[FsCell], int]) -> Callable[[IEnumerable_1[FsCell]], FsCell]:
            def _arrow30(source_1: IEnumerable_1[FsCell]) -> FsCell:
                class ObjectExpr29:
                    @property
                    def Compare(self) -> Callable[[int, int], int]:
                        return compare_primitives

                return min_by(projection, source_1, ObjectExpr29())

            return _arrow30

        min_col_index: int = get_index_by(_arrow31) or 0
        def _arrow34(projection_1: Callable[[FsCell], int]) -> Callable[[IEnumerable_1[FsCell]], FsCell]:
            def _arrow33(source_2: IEnumerable_1[FsCell]) -> FsCell:
                class ObjectExpr32:
                    @property
                    def Compare(self) -> Callable[[int, int], int]:
                        return compare_primitives

                return max_by(projection_1, source_2, ObjectExpr32())

            return _arrow33

        max_col_index: int = get_index_by(_arrow34) or 0
