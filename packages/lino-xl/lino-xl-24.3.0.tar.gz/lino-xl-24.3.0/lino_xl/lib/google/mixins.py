        ...
        def sync_deleted_records(cls, resource, synchronizer):
            ...
            for obj in rt.models.google.DeletedEntry.objects.select_related(
                ...
                if (cs := synchronizer.stats.calendars
                    ...
                    logger.info(
                        f"google.Synchronizer ({synchronizer.user}): synchronized {cs} calendars so far."
                    )
        ...
        def sync_deleted_records(cls, resource, synchronizer):
            ...
            for obj in rt.models.google.DeletedEntry.objects.select_related(
                ...
                if (evs := synchronizer.stats.events
                    ...
                    logger.info(
                        f"google.Synchronizer ({synchronizer.user}): synchronized {evs} events so far."
                    )
        ...
        def get_outward_insert_update_queryset(cls, user):
            ...
            if not cal_cases:
                qs = qs.annotate(cal=models.Value(
                    settings.SITE.site_config.site_calendar.pk if settings.
                    SITE.site_config.site_calendar else None,
                    output_field=dd.ForeignKey(rt.models.cal.Calendar)))
            else:
                qs = qs.annotate(cal=models.Case(
                    *cal_cases,
                    default=settings.SITE.site_config.site_calendar.
                    pk if settings.SITE.site_config.site_calendar else models.
                    Value(None),
                    output_field=dd.ForeignKey(rt.models.cal.Calendar)))
            return qs.filter(
                ...
                for attendee in attendees:
                    ...
                    if guest_email := attendee.get("email") is not None:
                        ...
                        if partner is not None:
                            ...
                            guest.full_clean()
                            if guest.pk is None:
                                ar = Guest.get_default_table().request(
                                    user=user)
        ...
        def sync_deleted_records(cls, resource, synchronizer):
            ...
            for obj in rt.models.google.DeletedContact.objects.select_related(
                ...
                if (cns := synchronizer.stats.contacts
                    ...
                    logger.info(
                        f"google.Synchronizer ({synchronizer.user}): synchronized {cns} contacts so far."
                    )
        ...
        def insert_or_update_google_contact(cls, contact, synchronizer):
            ...
            for address in contact.get("addresses", []):
                ...
                if (cc := address.get("countryCode")) is not None:
                    ...
                    if country is None:
                        raise Exception(
                            f"No such country in the database {address['country']} ({address['countryCode']})"
                        )
            ...
            if person.pk is None:
                person.save_new_instance(
                    person.get_default_table().request(user=user))
            else:
                person.save()
            ...
            self.full_clean()
            if self.pk is None:
                self.save_new_instance(ar=self.get_default_table().request(
                    user=user))
