...
def deduplicate(
    ...
    from .tools.utils import equalize_markdown
    #
    embeddins_lib = {
    ...
    try:
        import json
        embeddins_lib["sentences"] = [json.loads(item) for item in data["documents"]]    
    except:
        pass
    #
        ...
        for jindex, score in enumerate(scores):
            if score > cutoff and index < jindex:
                ...
                if (isinstance(embeddins_lib["sentences"][index], str)):
                    col1, col2 = equalize_markdown(embeddins_lib["sentences"][index], embeddins_lib["sentences"][jindex])
                    record["Document Content #1"] = col1
                    record["Document Content #2"] = col2
                else:
                    for col in embeddins_lib["sentences"][0].keys():
                        col1, col2 = equalize_markdown(embeddins_lib["sentences"][index][col], embeddins_lib["sentences"][jindex][col])
                        record[f'{col} #1'] = col1
                        record[f'{col} #2'] = col2
                for col in embeddins_lib["metadata"][0].keys():
                    record[f'{col} #1'] = embeddins_lib["metadata"][index][col]
                    record[f'{col} #2'] = embeddins_lib["metadata"][jindex][col]
                pairs.append(record)
                # if columns:
                #     composite_score = 0
                #     for col in columns:
                #         if col in embeddins_lib["sentences"][index] and col in embeddins_lib["sentences"][jindex]:
                #             col1, col2 = equalize(embeddins_lib["sentences"][index][col],
                #                                   embeddins_lib["sentences"][jindex][col])
                #             composite_score += util.cos_sim(embedding.embed_documents([cleanse_data(col1)]),
                #                                             embedding.embed_documents([cleanse_data(col2)]))
                #     composite_score /= len(columns)
                ...
                #             record[f'{col} #2'] = col2
                #         else:
                #             record[f'{col} #1'] = embeddins_lib["sentences"][index][col]
                #             record[f'{col} #2'] = embeddins_lib["sentences"][jindex][col]
                # else:
                #     # (col1, col2) = equalize(embeddins_lib["sentences"][index], embeddins_lib["sentences"][jindex])
                #     record['Step #1'] = embeddins_lib["sentences"][index]
                #     record['Step #2'] = embeddins_lib["sentences"][jindex]
