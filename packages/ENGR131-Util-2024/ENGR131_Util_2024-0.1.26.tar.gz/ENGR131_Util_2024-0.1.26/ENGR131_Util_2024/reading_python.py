...
class ReadingPython():
   ...
   def on_submit_button_clicked(self, b):
      ...
      self.execution_points = self.points[1]/(len(self.rows)*4)
      ...
      with self.printed_output:
         ...
         def checker(value, solution, count):
               ...
               # makes sure not to count unnecessary rows
               if count > num_true_responses * 4:
         ...
         # Sets the question count to i + 1
         count = i + 1
         ...
         for j, value in enumerate(self.output_execution):  
            ...
            if value[0] == num:
               ...
               count = checker(value[1], variable, count)
               count = checker(value[2], true_value, count)
               count = checker(value[3], dataType, count)
