...
def ddic(
    ...
    if PhiField is None:
        PhiField = numpy.zeros([numberOfLabels, 4, 4])
    ...
    global _correlateOneLabel

    def _correlateOneLabel(label):
    ...
    print("\n\tStarting Discrete DIC (with {} process{})".format(processes, "es" if processes > 1 else ""))
    widgets = [
    ...
    with multiprocessing.Pool(processes=processes) as pool:
        for returns in pool.imap_unordered(_correlateOneLabel, labelsToCorrelate):
            finishedLabels += 1
