from .subcommands import create, delete, open_, init
...
app.command("open", epilog=EPILOG)(open_.open_)
...
def dndgmod_callback(
    ...
    if not (mods_directory.exists() or ctx.invoked_subcommand == "init"):
        raise FileNotFoundError(f"Directory {mods_directory} doesn't exist! Try running `dndgmod init`")
