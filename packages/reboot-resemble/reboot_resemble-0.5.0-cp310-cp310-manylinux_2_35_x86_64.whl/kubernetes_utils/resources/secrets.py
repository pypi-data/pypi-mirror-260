...
class AbstractSecrets:
    ...
    async def _create(
        self, *, namespace: str, body: V1Secret, retry_api_exception: bool
    ) -> None:
    ...
    async def write(self, *, namespace: str, name: str, data: bytes) -> None:
        ...
        try:
            await self._create(
                namespace=namespace,
                body=secret,
                # See the note above: we're expecting a `Conflict` in some cases.
                retry_api_exception=False,
            )
...
class Secrets(LoggerMixin, AbstractSecrets):
    ...
    async def _create(
        ...
        return await self._apis.retry_api_call(
            retryable, retry_api_exception=retry_api_exception
        )
    ...
    async def _replace(
        ...
        return await self._apis.retry_api_call(
            retryable, retry_api_exception=True
        )
    ...
    async def read(self, *, namespace: str, name: str) -> bytes:
        ...
        try:
            v1_secret = await self._apis.retry_api_call(
                retryable, retry_api_exception=True
            )
        except ApiException as e:
