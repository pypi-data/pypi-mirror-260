    ...
    def compute_refstate_from_im_stack(self, kernel,
        ...
        if s3_dictionary is None:
            s3_dictionary = {'s3_access_key_id': None, 's3_secret_access_key': None,
                             ...
                             's3_path_2_folder': None}

        s3_flag = False
        im_extensions = [im_extensions] if not isinstance(im_extensions, list) else im_extensions
        opt_dispplay_and_verbose = [self.options['display'], self.options['verbose']]
        self.options['display'], self.options['verbose'] = False, False
        if opt_dispplay_and_verbose[1]:
        ...
        phi_ref, mods = self.compute_phases_mod(img_ref, kernel, roi_coef=roi_coef)
        ...
        mod = np.zeros(img_ref.shape, dtype=complex)
        for i, vec_dir in enumerate(self.vec_dir()):
            mod += mods[i]*vec_dir
        mod_abs = np.abs(mod*self.roi)
        loc_roi = ~(cv2.blur((255*(mod_abs <= 0.5*mod_abs.max())).astype('uint8'), (25, 25)) > 0)
        ...
        for i, img_name in enumerate(im_stack[1:]):
            ...
            phi_loc = self.compute_phases_mod(img_loc, kernel)[0]
            ...
            uxy_res = uxy_loc
            uxy_res[self.roi] = uxy_res[self.roi]-uxy_loc_rbm
            dude2, dude1 = np.gradient(uxy_loc)
        ...
        self.options['display'] = opt_dispplay_and_verbose[0]
        self.options['verbose'] = opt_dispplay_and_verbose[1]
        ...
        if self.options['verbose']:
            print("       ┌───────────────────────────────────────────────┐\n"
                  f"       │ Equivalent stds : .std(u1) = {stats['std_eq_u1']:.2E} [px]    │\n",
...
def provide_s3_path(s3_dictionary, im_extensions, verbose):
