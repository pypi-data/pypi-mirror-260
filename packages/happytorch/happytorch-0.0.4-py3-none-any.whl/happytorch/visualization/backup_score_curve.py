...
def load_test_data(label_dir, dataset_type, data_folder):
    ...
    datadata = np.load('/userHome/nemo/z_visual/avenue.npz')

    preds = datadata['arr_0']
    print(preds.shape, '-------', labels.shape)
    # preds = np.roll(preds, shift=20)
    ...
    for video in videos_list:
        ...
        temp = preds[label_length: videos[video_name]["length"] + label_length]
        videos[video_name]['pred'] = temp
        
        print(len(videos[video_name]["label"]), len(videos[video_name]['pred']))
...
def img_movies(img):
    movies = cv2.imread("/userHome/nemo/z_visual/movie1.png", cv2.IMREAD_UNCHANGED)
...
if __name__ == "__main__":
    ...
    for video in tqdm(videos_list):
        ...
        ax2 = plt.subplot(212)
        plt.plot(x, p, color='#001bf8')
        for s,e in zip(start_a, end_a):
            plt.axvspan(s, e, alpha=1.0, color='#b9cae7')
        ...
        plt.yticks([])
        fig.tight_layout()
        fig.savefig('/userHome/nemo/z_visual/avenue/label_{}.png'.format(video_name))
        plt.close()
        plt.close()
