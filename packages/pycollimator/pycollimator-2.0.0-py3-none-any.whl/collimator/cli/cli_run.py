...
@click.option(
    ...
    show_default=True,
    default=".",
    help="Directory where Wildcat runs, contains any inputs files needed for operation",
)
@click.option(
    "--datadir",
    type=click.Path(file_okay=False, dir_okay=True),
    help="Directory for signal output files.",
)
@click.option(
    "--logsdir",
    type=click.Path(file_okay=False, dir_okay=True),
...
def collimator_run(
    ...
    loaded_model = collimator.load_model(
        modeldir, model=model, datadir=datadir, logsdir=logsdir
    )
...
def run(modeldir, model_json, output_dir):
    ...
    model = collimator.load_model(
        modeldir, model_json, datadir=datadir, logsdir=logsdir
    )
