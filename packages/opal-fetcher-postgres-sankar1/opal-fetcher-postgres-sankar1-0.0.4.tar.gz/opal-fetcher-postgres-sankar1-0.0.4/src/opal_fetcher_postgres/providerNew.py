...
class PostgresConnectionNewParams(BaseModel):
    """
...
class PostgresFetcherNewConfig(FetcherConfig):
    """
    Config for PostgresFetchProviderNew, instance of `FetcherConfig`.
    ...
    fetcher: str = "PostgresFetchProviderNew"
    connection_params: Optional[PostgresConnectionNewParams] = Field(
        None,
...
class PostgresFetchEventNew(FetchEvent):
    """
    ...
    fetcher: str = "PostgresFetchProviderNew"
    config: PostgresFetcherNewConfig = None


class PostgresFetchProviderNew(BaseFetchProvider):
    """
    ...
    - parse_event() - this method gets a `FetchEvent` object and must transform this object to *your own custom event class*.
        ...
        - Notice that `PostgresFetchEventNew` is the custom event class
    ...
    def __init__(self, event: PostgresFetchEventNew) -> None:
        if event.config is None:
            event.config = PostgresFetcherNewConfig()
        super().__init__(event)
        self._connection: Optional[asyncpg.Connection] = None
        self._transaction: Optional[Transaction] = None

    def parse_event(self, event: FetchEvent) -> PostgresFetchEventNew:
        return PostgresFetchEventNew(**event.dict(exclude={"config"}), config=event.config)

    async def __aenter__(self):
        self._event: PostgresFetchEventNew  # type casting
