...
def report_outliers(
    ...
    if threshold > max_outlier_density:
        raise ValueError(f"The given threshold ({threshold}) is greater than the maximum outlier density ({max_outlier_density}).")
...
def find_cluster(
    ...
    entry = table[table['CLUSTERS'] == cluster]
...
def find_cluster_by_sernum(
    ...
    cluster_label = table[table['SERNUM'] == sernum].loc(['CLUSTERS'])[0]

    # Filter table for entry
    entries = table[table['CLUSTERS'] == cluster_label]
...
def find_differences(
    ...
    Args:
        ...
        sernum_values (Union[str, list]): The set of serial number(s) from the table that will be checked.
...
def filter_by_Util(
    ...
    Args:
        ...
        values (Union[str, list]): The list of values to check for
        table (Union[pd.DataFrame, str]): The DataFrame or file path to the DataFrame to filter.
        archive_path (str, optional): Path to a CSV file where the filtered DataFrame will be archived.
...
def to_strlist(
    ...
    Args:
        values (Union[str, List[str]]): Either a list of strings, or a string

    Returns:
        values (List[str]): The input list, or string converted to a list.

    Raises:
        ValueError: If the input is not a list of strings or an individual string.
    """
