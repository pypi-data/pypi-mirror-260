...
def convert_delimited_file(delimited_file_path, f4_file_path, index_columns=[], delimiter="\t", comment_prefix="#", compression_type=None, num_parallel=1, tmp_dir_path=None, compress_intermediate_data_files=True, verbose=False):
    ...
    if index_columns:
        build_indexes(f4_file_path, tmp_dir_path2, index_columns, num_rows, line_length, num_parallel, compress_intermediate_data_files, verbose)
...
def build_indexes(f4_file_path, tmp_dir_path, index_columns, num_rows, line_length, num_parallel, compress_intermediate_data_files, verbose=False):
    index_info_dict = {}

    if isinstance(index_columns, str):
        reverse_statuses = build_index(f4_file_path, tmp_dir_path, 0, [index_columns], num_rows, line_length, compress_intermediate_data_files, verbose)
        ...
        keys = joblib.Parallel(n_jobs=num_parallel)(
            joblib.delayed(build_index_parallel)(f4_file_path, tmp_dir_path, num_rows, line_length, i, index_column, compress_intermediate_data_files, verbose)
            for i, index_column in enumerate(index_columns)
        )
...
def build_index(f4_file_path, tmp_dir_path, index_number, index_columns, num_rows, line_length, compress_intermediate_data_files, verbose):
    ...
    with get_temp_file_handle(get_data_path(tmp_dir_path, "data"), "rb", compress_intermediate_data_files) as file_handle:
        for row_index in range(num_rows):
            ...
            for i, index_column in enumerate(index_columns):
                ...
                this_start_pos = start_pos + start_coords[i]
                this_end_pos = start_pos + end_coords[i]
...
def build_index_parallel(f4_file_path, tmp_dir_path, num_rows, line_length, index_number, index_column, compress_intermediate_data_files, verbose):
    index_column_list = [index_column] if isinstance(index_column, str) else index_column
    reverse_statuses = build_index(f4_file_path, tmp_dir_path, index_number, index_column_list, num_rows, line_length, compress_intermediate_data_files, verbose)
