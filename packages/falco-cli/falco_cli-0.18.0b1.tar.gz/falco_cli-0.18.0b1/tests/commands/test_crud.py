...
def fix_users_import():
    ...
    types.write_text(
        types.read_text().replace("myproject.users", "django.contrib.auth")
    )
...
def test_crud_entry_point_login(
    django_project, runner: CommandRunner, set_git_repo_to_clean
):
    install_crud_utils(runner)
...
def test_crud_config_pyproject_skip_git_check_set(
    django_project, runner: CommandRunner
):
...
def test_crud_config_pyproject_blueprints(django_project, runner: CommandRunner):
    ...
    create_pyproject_crud_config(
        blueprints=str(Path("blueprints")), skip_git_check=True
    )
    install_crud_utils(runner)
    runner.invoke("crud", "blog.post")
    views = (Path("blog") / "views.py").read_text()
