...
def make_tech_plot(ticker, df, tech_type, TECH_LIST, idx):
    ...
    if tech_type in ['sma','ema','macd','adx','ppo','psar','ichmoku','stoch','roc','ao',
        ...
        slope_before = base.trend_detector(_df, 'Ewm', base.get_working_day_before(5))
        
        # Here is Buttom, 분할매수 시작, slope 이 마이너스(-)에서 0 로 전환된 날 리포트 : 2
        _df['pivot'] = np.where((slope_before >= -0.02 & slope_before <= 0.02) & (_df['ewm'].shift(1) < _df['ewm']), 2, 0)
        # 분할매수 끝, slope 이 플러스(+) 인 상태에서, 이동평균값이 기준선 0 위로 전환된 날 리포트 : 1 
        _df['pivot'] = np.where((slope_before > 0.02) & (_df['ewm'].shift(1) < _df['ewm']), 1, _df['pivot'])

        # Here is Top, 분할매도 시작, slope 이 플러스(+)에서 0 으로 전환된 날 리포트 : -2
        _df['pivot'] = np.where((slope_before >= -0.02 & slope_before <= 0.02) & (_df['ewm'].shift(1) > _df['ewm']), -2, _df['pivot'])
...
def sma(ticker, df, tech_type, TECH_LIST, idx):
    ...
    except TypeError as e:  # lastest_date 가 짧아 np.polyfit 함수 오류로 인한 수작업 기울기 구하기.
