...
def make_tech_plot(ticker, df, tech_type, TECH_LIST, idx):
    ...
    if tech_type in ['sma','ema','macd','adx','ppo','psar','ichmoku','stoch','roc','ao',
        ...
        slope_before = base.get_working_day_before(5)

        # Here is Buttom, 분할매수 시작, slope 이 마이너스(-)에서 0 로 전환된 날 리포트 : 2
        _df['pivot'] = np.where((base.trend_detector(_df, 'ewm', slope_before) > 0) & (_df['ewm'].shift(1) < _df['ewm']), 2, 0)
        # 분할매수 끝, slope 이 플러스(+) 인 상태에서, 이동평균값이 기준선 0 위로 전환된 날 리포트 : 1 
        _df['pivot'] = np.where((base.trend_detector(_df, 'ewm', slope_before) > 0) & (_df['ewm'].shift(1) < _df['ewm']), 1, _df['pivot'])

        # Here is Top, 분할매도 시작, slope 이 플러스(+)에서 0 으로 전환된 날 리포트 : -2
        _df['pivot'] = np.where((base.trend_detector(_df, 'ewm', slope_before) < 0) & (_df['ewm'].shift(1) > _df['ewm']), -2, _df['pivot'])
