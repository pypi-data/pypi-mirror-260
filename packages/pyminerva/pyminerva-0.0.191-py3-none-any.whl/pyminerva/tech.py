...
def make_tech_plot(ticker, df, tech_type, TECH_LIST, idx):
    ...
    if tech_type in ['sma','ema','macd','adx','ppo','psar','ichmoku','stoch','roc','ao',
        ...
        _df['SellAndBuy'] = _df['Close'].ewm(span=5, adjust=False, min_periods=5).mean()
        ...
        _df['pivot'] = np.where((slope_before <= abs(0.02)) & (_df['SellAndBuy'].shift(1) < _df['SellAndBuy']) & \
                                _df['SellAndBuy'] < 0, 2, 0)
        # 분할매수 끝, slope 이 플러스(+) 인 상태에서, 이동평균값이 기준선 0 위로 전환된 날 리포트 : 1 
        _df['pivot'] = np.where((slope_before > 0.02) & (_df['SellAndBuy'].shift(1) < _df['SellAndBuy']) & \
