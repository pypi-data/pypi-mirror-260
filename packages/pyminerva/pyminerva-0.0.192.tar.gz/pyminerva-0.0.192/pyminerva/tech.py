...
def make_tech_plot(ticker, df, tech_type, TECH_LIST, idx):
    ...
    if tech_type in ['sma','ema','macd','adx','ppo','psar','ichmoku','stoch','roc','ao',
        ...
        if tech_type == 'bbands':  # 
            ...
            # print(_df)
            _df['pivot'] = np.where((_df[tech_type] >= _df['DCU_20_20']) & (slope_before <= abs(permit_range)) \
                                    (_df[tech_type].shift(1) <= _df['DCU_20_20']).shift(1), 2, 0)  # 분할매도 시작
            _df['pivot'] = np.where((_df[tech_type] <= _df['DCL_20_20']) & (slope_before <= abs(permit_range)) \
                                    (_df[tech_type].shift(1) >= _df['DCL_20_20']).shift(1), -2, _df['pivot']) # 분할매수 시작
        elif tech_type == 'kc':  # 
            plt.plot(_df.index, _df['KCUe_20_2'], color='orange')  # Upper line
            plt.plot(_df.index, _df['KCLe_20_2'], color='orange')  # Lower line
            _df['pivot'] = np.where((_df[tech_type] > _df['KCUe_20_2']) & (slope_before <= abs(permit_range)) \
                                    (_df[tech_type].shift(1) <= _df['KCUe_20_2']).shift(1), 2, 0)  # 분할매도 시작
            _df['pivot'] = np.where((_df[tech_type] < _df['KCLe_20_2']) & (slope_before <= abs(permit_range)) \
                                    (_df[tech_type].shift(1) >= _df['KCLe_20_2']).shift(1), -2, _df['pivot']) # 분할매수 시작
