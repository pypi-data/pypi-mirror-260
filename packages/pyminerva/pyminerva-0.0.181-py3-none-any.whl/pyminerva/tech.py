...
def get_tech_data(ticker, window):
    ...
    # Financial Data
    pnl = ticker_obj.financials
    bs = ticker_obj.balancesheet
...
def make_tech_plot(ticker, df, tech_type, TECH_LIST, idx):
    ...
    if tech_type in ['sma','ema','macd','adx','ppo','psar','ichmoku','stoch','roc','ao',
        ...
        _df['ewm'] = _df['Close'].ewm(span=5, adjust=False, min_periods=5).mean()

        # Here is Buttom, 분할매수 시작, slope 이 마이너스(-)에서 0 로 전환된 날 리포트 : 2
        _df['pivot'] = np.where((base.trend_detector(_df, 'ewm', base.get_working_day_before(5)) > 0) & (_df['ewm'].shift(1) < _df['ewm']), 2, 0)
        # 분할매수 끝, slope 이 플러스(+) 인 상태에서, 이동평균값이 기준선 0 위로 전환된 날 리포트 : 1 
        _df['pivot'] = np.where((base.trend_detector(_df, 'ewm', base.get_working_day_before(5)) >= 0) & (_df['ewm'].shift(1) <= _df['ewm']), 1, 0)

        # Here is Top, 분할매도 시작, slope 이 플러스(+)에서 0 으로 전환된 날 리포트 : -2
        _df['pivot'] = np.where((base.trend_detector(_df, 'ewm', base.get_working_day_before(5)) < 0) & (_df['ewm'].shift(1) > _df['ewm']), -2, _df['pivot'])
        # 분할매도 끝, slope 이 마이너스(-)인 상태에서 , 이동평균값이 기준선 0 아래로 전환된 날 리포트: -1
        _df['pivot'] = np.where((base.trend_detector(_df, 'ewm', base.get_working_day_before(5)) <= 0) & (_df['ewm'].shift(1) >= _df['ewm']), -1, _df['pivot'])
    ...
    if _1.empty and _3.empty:
        latest_date = '2024-12-31' # 임시
        decision = '현상태 유지'
    elif _1.empty:
...
def sma(ticker, df, tech_type, TECH_LIST, idx):
    ...
    print(from_latest_date_slope)
    print(from_5day_ago_slope)
    result = (from_latest_date_slope / from_5day_ago_slope) * 5
    
    return df, latest_date, result
