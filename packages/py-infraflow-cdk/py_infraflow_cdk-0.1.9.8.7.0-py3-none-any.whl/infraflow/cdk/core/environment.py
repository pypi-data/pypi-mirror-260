...
class Env(IEnv):
    ...
    def service_endpoints(self, services: list[InterfaceVpcEndpointAwsService]) -> list[InterfaceVpcEndpoint]:
        print(f"*** service names: {[s.name for s in services]}, and shortnames: {[s.shortName for s in services]}")
        return self.interface_endpoints(service_names=[s.shortName for s in services])
