...
class FengChao:
    ...
    @retry(stop=stop_after_attempt(Comments.RETRY_NUM))
    def do(self, method, url, payload=None, headers=None, stream=False) -> requests.Response:
        ...
        response = requests.request(method, url, json=payload, headers=headers, stream=stream, timeout=Comments.TIMEOUT)
        return response
