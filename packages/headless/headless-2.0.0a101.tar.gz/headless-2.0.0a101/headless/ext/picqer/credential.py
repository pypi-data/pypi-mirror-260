...
from headless.core import BaseRotatingCredential
from headless.types import BasicAuth
from headless.types import IRequest


class PicqerCredential(BaseRotatingCredential):
    api_email: str

    def __init__(self, api_email: str, api_keys: str):
        super().__init__(
            credentials=[
                BasicAuth(api_key, 'x')
                for api_key in str.split(api_keys, ':')
            ]
        )
