...
class ServerMetadata(pydantic.BaseModel):
    ...
    async def discover(
        ...
        if response.status_code >= 400:
            client.logger.warning("Unable to discover server metadata (url: %s)", url)
            return None
        try:
            return cls.parse_obj(await response.json())
        except pydantic.ValidationError:
            client.logger.warning("Invalid response from metadata endpoint (url: %s)", url)
            return None
        except ValueError:
            client.logger.warning("Unable to decode response as JSON (url: %s)", url)
            return None