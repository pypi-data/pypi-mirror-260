...
class DeployUpdateModelDeploymentRequest(object):
    ...
    def update_mask(self):
        ...
        The fields to update.  An empty value, or value of `*`, implies all updatable fields should apply.  The following fields are supported: `deployment.kubernetes_resource_spec` `deployment.kubernetes_configuration_shortcut` `deployment.security` `deployment.monitor_disable` Allowed security updates are: - no security -> plain_text/hash - plain_text -> hash - hash -> plain_text - old plain_text -> new plain_text - old hash -> new hash All other cases would be ignored and updated passphrase hash must not be blank.  # noqa: E501
