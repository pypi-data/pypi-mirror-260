...
class FixedPointType:
    ...
    def __getitem__(self, item):
        return FixedPointType(self.bw, self.bp, self.bdata.__getitem__(item), disbin=True)
    ...
    @property
    def conf(self):
        return self.bw, self.bp

    def mask_bitwidth(self, data: np.ndarray = None) -> np.ndarray:
        """
        ...
        if data is None:
            data = self.bdata
        return np.sign(data) * (np.abs(data) & mask)
