...
from mbodied.registry import REGISTRY
...
from mbodied.abstract.action import Action
...
def get_demo_images(output=None) -> np.ndarray:
    ...
    filenames = [
        files("mbodied").joinpath("demo_imgs/gripper_far_from_grasp.png"),
        files("mbodied").joinpath("demo_imgs/gripper_mid_to_grasp.png"),
        files("mbodied").joinpath("demo_imgs/gripper_almost_grasp.png"),
    ]
...
if __name__ == "__main__":
    if "--help" in sys.argv or "-h" in sys.argv:
        ...
        python3 -m mbodied.demo --model_type=octo --instruction="pick block"
        """
        )
    app.run(main)
else:
